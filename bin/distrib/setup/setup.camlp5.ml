  ...
  let rec loop comm =
    ...
    | (k, s) :: env ->
        ...
        if k = "" || s = "" then loop comm env
          ...
          then loop (comm ^ " -o " ^ stringify s ^ " > " ^ stringify s) env
        else loop comm env
    | [] -> comm
  in
  loop ""
...
let comm_log conf =
  ...
  in
  let comm_log =
    if bname = "" then "comm.log"
    else
      if !Path.reorg then 
        String.concat Filename.dir_sep
          [bname ^ ".gwb"; "etc"; "comm.log"]
      else "comm.log"
  in
  comm_log
...
let gwc conf =
  let rc =
    ...
    let commnd = comm ^ parameters conf.env in
    ...
    if rc <> 0 then rc
      ...
      let c =
        ...
        List.fold_left
          ...
          "" bases ^
        " -f -o " ^ out_file ^ " > " ^ (comm_log conf) ^ "2>&1"
      in
      Printf.eprintf "$ %s\n" c; flush stderr; Sys.command c
  in
  if rc > 1 then print_file conf "bso_err.htm"
