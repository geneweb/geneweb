...
type person = (iper, Adef.istr) gen_person
type ascend = ifam gen_ascend
type union = ifam gen_union
type family = (iper, Adef.istr) gen_family
type couple = iper gen_couple
type descend = iper gen_descend
  ...
  match !month_number_dates with
    MonthNumberHappened s ->
      Printf.fprintf !log_oc
        "  Warning: the file holds dates with numbered months \
         (like: 12/05/1912).\n\n  \
         GEDCOM standard *requires* that months in dates be identifiers. The \
         correct form for this example would be 12 MAY 1912 or 5 DEC \
         1912.\n\n  \
...
type gen =
  { g_per : (string, person, ascend, union) choice3 tab;
    ...
    g_hstr : (string, Adef.istr) Hashtbl.t;
...
let find_event_witness gen r =
  let rec find_witnesses =
    ...
    | r :: asso_l ->
        ...
        else begin r.rused <- false; find_witnesses asso_l end
  in
  let witnesses =
    match find_all_fields "ASSO" r.rsons with
      [] -> []
...
let find_pevent_name_from_tag gen tag =
  match tag with
    "BIRT" -> Epers_Birth
  | "BAPT" -> Epers_Baptism
  | "DEAT" -> Epers_Death
...
let primary_pevents =
  ["BAPT"; "BAPL"; "BARM"; "BASM"; "BIRT"; "BLES"; "BURI"; "CENS"; "CONF";
   "CONL"; "CREM"; "DEAT"; "EDUC"; "EMIG"; "ENDL"; "FCOM"; "GRAD"; "IMMI";
   "NATU"; "ORDN"; "PROP"; "RETI"; "RESI"; "SLGS"; "SLGC"; "WILL"]

let treat_indi_pevent gen r =
  let prim_events =
    List.fold_left
      (fun events tag ->
         List.fold_left
...
let add_indi gen r =
  ...
  let (first_name, surname) =
    (if first_name = "" then !default_name else first_name),
    (if surname = "" then !default_name else surname)
  in
  let qualifier =
    match name_sons with
      Some n ->
        ...
        (fun evt ->
           let name =
             ...
             | Efam_Marriage | Efam_NoMarriage | Efam_NoMention |
               Efam_Engage | Efam_Divorce | Efam_Separated | Efam_Annulation |
               Efam_MarriageBann | Efam_MarriageContract |
               Efam_MarriageLicense | Efam_PACS | Efam_Residence as e ->
                 e
           in
           let witl =
             List.map
    ...
    | rl ->
        ...
        let rl =
          List.map
            ...
            rl
        in
        rev_list := Gwcomp.Relations (x, sex, rl) :: !rev_list
  done;
  for i = 0 to Array.length pa - 1 do
    let p = pa.(i) in
    ...
    if n = "" then ()
      ...
      rev_list := Gwcomp.Notes (pk, n) :: !rev_list
  done;
  if g_bnot = "" then ()
  else rev_list := Gwcomp.Bnotes ("", g_bnot) :: !rev_list;
  List.rev !rev_list

(* Providing gw_syntax stream *)

let next_family_fun_templ gw_syntax in_file fi =
  fi.Db2link.f_curr_gwo_file <- in_file;
  let gw_syntax = ref gw_syntax in
  fun () ->
    match !gw_syntax with
      x :: l -> gw_syntax := l; Some x
    | [] -> None
...
let speclist =
  ["-o", Arg.String (fun s -> out_file := s),
   ...
   "-dn", Arg.String (fun s -> default_name := s),
   " - Default name -\n       \
...
let errmsg = "Usage: ged2gwb2 [<ged>] [options] where options are:"
...
let main () =
  ...
  let gw_syntax =
    ...
    Gc.compact ();
    let arrays = make_subarrays arrays in flush !log_oc; make_gwsyntax arrays
  in
  Lock.control (Mutil.lock_file !out_file) false
    (fun () ->
       let next_family_fun = next_family_fun_templ gw_syntax !in_file in
       if Db2link.link next_family_fun bdir then ()
       else (Printf.eprintf "*** database not created\n"; flush stderr; exit 2))
