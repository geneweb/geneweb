...
let make_base state =
  ...
  if not state.just_comp then (
    ...
    if (not state.force) && Sys.file_exists bdir then (
      ...
      flush stdout;
      exit 2);
    Lock.control (Mutil.lock_file state.out_file)
      false ~onerror:Lock.print_error_and_exit (fun () ->
        ...
        if Db1link.link state next_family_fun bdir then ()
        else (
