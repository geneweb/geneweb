...
let default_opts, default_state = make_opts_and_state ()
...
let print_location pos =
  Printf.fprintf !opts.log_oc "File \"%s\", line %d:\n" !opts.in_file pos
...
let print_bad_date pos d =
  if !state.bad_dates_warned then ()
    ...
    begin
      ...
      print_location pos;
      Printf.fprintf !opts.log_oc "Can't decode date %s\n" d;
      flush !opts.log_oc
    end
...
let check_month m =
  if m < 1 || m > 12 then
    begin
      Printf.fprintf !opts.log_oc "Bad (numbered) month in date: %d\n" m;
      flush !opts.log_oc
    end
...
let make_date n1 n2 n3 =
  let n3 =
    if !opts.no_negative_dates then
      match n3 with
        Some n3 -> Some (abs n3)
      | None -> None
    else n3
...
let output_pindex i str =
  if !opts.track_ged2gw_id then Printf.printf "IDGED2IDPERS %i %s\n" i str
...
(* TODO use Geneweb.Update.infer_death *)
let infer_death birth bapt =
  match birth, bapt with
  | Some (Dgreg (d, _)), _ ->
    let a = this_year - d.year in
    if a > !opts.dead_years then DeadDontKnowWhen
    else if a < !opts.alive_years then NotDead
    else DontKnowIfDead
  | _, Some (Dgreg (d, _)) ->
    let a = this_year - d.year in
    if a > !opts.dead_years then DeadDontKnowWhen
...
let particle s i =
  List.exists (string_ini_eq s i) !opts.particles
         ...
         (fun r lines ->
            ...
            if r.rlab = "NOTE" && r.rval <> "" && r.rval.[0] = '@' then
              ...
              | None ->
                  ...
                  Printf.fprintf !opts.log_oc "Note %s not found\n" addr;
                  flush !opts.log_oc;
                  lines
...
let treat_source gen r =
  if String.length r.rval > 0 && r.rval.[0] = '@' then
    ...
    | None ->
        print_location r.rpos;
        Printf.fprintf !opts.log_oc "Source %s not found\n" r.rval;
        flush !opts.log_oc;
        "", []
  else strip_spaces r.rval, r.rsons
...
let applycase_surname s =
  match !opts.case_surnames with
    NoCase -> s
  | LowerCase -> capitalize_name s
  | UpperCase ->
      if !opts.charset = Utf8 then uppercase_name s else String.uppercase_ascii s
...
let add_indi gen r =
  ...
  let (first_name, surname, occ, public_name, first_names_aliases) =
    ...
    | Some n ->
      ...
      let (f, fal) =
        match !opts.first_names_brackets with
          Some (bb, eb) ->
    ...
    Stream.from
      (fun _ ->
         try
           ...
           if c = '\n' then !state.line_cnt <- succ !state.line_cnt;
           Some c
...
let make_base opts_ state_ =
  (* set global state;
     we use a global ref for the state because I couldn't manage to do without it because of camlp5 *)
  opts := opts_;
  state := state_;
  Secure.set_base_dir (Filename.dirname !opts.out_file);
  let arrays = make_arrays !opts.in_file in
  Gc.compact ();
  let arrays = make_subarrays arrays in
  finish_base arrays ;
  Gwdb.make !opts.out_file !opts.particles arrays
