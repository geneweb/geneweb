...
let make_conf from_addr request script_name env =
  let secret_salt =
    try Some (Unix.getenv "SECRET_SALT") with Not_found -> Some "100"
  in
...
let main () =
  ...
  let speclist =
    [
      ...
      ("-syslog", Arg.Unit (fun () -> Logs.syslog_flag := true), "Enable syslog");
      ( "-log_level",
      ...
      ( "-cache-in-memory",
        Arg.String
          (fun s ->
            ...
            else failwith "-cache-in-memory option unavailable for this build."),
