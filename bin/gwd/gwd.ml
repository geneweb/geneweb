...
let log_and_robot_check conf auth from request script_name contents =
  if !robot_xcl = None then
    ...
    Lock.control (!GWPARAM.adm_file "gwd.lck") true ~onerror:ignore (fun () ->
        let tm = Unix.time () in
...
let geneweb_server () =
  ...
  if not auto_call then (
    ...
    with Sys_error e ->
      GWPARAM.syslog `LOG_CRIT
        (Format.sprintf "failure creating %s (%s)\n" !GWPARAM.cnt_dir e));
...
let main () =
  ...
  let speclist =
    [
      ...
      ( "-log_level",
        Arg.Set_int Geneweb.GWPARAM.verbosity,
        {|<N> Send messages with severity <= <N> to syslog (default: |}
        ^ string_of_int !Geneweb.GWPARAM.verbosity
        ^ {|).|} );
      ( "-robot_xcl",
  ...
  if !use_auth_digest_scheme && !force_cgi then
    Geneweb.GWPARAM.syslog `LOG_WARNING
      "-digest option is not compatible with CGI mode.\n";
  (if !images_dir <> "" then
   let abs_dir =
