...
let geneweb_cgi ~predictable_mode ~loaded_plugins ~secret_salt addr script_name
    contents =
  ...
  connection ~cgi:true ~predictable_mode ~loaded_plugins ~secret_salt conn
...
let geneweb_server ~predictable_mode ~loaded_plugins ?interface ~port
  ...
  Server.start ?addr:interface ~port ~timeout:!conn_timeout
    ...
    (connection ~cgi:false ~predictable_mode ~loaded_plugins ~secret_salt)

let main ~plugins ?interface ~port ~predictable_mode ~secret_salt ~cgi () =
  let gwd_cmd =
...
let windows_worker ~plugins ~interface ~port ~predictable_mode () =
  ...
  main ~plugins ~interface ~port ~predictable_mode ~secret_salt ~cgi:false ()

let master ~plugins ~interface ~port ~predictable_mode ~cgi () =
  let secret_salt =
    if cgi then Option.value ~default:"" !cgi_secret_salt
  ...
  main ~plugins ~interface ~port ~predictable_mode ~secret_salt ~cgi ()
