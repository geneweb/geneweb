      ...
      let spouses =
        Array.fold_right
          (fun ifam spouses ->
            ...
            if p_surname base spouse <> "?" then spouse :: spouses else spouses)
          (get_family p) []
      in
      (match spouses with
      | [] -> ()
      ...
      then (
        ...
        if not @@ try_plugin plugins conf bfile m then
          ((if List.assoc_opt "counter" conf.base_env <> Some "no" then
           ...
           | "CHG_FAM_ORD" -> w_wizard @@ w_base @@ UpdateFam.print_change_order
           | "CHG_FAM_ORD_OK" ->
           ...
           | "N" -> (
               ...
               match p_getenv conf.env "v" with
               | Some v ->
                   Some.search_surname_print conf base Some.surname_not_found v
           ...
           | "NG" -> (
               ...
               | Some i ->
                   relation_print conf base (pget conf base (iper_of_string i)))
           ...
           | "PPS" -> w_base @@ Place_v7.print_all_places_surnames
