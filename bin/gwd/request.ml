...
let person_selected_with_redirect ~conf ~base ?(parameters = []) ~person () =
  ...
  | None ->
    let open Def in
    let url = (Geneweb.Util.commd conf ^^^ Geneweb.Util.acces conf base person :> string) in
    let url = if parameters = [] then url
      else List.fold_left (fun url (param_name, param_value) -> Printf.sprintf "%s&%s=%s" url param_name param_value) url parameters
    in
