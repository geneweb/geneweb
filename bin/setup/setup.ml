...
let header_no_page_title conf title =
  ...
  Geneweb.Output.header printer_conf "Content-type: text/html; charset=%s"
    (charset conf);
    ...
    while true do
      ...
      | '%' -> (
          ...
          | 'f' ->
              ...
              let s = file_contents (!bin_dir ^ "/setup/" ^ in_file) in
          ...
          | 't' -> if Stream.next strm = '{' then ignore (parse_upto '}' strm)
          | 'v' ->
          ...
          | ('A' .. 'Z' | '0' .. '9') as c -> (
              ...
              | 'H' ->
                  ...
                  let outfile =
                    if bname <> "" then bname ^ ".gwb" ^ outfile else outfile
                  in
...
let error conf str =
  header conf (fun _ ->
      Geneweb.Output.print_sstring printer_conf "Incorrect request");
...
let has_gwu dir = try Array.mem "gwu" (Sys.readdir dir) with _ -> false
...
let cleanup_1 conf =
  ...
  flush stderr;
  Files.rm_rf (Filename.concat "old" in_base_dir);
...
let delete_1 conf =
  List.iter (fun (k, v) -> if v = "del" then Files.rm_rf (k ^ ".gwb")) conf.env;
...
let gwf conf =
  ...
  if in_base = "" then print_file conf "err_miss.htm"
    ...
    let benv = read_base_env in_base in
    let trailer =
      in_base ^ ".trl" |> Filename.concat "lang" |> file_contents
...
let print_typed_file conf typ fname =
  ...
  | Some ic ->
      ...
      Geneweb.Output.header printer_conf "Content-length: %d"
        (in_channel_length ic);
      (try
         while true do
           let c = input_char ic in
           Geneweb.Output.printf printer_conf "%c" c
         done
       with End_of_file -> ());
      close_in ic
  | None ->
...
let wrap_setup a b (c : Adef.encoded_string) = try setup a b c with Exit -> ()
...
let () =
  intro ();
  Wserver.f
    ~syslog:(fun _ -> prerr_endline)
    ~addr:None ~port:!port ~timeout:0 ~max_clients:None ~handler:wrap_setup
