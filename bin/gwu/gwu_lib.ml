...
let print_infos opts base is_child csrc cbp p =
  ...
  | Def.IfTitles -> ()
  | Def.Public -> Printf.ksprintf (oc opts) " #apubl"
  | Def.Private -> Printf.ksprintf (oc opts) " #apriv");
  ...
  | Def.Death (dr, d) ->
      Printf.ksprintf (oc opts) " ";
      (match dr with
      | Def.Killed -> Printf.ksprintf (oc opts) "k"
  ...
  | Def.DontKnowIfDead -> (
      ...
      | Some _, _ | _, Some _ -> Printf.ksprintf (oc opts) " ?"
      | _ -> ())
  | Def.OfCourseDead -> Printf.ksprintf (oc opts) " od"
  | Def.NotDead -> ());
...
let print_relation_parent opts base mark defined_p p =
  ...
  if
    Array.length (Gwdb.get_family p) = 0
    && Gwdb.get_parents p = None
    && not (Gwdb.Marker.get mark (Gwdb.get_iper p))
  then (
...
let print_relation_for_person opts base gen def_p r =
  ...
  | _ ->
      if err_same_sex then
        ...
        | Some fath, Some moth ->
            if Gwdb.get_sex fath = Def.Male && Gwdb.get_sex moth = Def.Female
            then (
