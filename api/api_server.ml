(* camlp5r pa_extend.cmo ../src/pa_html.cmo ../src/pa_lock.cmo *)
(* $Id: api_server.ml,v 5.61 2009-03-11 10:56:09 ddr Exp $ *)
...
value conf_and_connection from (addr, request) script_name contents env =
  ...
  do {
    ...
    | _ ->
        ...
        | _ ->
            do {
              Api_request.treat_request_on_base conf
                (log_file.val, log_oc, flush_log);
              if conf.manitou && sleep > 0 then Unix.sleep sleep
              else ()
            } ] ] }
...
value connection (addr, request) script_name contents =
  let from =
    match addr with
    [ Unix.ADDR_UNIX x -> x
    | Unix.ADDR_INET iaddr port -> Unix.string_of_inet_addr iaddr ]
  in
  do {
...
value geneweb_server () =
  ...
  do {
    if not auto_call then do {
      ...
      in
      eprintf "API GeneWeb %s - " Version.txt;
...
<file>
       ...
       ("-add_lexicon",
        ...
        "<lexicon>\n       Add file as lexicon.");
       ("-add_place_dico",
        Arg.String (fun x -> place_dico_list.val := [x :: place_dico_list.val]),
        "<file>\n       Add file as dictionary of places.");
