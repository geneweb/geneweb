...
and parse_conf_base x =
  let x = Piqirun.parse_record x in
  let _configuration, x = Piqirun.parse_required_field 1 parse_string x in
  Piqirun.check_unparsed_fields x;
  {
    Conf_base.configuration = _configuration;
  }
...
and gen__conf_base code x =
  let _configuration = Piqirun.gen_required_field 1 gen__string x.Conf_base.configuration in
  Piqirun.gen_record code (_configuration :: [])
...
let gen_conf_base x = gen__conf_base (-1) x
...
and default_conf_base () =
  {
    Conf_base.configuration = default_string ();
  }
