...
and Last_visits : sig type t = { mutable user : string }
                       end = Last_visits
and
...
and parse_infos_base x =
  ...
  let (_last_modified_person, x) =
    Piqirun.parse_optional_field 4 parse_protobuf_int64 x in
  let (_real_nb_persons, x) =
    Piqirun.parse_optional_field 5 parse_protobuf_int64 x
  in
...
and parse_last_visits x =
  let x = Piqirun.parse_record x in
  let (_user, x) = Piqirun.parse_required_field 1 parse_string x
  in (Piqirun.check_unparsed_fields x; { Last_visits.user = _user; })
...
and gen__infos_base code x =
  ...
  let _last_modified_person =
    Piqirun.gen_optional_field 4 gen__protobuf_int64
      x.Infos_base.last_modified_person in
  let _real_nb_persons =
    Piqirun.gen_optional_field 5 gen__protobuf_int64
      x.Infos_base.real_nb_persons
  in
...
and gen__last_visits code x =
  let _user = Piqirun.gen_required_field 1 gen__string x.Last_visits.user
  in Piqirun.gen_record code [ _user ]
