...
and parse_last_modifications x =
  ...
  let (_max_res, x) =
    Piqirun.parse_optional_field 2 parse_protobuf_int32 x in
  let (_range, x) = Piqirun.parse_optional_field 3 parse_filter_date_range x
  in
    (Piqirun.check_unparsed_fields x;
     {
...
and gen__last_modifications code x =
  ...
  let _max_res =
    Piqirun.gen_optional_field 2 gen__protobuf_int32
      x.Last_modifications.max_res in
  let _range =
    Piqirun.gen_optional_field 3 gen__filter_date_range
      x.Last_modifications.range
  in Piqirun.gen_record code [ _wizard; _max_res; _range ]
