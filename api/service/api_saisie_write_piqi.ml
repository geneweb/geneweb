module rec Api_saisie_write_piqi :
             sig
               ...
               type create_or_link =
                 [ | `create | `link | `create_default_occ
                 ]
               ...
               type access =
                 [ | `access_iftitles | `access_public | `access_private
                 ]
               ...
               type person_or_family =
                 [ | `person_form1 | `person_form2 | `family_form
                 ]
               ...
               type auto_complete_field =
                 [ | `lastname | `firstname | `place | `source
                 ]
               ...
               type transl_relation_parent_type =
                 Transl_relation_parent_type.
                 t
               
               type config_transl_relation_parent_type =
                 Config_transl_relation_parent_type.
                 t
               ...
               type config_transl_update_warning_js =
                 Config_transl_update_warning_js.
                 t
               
               type transl_short_greg_month = Transl_short_greg_month.t
               
               type config_transl_short_greg_month =
                 Config_transl_short_greg_month.
                 t
    ...
    sig
      type t =
        { mutable index : Api_saisie_write_piqi.protobuf_int32;
          mutable sex : Api_saisie_write_piqi.sex; mutable lastname : string;
          mutable firstname : string; mutable dates : string option;
          mutable image : string option;
          mutable sosa : Api_saisie_write_piqi.sosa; mutable family : string
        }
...
and
  Was_witness :
    sig type t = { mutable husband : string; mutable wife : string }
         end =
    Was_witness
and
  Person_search_info :
    ...
    sig
      type t =
        { mutable digest : string;
          ...
          mutable psources : string option; mutable notes : string option;
    ...
    sig
      type t =
        { mutable form : Api_saisie_write_piqi.person_or_family option;
          mutable witness : bool; mutable rparents : bool;
          mutable event : bool;
    ...
    sig
      type t =
        { mutable is_base_updated : bool;
          ...
          mutable firstname_str : string option; mutable n : string option;
...
and
  Index_person :
    sig type t = { mutable index : Api_saisie_write_piqi.protobuf_int32 }
         end =
    Index_person
and
  Index_family :
...
and
  Auto_complete_result : sig type t = { mutable result : string list }
                              end =
    Auto_complete_result
and
  Person_search_list_params :
    sig
      type t =
...
and
  Person_search_list :
    sig
      type t = { mutable persons : Api_saisie_write_piqi.person_search list }
...
and
  Transl_calendar :
    sig
      type t =
        { mutable pos : Api_saisie_write_piqi.calendar; mutable sval : string
        }
      
    end = Transl_calendar
and
  Config_transl_calendar :
    sig type t = { mutable msg : Api_saisie_write_piqi.transl_calendar list }
...
and
  Config_transl_precision :
    sig
      type t = { mutable msg : Api_saisie_write_piqi.transl_precision list }
...
and
  Config_transl_death_type :
    sig
      type t = { mutable msg : Api_saisie_write_piqi.transl_death_type list }
  ...
  Config_transl_relation_parent_type :
    sig
      type t =
        {
          mutable msg :
          Api_saisie_write_piqi.transl_relation_parent_type list
        }
...
and
  Transl_access :
    sig
      type t =
        { mutable pos : Api_saisie_write_piqi.access; mutable sval : string
        }
      
    end = Transl_access
and
  Config_transl_access :
    sig type t = { mutable msg : Api_saisie_write_piqi.transl_access list }
        
    end = Config_transl_access
and
  Transl_update_warning_js :
    sig
      type t =
    ...
    sig
      type t =
        { mutable transl_cal : Api_saisie_write_piqi.config_transl_calendar;
          mutable transl_wit : Api_saisie_write_piqi.
          config_transl_witness_type;
          mutable transl_prec : Api_saisie_write_piqi.config_transl_precision;
          mutable transl_death : Api_saisie_write_piqi.
          config_transl_death_type;
          mutable transl_rel : Api_saisie_write_piqi.
          config_transl_relation_parent_type;
          mutable transl_fevents : Api_saisie_write_piqi.
          config_transl_fevent_name;
          mutable transl_pevents : Api_saisie_write_piqi.
          config_transl_pevent_name;
          mutable transl_access : Api_saisie_write_piqi.config_transl_access;
          mutable transl_warning : Api_saisie_write_piqi.
          config_transl_update_warning_js;
          mutable transl_short_greg_month : Api_saisie_write_piqi.
          config_transl_short_greg_month;
          mutable transl_french_month : Api_saisie_write_piqi.
          config_transl_french_month;
          mutable transl_hebrew_month : Api_saisie_write_piqi.
          config_transl_hebrew_month; mutable gwf_place_format : string;
...
let rec parse_protobuf_int32 x = Piqirun.int32_of_signed_varint x
...
and parse_dmy x =
  ...
  let (_delta, x) = Piqirun.parse_optional_field 4 parse_protobuf_int32 x
  in
...
and parse_date x =
  ...
  let (_text, x) = Piqirun.parse_optional_field 5 parse_string x
  in
    (Piqirun.check_unparsed_fields x;
     {
       Date.cal = _cal;
...
and parse_person_search x =
  ...
  let (_family, x) = Piqirun.parse_required_field 8 parse_string x
  in
    (Piqirun.check_unparsed_fields x;
     {
...
and parse_simple_person x =
  ...
  let (_birth_short_date, x) =
    Piqirun.parse_optional_field 5 parse_string x in
  let (_birth_place, x) = Piqirun.parse_optional_field 6 parse_string x in
  let (_death_short_date, x) =
    Piqirun.parse_optional_field 7 parse_string x in
  ...
  let (_sosa, x) = Piqirun.parse_required_field 10 parse_sosa x
  in
    (Piqirun.check_unparsed_fields x;
     {
...
and parse_was_witness x =
  ...
  let (_wife, x) = Piqirun.parse_required_field 2 parse_string x
  in
    (Piqirun.check_unparsed_fields x;
     { Was_witness.husband = _husband; Was_witness.wife = _wife; })
and parse_person_search_info x =
  ...
  let (_related, x) =
    Piqirun.parse_repeated_field 17 parse_relation_person x in
  let (_rparents, x) =
    Piqirun.parse_repeated_field 18 parse_relation_person x in
  let (_was_witness, x) =
    Piqirun.parse_repeated_field 19 parse_was_witness x in
  let (_sosa, x) = Piqirun.parse_required_field 20 parse_sosa x
  in
    (Piqirun.check_unparsed_fields x;
     {
...
and parse_person_link x =
  let x = Piqirun.parse_record x in
  let (_create_link, x) =
    Piqirun.parse_required_field 1 parse_create_or_link x in
  ...
  let (_dates, x) = Piqirun.parse_optional_field 7 parse_string x
  in
    (Piqirun.check_unparsed_fields x;
     {
...
and parse_witness x =
  ...
  let (_witness_type, x) =
    Piqirun.parse_required_field 1 parse_witness_type x in
  let (_person, x) = Piqirun.parse_optional_field 2 parse_person_link x
  in
    (Piqirun.check_unparsed_fields x;
     { Witness.witness_type = _witness_type; Witness.person = _person; })
and parse_fevent x =
  let x = Piqirun.parse_record x in
  let (_fevent_type, x) =
    Piqirun.parse_optional_field 1 parse_fevent_name x in
  ...
  let (_event_perso, x) = Piqirun.parse_optional_field 8 parse_string x
  in
    (Piqirun.check_unparsed_fields x;
     {
...
and parse_relation_parent x =
  let x = Piqirun.parse_record x in
  let (_rpt_type, x) =
    Piqirun.parse_required_field 1 parse_relation_parent_type x in
  let (_person, x) = Piqirun.parse_optional_field 2 parse_person_link x in
  let (_source, x) = Piqirun.parse_optional_field 3 parse_string x
  in
    (Piqirun.check_unparsed_fields x;
     {
...
and parse_title x =
  ...
  let (_nth, x) = Piqirun.parse_optional_field 6 parse_protobuf_int32 x
  in
    (Piqirun.check_unparsed_fields x;
     {
       Title.name = _name;
...
and parse_pevent x =
  let x = Piqirun.parse_record x in
  let (_pevent_type, x) =
    Piqirun.parse_optional_field 1 parse_pevent_name x in
  let (_date, x) = Piqirun.parse_optional_field 2 parse_date x in
  let (_place, x) = Piqirun.parse_optional_field 3 parse_string x in
  let (_reason, x) = Piqirun.parse_optional_field 4 parse_string x in
  let (_note, x) = Piqirun.parse_optional_field 5 parse_string x in
...
and parse_person x =
  ...
  let (_create_link, x) =
    Piqirun.parse_required_field 2 parse_create_or_link x in
  ...
  let (_death_type, x) =
    Piqirun.parse_required_field 14 parse_death_type x in
  ...
  let (_related, x) =
    Piqirun.parse_repeated_field 20 parse_protobuf_int32 x in
  let (_rparents, x) =
    Piqirun.parse_repeated_field 21 parse_relation_parent x in
  let (_access, x) =
    Piqirun.parse_required_field 22 parse_access x ~default: "\b\000" in
  let (_parents, x) =
    Piqirun.parse_optional_field 23 parse_protobuf_int32 x in
  let (_families, x) = Piqirun.parse_repeated_field 24 parse_protobuf_int32 x
  in
    (Piqirun.check_unparsed_fields x;
     {
...
and parse_family x =
  ...
  let (_old_witnesses, x) =
    Piqirun.parse_repeated_field 10 parse_protobuf_int32 x
  in
    (Piqirun.check_unparsed_fields x;
     {
...
and parse_create_conflict x =
  ...
  let (_pos_witness, x) =
    Piqirun.parse_optional_field 6 parse_protobuf_int32 x in
  let (_lastname, x) = Piqirun.parse_required_field 7 parse_string x in
  let (_firstname, x) = Piqirun.parse_required_field 8 parse_string x
  in
    (Piqirun.check_unparsed_fields x;
     {
...
and parse_modification_status x =
  ...
  let (_occ, x) = Piqirun.parse_optional_field 6 parse_protobuf_int32 x in
  let (_index_family, x) =
    Piqirun.parse_optional_field 7 parse_protobuf_int32 x in
  let (_conflict, x) =
    Piqirun.parse_optional_field 8 parse_create_conflict x in
  ...
  let (_p, x) = Piqirun.parse_optional_field 12 parse_string x
  in
    (Piqirun.check_unparsed_fields x;
     {
...
and parse_index_family x =
  let x = Piqirun.parse_record x in
  let (_index, x) = Piqirun.parse_required_field 1 parse_protobuf_int32 x
  in (Piqirun.check_unparsed_fields x; { Index_family.index = _index; })
and parse_index_person_and_family x =
  let x = Piqirun.parse_record x in
  let (_index_person, x) =
    Piqirun.parse_required_field 1 parse_protobuf_int32 x in
  let (_index_family, x) =
    Piqirun.parse_required_field 2 parse_protobuf_int32 x
  in
    (Piqirun.check_unparsed_fields x;
     {
...
and parse_family_spouse x =
  ...
  let (_index_family, x) =
    Piqirun.parse_required_field 1 parse_protobuf_int32 x in
  let (_index_person, x) =
    Piqirun.parse_required_field 2 parse_protobuf_int32 x in
  let (_sex, x) = Piqirun.parse_required_field 3 parse_sex x in
  let (_lastname, x) = Piqirun.parse_required_field 4 parse_string x in
  let (_firstname, x) = Piqirun.parse_required_field 5 parse_string x in
  ...
  let (_sosa, x) = Piqirun.parse_required_field 8 parse_sosa x
  in
    (Piqirun.check_unparsed_fields x;
     {
...
and parse_add_child_request x =
  ...
  let (_sex, x) = Piqirun.parse_optional_field 3 parse_sex x
  in
    (Piqirun.check_unparsed_fields x;
     {
...
and parse_add_child x =
  let x = Piqirun.parse_record x in
  let (_person_lastname, x) =
    Piqirun.parse_required_field 1 parse_string x in
  let (_person_firstname, x) =
    Piqirun.parse_required_field 2 parse_string x in
  let (_family_spouse, x) =
    Piqirun.parse_repeated_field 3 parse_family_spouse x in
  let (_child, x) = Piqirun.parse_required_field 4 parse_person x
  in
    (Piqirun.check_unparsed_fields x;
     {
...
and parse_add_parents x =
  ...
  let (_person_firstname, x) =
    Piqirun.parse_required_field 2 parse_string x in
  let (_family, x) = Piqirun.parse_required_field 3 parse_family x
  in
    (Piqirun.check_unparsed_fields x;
     {
...
and parse_add_parents_ok x =
  ...
  let (_index_person, x) =
    Piqirun.parse_required_field 1 parse_protobuf_int32 x in
  let (_family, x) = Piqirun.parse_required_field 2 parse_family x
  in
    (Piqirun.check_unparsed_fields x;
     {
...
and parse_edit_family_request x =
  ...
  let (_first_family, x) = Piqirun.parse_optional_field 2 parse_edit_family x
  in
    (Piqirun.check_unparsed_fields x;
     {
...
and parse_add_sibling_request x =
  let x = Piqirun.parse_record x in
  let (_index, x) = Piqirun.parse_required_field 1 parse_protobuf_int32 x in
  let (_sex, x) = Piqirun.parse_optional_field 2 parse_sex x
  in
    (Piqirun.check_unparsed_fields x;
     { Add_sibling_request.index = _index; Add_sibling_request.sex = _sex; })
and parse_add_sibling x =
  ...
  let (_person_firstname, x) =
    Piqirun.parse_required_field 2 parse_string x in
  let (_sibling, x) = Piqirun.parse_required_field 3 parse_person x
  in
    (Piqirun.check_unparsed_fields x;
     {
...
and parse_add_sibling_ok x =
  ...
  let (_index_person, x) =
    Piqirun.parse_required_field 1 parse_protobuf_int32 x in
  let (_sibling, x) = Piqirun.parse_required_field 2 parse_person x
  in
    (Piqirun.check_unparsed_fields x;
     {
...
and parse_add_first_fam x =
  ...
  let (_children, x) = Piqirun.parse_repeated_field 5 parse_person x
  in
    (Piqirun.check_unparsed_fields x;
     {
...
and parse_auto_complete x =
  let x = Piqirun.parse_record x in
  let (_field, x) =
    Piqirun.parse_required_field 1 parse_auto_complete_field x in
  let (_place_field, x) =
    Piqirun.parse_optional_field 2 parse_auto_complete_place_field x in
  let (_input, x) = Piqirun.parse_required_field 3 parse_string x in
  let (_limit, x) = Piqirun.parse_required_field 4 parse_protobuf_int32 x
  in
    (Piqirun.check_unparsed_fields x;
     {
...
and parse_auto_complete_result x =
  let x = Piqirun.parse_record x in
  let (_result, x) = Piqirun.parse_repeated_field 1 parse_string x
  in
    (Piqirun.check_unparsed_fields x;
...
and parse_person_search_list_params x =
  ...
  let (_limit, x) = Piqirun.parse_required_field 3 parse_protobuf_int32 x
  in
    (Piqirun.check_unparsed_fields x;
     {
...
and parse_person_search_list x =
  let x = Piqirun.parse_record x in
  let (_persons, x) = Piqirun.parse_repeated_field 1 parse_person_search x
  in
    (Piqirun.check_unparsed_fields x;
...
and parse_transl_calendar x =
  ...
  let (_sval, x) = Piqirun.parse_required_field 2 parse_string x
  in
    (Piqirun.check_unparsed_fields x;
     { Transl_calendar.pos = _pos; Transl_calendar.sval = _sval; })
and parse_config_transl_calendar x =
  let x = Piqirun.parse_record x in
  let (_msg, x) = Piqirun.parse_repeated_field 1 parse_transl_calendar x
  in
    (Piqirun.check_unparsed_fields x; { Config_transl_calendar.msg = _msg; })
and parse_transl_witness_type x =
  ...
  in
    (Piqirun.check_unparsed_fields x;
     { Transl_witness_type.pos = _pos; Transl_witness_type.sval = _sval; })
and parse_config_transl_witness_type x =
  let x = Piqirun.parse_record x in
  let (_msg, x) = Piqirun.parse_repeated_field 1 parse_transl_witness_type x
  in
    (Piqirun.check_unparsed_fields x;
...
and parse_transl_precision x =
  ...
  in
    (Piqirun.check_unparsed_fields x;
     { Transl_precision.pos = _pos; Transl_precision.sval = _sval; })
and parse_config_transl_precision x =
  let x = Piqirun.parse_record x in
  let (_msg, x) = Piqirun.parse_repeated_field 1 parse_transl_precision x
  in
    (Piqirun.check_unparsed_fields x;
...
and parse_transl_death_type x =
  ...
  in
    (Piqirun.check_unparsed_fields x;
     { Transl_death_type.pos = _pos; Transl_death_type.sval = _sval; })
and parse_config_transl_death_type x =
  let x = Piqirun.parse_record x in
  let (_msg, x) = Piqirun.parse_repeated_field 1 parse_transl_death_type x
  in
    (Piqirun.check_unparsed_fields x;
...
and parse_config_transl_relation_parent_type x =
  let x = Piqirun.parse_record x in
  let (_msg, x) =
    Piqirun.parse_repeated_field 1 parse_transl_relation_parent_type x
  in
    (Piqirun.check_unparsed_fields x;
...
and parse_transl_fevent_name x =
  ...
  in
    (Piqirun.check_unparsed_fields x;
     { Transl_fevent_name.pos = _pos; Transl_fevent_name.sval = _sval; })
and parse_config_transl_fevent_name x =
  let x = Piqirun.parse_record x in
  let (_msg, x) = Piqirun.parse_repeated_field 1 parse_transl_fevent_name x
  in
    (Piqirun.check_unparsed_fields x;
...
and parse_transl_pevent_name x =
  ...
  in
    (Piqirun.check_unparsed_fields x;
     { Transl_pevent_name.pos = _pos; Transl_pevent_name.sval = _sval; })
and parse_config_transl_pevent_name x =
  let x = Piqirun.parse_record x in
  let (_msg, x) = Piqirun.parse_repeated_field 1 parse_transl_pevent_name x
  in
    (Piqirun.check_unparsed_fields x;
...
and parse_transl_access x =
  ...
  in
    (Piqirun.check_unparsed_fields x;
     { Transl_access.pos = _pos; Transl_access.sval = _sval; })
and parse_config_transl_access x =
  let x = Piqirun.parse_record x in
  let (_msg, x) = Piqirun.parse_repeated_field 1 parse_transl_access x
  in (Piqirun.check_unparsed_fields x; { Config_transl_access.msg = _msg; })
...
and parse_config_transl_update_warning_js x =
  let x = Piqirun.parse_record x in
  let (_msg, x) =
    Piqirun.parse_repeated_field 1 parse_transl_update_warning_js x
  in
    (Piqirun.check_unparsed_fields x;
...
and parse_config_transl_short_greg_month x =
  let x = Piqirun.parse_record x in
  let (_msg, x) =
    Piqirun.parse_repeated_field 1 parse_transl_short_greg_month x
  in
    (Piqirun.check_unparsed_fields x;
...
and parse_transl_french_month x =
  ...
  in
    (Piqirun.check_unparsed_fields x;
     { Transl_french_month.pos = _pos; Transl_french_month.sval = _sval; })
and parse_config_transl_french_month x =
  let x = Piqirun.parse_record x in
  let (_msg, x) = Piqirun.parse_repeated_field 1 parse_transl_french_month x
  in
    (Piqirun.check_unparsed_fields x;
...
and parse_transl_hebrew_month x =
  ...
  in
    (Piqirun.check_unparsed_fields x;
     { Transl_hebrew_month.pos = _pos; Transl_hebrew_month.sval = _sval; })
and parse_config_transl_hebrew_month x =
  let x = Piqirun.parse_record x in
  let (_msg, x) = Piqirun.parse_repeated_field 1 parse_transl_hebrew_month x
  in
    (Piqirun.check_unparsed_fields x;
...
and parse_config x =
  let x = Piqirun.parse_record x in
  let (_transl_cal, x) =
    Piqirun.parse_required_field 1 parse_config_transl_calendar x in
  let (_transl_wit, x) =
    Piqirun.parse_required_field 2 parse_config_transl_witness_type x in
  let (_transl_prec, x) =
    Piqirun.parse_required_field 3 parse_config_transl_precision x in
  let (_transl_death, x) =
    Piqirun.parse_required_field 4 parse_config_transl_death_type x in
  let (_transl_rel, x) =
    Piqirun.parse_required_field 5 parse_config_transl_relation_parent_type x in
  let (_transl_fevents, x) =
    Piqirun.parse_required_field 6 parse_config_transl_fevent_name x in
  let (_transl_pevents, x) =
    Piqirun.parse_required_field 7 parse_config_transl_pevent_name x in
  let (_transl_access, x) =
    Piqirun.parse_required_field 8 parse_config_transl_access x in
  let (_transl_warning, x) =
    Piqirun.parse_required_field 9 parse_config_transl_update_warning_js x in
  let (_transl_short_greg_month, x) =
    Piqirun.parse_required_field 10 parse_config_transl_short_greg_month x in
  let (_transl_french_month, x) =
    Piqirun.parse_required_field 11 parse_config_transl_french_month x in
  let (_transl_hebrew_month, x) =
    Piqirun.parse_required_field 12 parse_config_transl_hebrew_month x in
  let (_gwf_place_format, x) =
    Piqirun.parse_required_field 13 parse_string x in
  let (_gwf_place_format_placeholder, x) =
    Piqirun.parse_required_field 14 parse_string x
  in
    (Piqirun.check_unparsed_fields x;
     {
...
let rec gen__protobuf_int32 code x = Piqirun.int32_to_signed_varint code x
...
and gen__dmy code x =
  ...
  let _month =
    Piqirun.gen_optional_field 2 gen__protobuf_int32 x.Dmy.month in
  let _year = Piqirun.gen_optional_field 3 gen__int32 x.Dmy.year in
  let _delta = Piqirun.gen_optional_field 4 gen__protobuf_int32 x.Dmy.delta
...
and gen__date code x =
  ...
  let _text = Piqirun.gen_optional_field 5 gen__string x.Date.text
  in Piqirun.gen_record code [ _cal; _prec; _dmy; _dmy2; _text ]
and gen__person_search code x =
  let _index =
    Piqirun.gen_required_field 1 gen__protobuf_int32 x.Person_search.index in
  let _sex = Piqirun.gen_required_field 2 gen__sex x.Person_search.sex in
  let _lastname =
    Piqirun.gen_required_field 3 gen__string x.Person_search.lastname in
  let _firstname =
    Piqirun.gen_required_field 4 gen__string x.Person_search.firstname in
  let _dates =
    Piqirun.gen_optional_field 5 gen__string x.Person_search.dates in
  let _image =
    Piqirun.gen_optional_field 6 gen__string x.Person_search.image in
  let _sosa = Piqirun.gen_required_field 7 gen__sosa x.Person_search.sosa in
  let _family =
    Piqirun.gen_required_field 8 gen__string x.Person_search.family
  in
    Piqirun.gen_record code
      [ _index; _sex; _lastname; _firstname; _dates; _image; _sosa; _family ]
and gen__simple_person code x =
  ...
  let _birth_short_date =
    Piqirun.gen_optional_field 5 gen__string x.Simple_person.birth_short_date in
  let _birth_place =
    Piqirun.gen_optional_field 6 gen__string x.Simple_person.birth_place in
  let _death_short_date =
    Piqirun.gen_optional_field 7 gen__string x.Simple_person.death_short_date in
  let _death_place =
    Piqirun.gen_optional_field 8 gen__string x.Simple_person.death_place in
  let _image =
    Piqirun.gen_optional_field 9 gen__string x.Simple_person.image in
  let _sosa = Piqirun.gen_required_field 10 gen__sosa x.Simple_person.sosa
  in
    Piqirun.gen_record code
      [ _index; _sex; _lastname; _firstname; _birth_short_date; _birth_place;
        _death_short_date; _death_place; _image; _sosa ]
...
and gen__was_witness code x =
  let _husband =
    Piqirun.gen_required_field 1 gen__string x.Was_witness.husband in
  let _wife = Piqirun.gen_required_field 2 gen__string x.Was_witness.wife
  in Piqirun.gen_record code [ _husband; _wife ]
and gen__person_search_info code x =
  let _index =
    Piqirun.gen_required_field 1 gen__protobuf_int32
      x.Person_search_info.index in
  let _sex =
    Piqirun.gen_required_field 2 gen__sex x.Person_search_info.sex in
  let _lastname =
    Piqirun.gen_required_field 3 gen__string x.Person_search_info.lastname in
  let _firstname =
    Piqirun.gen_required_field 4 gen__string x.Person_search_info.firstname in
  let _public_name =
    Piqirun.gen_optional_field 5 gen__string x.Person_search_info.public_name in
  let _aliases =
    Piqirun.gen_repeated_field 6 gen__string x.Person_search_info.aliases in
  let _qualifiers =
    Piqirun.gen_repeated_field 7 gen__string x.Person_search_info.qualifiers in
  let _firstname_aliases =
    Piqirun.gen_repeated_field 8 gen__string
      x.Person_search_info.firstname_aliases in
  let _surname_aliases =
    Piqirun.gen_repeated_field 9 gen__string
      x.Person_search_info.surname_aliases in
  let _image =
    Piqirun.gen_optional_field 10 gen__string x.Person_search_info.image in
  let _events =
    Piqirun.gen_repeated_field 11 gen__event x.Person_search_info.events in
  let _occupation =
    Piqirun.gen_optional_field 12 gen__string x.Person_search_info.occupation in
  let _notes =
    Piqirun.gen_optional_field 13 gen__string x.Person_search_info.notes in
  let _psources =
    Piqirun.gen_optional_field 14 gen__string x.Person_search_info.psources in
  let _has_sources =
    Piqirun.gen_required_field 15 gen__bool x.Person_search_info.has_sources in
  let _titles =
    Piqirun.gen_repeated_field 16 gen__string x.Person_search_info.titles in
  let _related =
    Piqirun.gen_repeated_field 17 gen__relation_person
      x.Person_search_info.related in
  let _rparents =
    Piqirun.gen_repeated_field 18 gen__relation_person
      x.Person_search_info.rparents in
  let _was_witness =
    Piqirun.gen_repeated_field 19 gen__was_witness
      x.Person_search_info.was_witness in
  let _sosa =
    Piqirun.gen_required_field 20 gen__sosa x.Person_search_info.sosa
  in
    Piqirun.gen_record code
      [ _index; _sex; _lastname; _firstname; _public_name; _aliases;
        _qualifiers; _firstname_aliases; _surname_aliases; _image; _events;
        _occupation; _notes; _psources; _has_sources; _titles; _related;
        _rparents; _was_witness; _sosa ]
and gen__person_link code x =
  let _create_link =
    Piqirun.gen_required_field 1 gen__create_or_link
      x.Person_link.create_link in
  let _index =
    Piqirun.gen_required_field 2 gen__protobuf_int32 x.Person_link.index in
  let _sex = Piqirun.gen_required_field 3 gen__sex x.Person_link.sex in
  let _lastname =
    Piqirun.gen_required_field 4 gen__string x.Person_link.lastname in
  let _firstname =
    Piqirun.gen_required_field 5 gen__string x.Person_link.firstname in
  let _occ =
    Piqirun.gen_optional_field 6 gen__protobuf_int32 x.Person_link.occ in
  let _dates = Piqirun.gen_optional_field 7 gen__string x.Person_link.dates
  in
    Piqirun.gen_record code
      [ _create_link; _index; _sex; _lastname; _firstname; _occ; _dates ]
and gen__witness code x =
  let _witness_type =
    Piqirun.gen_required_field 1 gen__witness_type x.Witness.witness_type in
  let _person =
    Piqirun.gen_optional_field 2 gen__person_link x.Witness.person
  in Piqirun.gen_record code [ _witness_type; _person ]
and gen__fevent code x =
  let _fevent_type =
    Piqirun.gen_optional_field 1 gen__fevent_name x.Fevent.fevent_type in
  ...
  let _witnesses =
    Piqirun.gen_repeated_field 7 gen__witness x.Fevent.witnesses in
  let _event_perso =
    Piqirun.gen_optional_field 8 gen__string x.Fevent.event_perso
  in
    Piqirun.gen_record code
      [ _fevent_type; _date; _place; _reason; _note; _src; _witnesses;
        _event_perso ]
and gen__relation_parent code x =
  let _rpt_type =
    Piqirun.gen_required_field 1 gen__relation_parent_type
      x.Relation_parent.rpt_type in
  let _person =
    Piqirun.gen_optional_field 2 gen__person_link x.Relation_parent.person in
  let _source =
    Piqirun.gen_optional_field 3 gen__string x.Relation_parent.source
  in Piqirun.gen_record code [ _rpt_type; _person; _source ]
and gen__title code x =
  ...
  let _date_begin =
    Piqirun.gen_optional_field 4 gen__date x.Title.date_begin in
  let _date_end = Piqirun.gen_optional_field 5 gen__date x.Title.date_end in
  let _nth = Piqirun.gen_optional_field 6 gen__protobuf_int32 x.Title.nth
  in
    Piqirun.gen_record code
      [ _name; _title; _fief; _date_begin; _date_end; _nth ]
and gen__pevent code x =
  let _pevent_type =
    Piqirun.gen_optional_field 1 gen__pevent_name x.Pevent.pevent_type in
  ...
  let _witnesses =
    Piqirun.gen_repeated_field 7 gen__witness x.Pevent.witnesses in
  let _event_perso =
    Piqirun.gen_optional_field 8 gen__string x.Pevent.event_perso
  in
    Piqirun.gen_record code
      [ _pevent_type; _date; _place; _reason; _note; _src; _witnesses;
        _event_perso ]
and gen__person code x =
  let _digest = Piqirun.gen_required_field 1 gen__string x.Person.digest in
  let _create_link =
    Piqirun.gen_required_field 2 gen__create_or_link x.Person.create_link in
  let _index =
    Piqirun.gen_required_field 3 gen__protobuf_int32 x.Person.index in
  let _sex = Piqirun.gen_required_field 4 gen__sex x.Person.sex in
  let _lastname =
    Piqirun.gen_required_field 5 gen__string x.Person.lastname in
  let _firstname =
    Piqirun.gen_required_field 6 gen__string x.Person.firstname in
  ...
  let _death_type =
    Piqirun.gen_required_field 14 gen__death_type x.Person.death_type in
  let _occupation =
    Piqirun.gen_optional_field 15 gen__string x.Person.occupation in
  let _psources =
    Piqirun.gen_optional_field 16 gen__string x.Person.psources in
  ...
  let _pevents =
    Piqirun.gen_repeated_field 19 gen__pevent x.Person.pevents in
  let _related =
    Piqirun.gen_repeated_field 20 gen__protobuf_int32 x.Person.related in
  let _rparents =
    Piqirun.gen_repeated_field 21 gen__relation_parent x.Person.rparents in
  let _access = Piqirun.gen_required_field 22 gen__access x.Person.access in
  let _parents =
    Piqirun.gen_optional_field 23 gen__protobuf_int32 x.Person.parents in
  let _families =
    Piqirun.gen_repeated_field 24 gen__protobuf_int32 x.Person.families
  in
    Piqirun.gen_record code
      [ _digest; _create_link; _index; _sex; _lastname; _firstname; _occ;
        _public_name; _aliases; _qualifiers; _firstname_aliases;
        _surname_aliases; _image; _death_type; _occupation; _psources;
        _notes; _titles; _pevents; _related; _rparents; _access; _parents;
        _families ]
and gen__family code x =
  let _digest = Piqirun.gen_required_field 1 gen__string x.Family.digest in
  let _index =
    Piqirun.gen_required_field 2 gen__protobuf_int32 x.Family.index in
  let _fevents = Piqirun.gen_repeated_field 3 gen__fevent x.Family.fevents in
  let _fsources =
    Piqirun.gen_optional_field 4 gen__string x.Family.fsources in
  let _origin_file =
    Piqirun.gen_optional_field 5 gen__string x.Family.origin_file in
  ...
  let _children =
    Piqirun.gen_repeated_field 9 gen__person_link x.Family.children in
  let _old_witnesses =
    Piqirun.gen_repeated_field 10 gen__protobuf_int32 x.Family.old_witnesses
  in
    Piqirun.gen_record code
      [ _digest; _index; _fevents; _fsources; _origin_file; _comment;
        _father; _mother; _children; _old_witnesses ]
and gen__create_conflict code x =
  let _form =
    Piqirun.gen_optional_field 1 gen__person_or_family x.Create_conflict.form in
  let _witness =
    Piqirun.gen_required_field 2 gen__bool x.Create_conflict.witness in
  let _rparents =
    Piqirun.gen_required_field 3 gen__bool x.Create_conflict.rparents in
  let _event =
    Piqirun.gen_required_field 4 gen__bool x.Create_conflict.event in
  let _pos =
    Piqirun.gen_optional_field 5 gen__protobuf_int32 x.Create_conflict.pos in
  let _pos_witness =
    Piqirun.gen_optional_field 6 gen__protobuf_int32
      x.Create_conflict.pos_witness in
  let _lastname =
    Piqirun.gen_required_field 7 gen__string x.Create_conflict.lastname in
  let _firstname =
    Piqirun.gen_required_field 8 gen__string x.Create_conflict.firstname
  in
    Piqirun.gen_record code
      [ _form; _witness; _rparents; _event; _pos; _pos_witness; _lastname;
        _firstname ]
and gen__modification_status code x =
  let _is_base_updated =
    Piqirun.gen_required_field 1 gen__bool
      x.Modification_status.is_base_updated in
  let _base_warnings =
  ...
  let _index_person =
    Piqirun.gen_optional_field 3 gen__protobuf_int32
      x.Modification_status.index_person in
  let _lastname =
    Piqirun.gen_required_field 4 gen__string x.Modification_status.lastname in
  let _firstname =
    Piqirun.gen_required_field 5 gen__string x.Modification_status.firstname in
  let _occ =
    Piqirun.gen_optional_field 6 gen__protobuf_int32
      x.Modification_status.occ in
  let _index_family =
    Piqirun.gen_optional_field 7 gen__protobuf_int32
      x.Modification_status.index_family in
  let _conflict =
    Piqirun.gen_optional_field 8 gen__create_conflict
      x.Modification_status.conflict in
  let _lastname_str =
    Piqirun.gen_optional_field 9 gen__string
      x.Modification_status.lastname_str in
  let _firstname_str =
    Piqirun.gen_optional_field 10 gen__string
      x.Modification_status.firstname_str in
  let _n =
    Piqirun.gen_optional_field 11 gen__string x.Modification_status.n in
  let _p = Piqirun.gen_optional_field 12 gen__string x.Modification_status.p
  in
    Piqirun.gen_record code
      [ _is_base_updated; _base_warnings; _index_person; _lastname;
        _firstname; _occ; _index_family; _conflict; _lastname_str;
        _firstname_str; _n; _p ]
...
and gen__index_family code x =
  let _index =
    Piqirun.gen_required_field 1 gen__protobuf_int32 x.Index_family.index
  in Piqirun.gen_record code [ _index ]
and gen__index_person_and_family code x =
  let _index_person =
    Piqirun.gen_required_field 1 gen__protobuf_int32
      x.Index_person_and_family.index_person in
  let _index_family =
    Piqirun.gen_required_field 2 gen__protobuf_int32
      x.Index_person_and_family.index_family
  in Piqirun.gen_record code [ _index_person; _index_family ]
and gen__family_spouse code x =
  let _index_family =
    Piqirun.gen_required_field 1 gen__protobuf_int32
      x.Family_spouse.index_family in
  let _index_person =
    Piqirun.gen_required_field 2 gen__protobuf_int32
      x.Family_spouse.index_person in
  let _sex = Piqirun.gen_required_field 3 gen__sex x.Family_spouse.sex in
  let _lastname =
    Piqirun.gen_required_field 4 gen__string x.Family_spouse.lastname in
  let _firstname =
    Piqirun.gen_required_field 5 gen__string x.Family_spouse.firstname in
  let _dates =
    Piqirun.gen_optional_field 6 gen__string x.Family_spouse.dates in
  let _image =
    Piqirun.gen_optional_field 7 gen__string x.Family_spouse.image in
  let _sosa = Piqirun.gen_required_field 8 gen__sosa x.Family_spouse.sosa
  in
    Piqirun.gen_record code
      [ _index_family; _index_person; _sex; _lastname; _firstname; _dates;
        _image; _sosa ]
and gen__add_child_request code x =
  let _index =
    Piqirun.gen_required_field 1 gen__protobuf_int32
      x.Add_child_request.index in
  let _index_family =
    Piqirun.gen_optional_field 2 gen__protobuf_int32
      x.Add_child_request.index_family in
  let _sex = Piqirun.gen_optional_field 3 gen__sex x.Add_child_request.sex
  in Piqirun.gen_record code [ _index; _index_family; _sex ]
and gen__add_child code x =
  let _person_lastname =
    Piqirun.gen_required_field 1 gen__string x.Add_child.person_lastname in
  let _person_firstname =
    Piqirun.gen_required_field 2 gen__string x.Add_child.person_firstname in
  let _family_spouse =
    Piqirun.gen_repeated_field 3 gen__family_spouse x.Add_child.family_spouse in
  let _child = Piqirun.gen_required_field 4 gen__person x.Add_child.child
  in
    Piqirun.gen_record code
      [ _person_lastname; _person_firstname; _family_spouse; _child ]
and gen__add_child_ok code x =
  let _index_person =
    Piqirun.gen_required_field 1 gen__protobuf_int32
      x.Add_child_ok.index_person in
  let _index_family =
    Piqirun.gen_required_field 2 gen__protobuf_int32
      x.Add_child_ok.index_family in
  let _new_family =
    Piqirun.gen_required_field 3 gen__bool x.Add_child_ok.new_family in
  let _child = Piqirun.gen_required_field 4 gen__person x.Add_child_ok.child
  in
    Piqirun.gen_record code
      [ _index_person; _index_family; _new_family; _child ]
and gen__add_parents code x =
  let _person_lastname =
    Piqirun.gen_required_field 1 gen__string x.Add_parents.person_lastname in
  let _person_firstname =
    Piqirun.gen_required_field 2 gen__string x.Add_parents.person_firstname in
  let _family = Piqirun.gen_required_field 3 gen__family x.Add_parents.family
  in Piqirun.gen_record code [ _person_lastname; _person_firstname; _family ]
and gen__add_parents_ok code x =
  let _index_person =
    Piqirun.gen_required_field 1 gen__protobuf_int32
      x.Add_parents_ok.index_person in
  let _family =
    Piqirun.gen_required_field 2 gen__family x.Add_parents_ok.family
  in Piqirun.gen_record code [ _index_person; _family ]
and gen__add_family code x =
  let _person_lastname =
    Piqirun.gen_required_field 1 gen__string x.Add_family.person_lastname in
  let _person_firstname =
    Piqirun.gen_required_field 2 gen__string x.Add_family.person_firstname in
  let _family = Piqirun.gen_required_field 3 gen__family x.Add_family.family
  in Piqirun.gen_record code [ _person_lastname; _person_firstname; _family ]
and gen__add_family_ok code x =
  let _index_person =
    Piqirun.gen_required_field 1 gen__protobuf_int32
      x.Add_family_ok.index_person in
  let _family =
    Piqirun.gen_required_field 2 gen__family x.Add_family_ok.family
  in Piqirun.gen_record code [ _index_person; _family ]
and gen__edit_family_request code x =
  let _spouses =
    Piqirun.gen_repeated_field 1 gen__family_spouse
      x.Edit_family_request.spouses in
  let _first_family =
    Piqirun.gen_optional_field 2 gen__edit_family
      x.Edit_family_request.first_family
  in Piqirun.gen_record code [ _spouses; _first_family ]
and gen__edit_family code x =
  let _person_lastname =
    Piqirun.gen_required_field 1 gen__string x.Edit_family.person_lastname in
  let _person_firstname =
    Piqirun.gen_required_field 2 gen__string x.Edit_family.person_firstname in
  let _family = Piqirun.gen_required_field 3 gen__family x.Edit_family.family
  in Piqirun.gen_record code [ _person_lastname; _person_firstname; _family ]
and gen__edit_family_ok code x =
  let _index_person =
    Piqirun.gen_required_field 1 gen__protobuf_int32
      x.Edit_family_ok.index_person in
  let _family =
    Piqirun.gen_required_field 2 gen__family x.Edit_family_ok.family
  in Piqirun.gen_record code [ _index_person; _family ]
and gen__add_sibling_request code x =
  let _index =
    Piqirun.gen_required_field 1 gen__protobuf_int32
      x.Add_sibling_request.index in
  let _sex = Piqirun.gen_optional_field 2 gen__sex x.Add_sibling_request.sex
  in Piqirun.gen_record code [ _index; _sex ]
and gen__add_sibling code x =
  let _person_lastname =
    Piqirun.gen_required_field 1 gen__string x.Add_sibling.person_lastname in
  let _person_firstname =
    Piqirun.gen_required_field 2 gen__string x.Add_sibling.person_firstname in
  let _sibling =
    Piqirun.gen_required_field 3 gen__person x.Add_sibling.sibling
  in
    Piqirun.gen_record code [ _person_lastname; _person_firstname; _sibling ]
and gen__add_sibling_ok code x =
  let _index_person =
    Piqirun.gen_required_field 1 gen__protobuf_int32
      x.Add_sibling_ok.index_person in
  let _sibling =
    Piqirun.gen_required_field 2 gen__person x.Add_sibling_ok.sibling
  in Piqirun.gen_record code [ _index_person; _sibling ]
and gen__add_first_fam code x =
  let _sosa =
    Piqirun.gen_required_field 1 gen__person x.Add_first_fam.sosa in
  let _father =
    Piqirun.gen_required_field 2 gen__person x.Add_first_fam.father in
  let _mother =
    Piqirun.gen_required_field 3 gen__person x.Add_first_fam.mother in
  let _spouse =
    Piqirun.gen_required_field 4 gen__person x.Add_first_fam.spouse in
  let _children =
    Piqirun.gen_repeated_field 5 gen__person x.Add_first_fam.children
  in Piqirun.gen_record code [ _sosa; _father; _mother; _spouse; _children ]
and gen__auto_complete code x =
  let _field =
    Piqirun.gen_required_field 1 gen__auto_complete_field
      x.Auto_complete.field in
  let _place_field =
    Piqirun.gen_optional_field 2 gen__auto_complete_place_field
      x.Auto_complete.place_field in
  let _input =
    Piqirun.gen_required_field 3 gen__string x.Auto_complete.input in
  let _limit =
    Piqirun.gen_required_field 4 gen__protobuf_int32 x.Auto_complete.limit
  in Piqirun.gen_record code [ _field; _place_field; _input; _limit ]
and gen__auto_complete_result code x =
  let _result =
    Piqirun.gen_repeated_field 1 gen__string x.Auto_complete_result.result
  in Piqirun.gen_record code [ _result ]
and gen__person_search_list_params code x =
  let _lastname =
    Piqirun.gen_optional_field 1 gen__string
      x.Person_search_list_params.lastname in
  let _firstname =
    Piqirun.gen_optional_field 2 gen__string
      x.Person_search_list_params.firstname in
  let _limit =
    Piqirun.gen_required_field 3 gen__protobuf_int32
      x.Person_search_list_params.limit
  in Piqirun.gen_record code [ _lastname; _firstname; _limit ]
and gen__person_search_list code x =
  let _persons =
    Piqirun.gen_repeated_field 1 gen__person_search
      x.Person_search_list.persons
  in Piqirun.gen_record code [ _persons ]
and gen__transl_calendar code x =
  let _pos =
    Piqirun.gen_required_field 1 gen__calendar x.Transl_calendar.pos in
  let _sval = Piqirun.gen_required_field 2 gen__string x.Transl_calendar.sval
  in Piqirun.gen_record code [ _pos; _sval ]
and gen__config_transl_calendar code x =
  let _msg =
    Piqirun.gen_repeated_field 1 gen__transl_calendar
      x.Config_transl_calendar.msg
  in Piqirun.gen_record code [ _msg ]
and gen__transl_witness_type code x =
  let _pos =
    Piqirun.gen_required_field 1 gen__witness_type x.Transl_witness_type.pos in
  let _sval =
    Piqirun.gen_required_field 2 gen__string x.Transl_witness_type.sval
  in Piqirun.gen_record code [ _pos; _sval ]
and gen__config_transl_witness_type code x =
  let _msg =
    Piqirun.gen_repeated_field 1 gen__transl_witness_type
      x.Config_transl_witness_type.msg
  in Piqirun.gen_record code [ _msg ]
and gen__transl_precision code x =
  let _pos =
    Piqirun.gen_required_field 1 gen__precision x.Transl_precision.pos in
  let _sval =
    Piqirun.gen_required_field 2 gen__string x.Transl_precision.sval
  in Piqirun.gen_record code [ _pos; _sval ]
and gen__config_transl_precision code x =
  let _msg =
    Piqirun.gen_repeated_field 1 gen__transl_precision
      x.Config_transl_precision.msg
  in Piqirun.gen_record code [ _msg ]
and gen__transl_death_type code x =
  let _pos =
    Piqirun.gen_required_field 1 gen__death_type x.Transl_death_type.pos in
  let _sval =
    Piqirun.gen_required_field 2 gen__string x.Transl_death_type.sval
  in Piqirun.gen_record code [ _pos; _sval ]
and gen__config_transl_death_type code x =
  let _msg =
    Piqirun.gen_repeated_field 1 gen__transl_death_type
      x.Config_transl_death_type.msg
  in Piqirun.gen_record code [ _msg ]
and gen__transl_relation_parent_type code x =
  let _pos =
    Piqirun.gen_required_field 1 gen__relation_parent_type
      x.Transl_relation_parent_type.pos in
  let _sval =
    Piqirun.gen_required_field 2 gen__string
      x.Transl_relation_parent_type.sval
  in Piqirun.gen_record code [ _pos; _sval ]
and gen__config_transl_relation_parent_type code x =
  let _msg =
    Piqirun.gen_repeated_field 1 gen__transl_relation_parent_type
      x.Config_transl_relation_parent_type.msg
  in Piqirun.gen_record code [ _msg ]
and gen__transl_fevent_name code x =
  let _pos =
    Piqirun.gen_required_field 1 gen__fevent_name x.Transl_fevent_name.pos in
  let _sval =
    Piqirun.gen_required_field 2 gen__string x.Transl_fevent_name.sval
  in Piqirun.gen_record code [ _pos; _sval ]
and gen__config_transl_fevent_name code x =
  let _msg =
    Piqirun.gen_repeated_field 1 gen__transl_fevent_name
      x.Config_transl_fevent_name.msg
  in Piqirun.gen_record code [ _msg ]
and gen__transl_pevent_name code x =
  let _pos =
    Piqirun.gen_required_field 1 gen__pevent_name x.Transl_pevent_name.pos in
  let _sval =
    Piqirun.gen_required_field 2 gen__string x.Transl_pevent_name.sval
  in Piqirun.gen_record code [ _pos; _sval ]
and gen__config_transl_pevent_name code x =
  let _msg =
    Piqirun.gen_repeated_field 1 gen__transl_pevent_name
      x.Config_transl_pevent_name.msg
  in Piqirun.gen_record code [ _msg ]
and gen__transl_access code x =
  let _pos = Piqirun.gen_required_field 1 gen__access x.Transl_access.pos in
  let _sval = Piqirun.gen_required_field 2 gen__string x.Transl_access.sval
  in Piqirun.gen_record code [ _pos; _sval ]
and gen__config_transl_access code x =
  let _msg =
    Piqirun.gen_repeated_field 1 gen__transl_access
      x.Config_transl_access.msg
  in Piqirun.gen_record code [ _msg ]
and gen__transl_update_warning_js code x =
  let _pos =
    Piqirun.gen_required_field 1 gen__update_warning_js
      x.Transl_update_warning_js.pos in
  let _sval =
    Piqirun.gen_required_field 2 gen__string x.Transl_update_warning_js.sval
  in Piqirun.gen_record code [ _pos; _sval ]
and gen__config_transl_update_warning_js code x =
  let _msg =
    Piqirun.gen_repeated_field 1 gen__transl_update_warning_js
      x.Config_transl_update_warning_js.msg
  in Piqirun.gen_record code [ _msg ]
and gen__transl_short_greg_month code x =
  let _pos =
    Piqirun.gen_required_field 1 gen__short_greg_month
      x.Transl_short_greg_month.pos in
  let _sval =
    Piqirun.gen_required_field 2 gen__string x.Transl_short_greg_month.sval
  in Piqirun.gen_record code [ _pos; _sval ]
and gen__config_transl_short_greg_month code x =
  let _msg =
    Piqirun.gen_repeated_field 1 gen__transl_short_greg_month
      x.Config_transl_short_greg_month.msg
  in Piqirun.gen_record code [ _msg ]
and gen__transl_french_month code x =
  let _pos =
    Piqirun.gen_required_field 1 gen__french_month x.Transl_french_month.pos in
  let _sval =
    Piqirun.gen_required_field 2 gen__string x.Transl_french_month.sval
  in Piqirun.gen_record code [ _pos; _sval ]
and gen__config_transl_french_month code x =
  let _msg =
    Piqirun.gen_repeated_field 1 gen__transl_french_month
      x.Config_transl_french_month.msg
  in Piqirun.gen_record code [ _msg ]
and gen__transl_hebrew_month code x =
  let _pos =
    Piqirun.gen_required_field 1 gen__hebrew_month x.Transl_hebrew_month.pos in
  let _sval =
    Piqirun.gen_required_field 2 gen__string x.Transl_hebrew_month.sval
  in Piqirun.gen_record code [ _pos; _sval ]
and gen__config_transl_hebrew_month code x =
  let _msg =
    Piqirun.gen_repeated_field 1 gen__transl_hebrew_month
      x.Config_transl_hebrew_month.msg
  in Piqirun.gen_record code [ _msg ]
and gen__config code x =
  let _transl_cal =
    Piqirun.gen_required_field 1 gen__config_transl_calendar
      x.Config.transl_cal in
  let _transl_wit =
    Piqirun.gen_required_field 2 gen__config_transl_witness_type
      x.Config.transl_wit in
  let _transl_prec =
    Piqirun.gen_required_field 3 gen__config_transl_precision
      x.Config.transl_prec in
  let _transl_death =
    Piqirun.gen_required_field 4 gen__config_transl_death_type
      x.Config.transl_death in
  let _transl_rel =
    Piqirun.gen_required_field 5 gen__config_transl_relation_parent_type
      x.Config.transl_rel in
  let _transl_fevents =
    Piqirun.gen_required_field 6 gen__config_transl_fevent_name
      x.Config.transl_fevents in
  let _transl_pevents =
    Piqirun.gen_required_field 7 gen__config_transl_pevent_name
      x.Config.transl_pevents in
  let _transl_access =
    Piqirun.gen_required_field 8 gen__config_transl_access
      x.Config.transl_access in
  let _transl_warning =
    Piqirun.gen_required_field 9 gen__config_transl_update_warning_js
      x.Config.transl_warning in
  let _transl_short_greg_month =
    Piqirun.gen_required_field 10 gen__config_transl_short_greg_month
      x.Config.transl_short_greg_month in
  let _transl_french_month =
    Piqirun.gen_required_field 11 gen__config_transl_french_month
      x.Config.transl_french_month in
  let _transl_hebrew_month =
    Piqirun.gen_required_field 12 gen__config_transl_hebrew_month
      x.Config.transl_hebrew_month in
  let _gwf_place_format =
    Piqirun.gen_required_field 13 gen__string x.Config.gwf_place_format in
  let _gwf_place_format_placeholder =
    Piqirun.gen_required_field 14 gen__string
      x.Config.gwf_place_format_placeholder
  in
    Piqirun.gen_record code
      [ _transl_cal; _transl_wit; _transl_prec; _transl_death; _transl_rel;
        _transl_fevents; _transl_pevents; _transl_access; _transl_warning;
        _transl_short_greg_month; _transl_french_month; _transl_hebrew_month;
        _gwf_place_format; _gwf_place_format_placeholder ]
...
and gen__create_or_link code x =
  Piqirun.int32_to_signed_varint code
    (match x with | `create -> 0l | `link -> 1l | `create_default_occ -> 2l)
...
let gen_transl_relation_parent_type x =
  gen__transl_relation_parent_type (-1) x
  
let gen_config_transl_relation_parent_type x =
  gen__config_transl_relation_parent_type (-1) x
...
let gen_config_transl_update_warning_js x =
  gen__config_transl_update_warning_js (-1) x
  
let gen_transl_short_greg_month x = gen__transl_short_greg_month (-1) x
  
let gen_config_transl_short_greg_month x =
  gen__config_transl_short_greg_month (-1) x
