    ...
    sig
      type t =
        { mutable index : Api_app_piqi.protobuf_int32;
          mutable sex : Api_app_piqi.sex; mutable lastname : string;
          ...
          mutable surname_aliases : string list; mutable image : bool;
...
and
  Base_warnings :
    sig
      type t =
        { mutable already_defined : bool; mutable own_ancestor : bool;
          mutable bad_sex_of_married_person : bool;
          mutable birth_after_death : bool; mutable incoherent_sex : bool;
          ...
          mutable parent_too_young : bool; mutable title_dates_error : bool;
          mutable undefined_sex : bool; mutable young_for_marriage : bool;
          mutable close_children : bool; mutable parent_too_old : bool;
          mutable changed_order_of_marriages : bool;
          mutable big_age_between_spouses : bool; mutable dead_old : bool;
...
and parse_witness_event x =
  ...
  let (_witness, x) = Piqirun.parse_required_field 2 parse_protobuf_int32 x
  in
    (Piqirun.check_unparsed_fields x;
     {
...
and parse_event x =
  ...
  let (_witnesses, x) =
    Piqirun.parse_repeated_field 8 parse_witness_event x in
  let (_index_spouse, x) =
    Piqirun.parse_optional_field 9 parse_protobuf_int32 x
  in
    (Piqirun.check_unparsed_fields x;
     {
       Event.name = _name;
...
and parse_person x =
  ...
  let (_image, x) =
    Piqirun.parse_required_field 11 parse_bool x ~default: "\b\000" in
  ...
  let (_death_type, x) =
    Piqirun.parse_required_field 21 parse_death_type x in
  ...
  let (_related, x) =
    Piqirun.parse_repeated_field 28 parse_protobuf_int32 x in
  let (_rparents, x) =
    Piqirun.parse_repeated_field 29 parse_relation_parent x in
  let (_access, x) =
    Piqirun.parse_required_field 30 parse_access x ~default: "\b\000" in
  let (_parents, x) =
    Piqirun.parse_optional_field 31 parse_protobuf_int32 x in
  let (_families, x) =
    Piqirun.parse_repeated_field 32 parse_protobuf_int32 x in
  let (_events, x) = Piqirun.parse_repeated_field 33 parse_event x
  in
    (Piqirun.check_unparsed_fields x;
     {
       Person.index = _index;
...
and parse_family x =
  ...
  let (_witnesses, x) =
    Piqirun.parse_repeated_field 8 parse_protobuf_int32 x in
  ...
  let (_mother, x) =
    Piqirun.parse_required_field 11 parse_protobuf_int32 x in
  let (_children, x) = Piqirun.parse_repeated_field 12 parse_protobuf_int32 x
  in
    (Piqirun.check_unparsed_fields x;
     {
       Family.index = _index;
...
and parse_base_warnings x =
  let x = Piqirun.parse_record x in
  let (_already_defined, x) =
    Piqirun.parse_required_field 1 parse_bool x ~default: "\b\000" in
  let (_own_ancestor, x) =
    Piqirun.parse_required_field 2 parse_bool x ~default: "\b\000" in
  let (_bad_sex_of_married_person, x) =
    Piqirun.parse_required_field 3 parse_bool x ~default: "\b\000" in
  let (_birth_after_death, x) =
    Piqirun.parse_required_field 4 parse_bool x ~default: "\b\000" in
  let (_incoherent_sex, x) =
    Piqirun.parse_required_field 5 parse_bool x ~default: "\b\000" in
  let (_changed_order_of_children, x) =
    Piqirun.parse_required_field 6 parse_bool x ~default: "\b\000" in
  let (_children_not_in_order, x) =
    Piqirun.parse_required_field 7 parse_bool x ~default: "\b\000" in
  let (_dead_too_early_to_be_father, x) =
    Piqirun.parse_required_field 8 parse_bool x ~default: "\b\000" in
  let (_incoherent_ancestor_date, x) =
    Piqirun.parse_required_field 9 parse_bool x ~default: "\b\000" in
  let (_marriage_date_after_death, x) =
    Piqirun.parse_required_field 10 parse_bool x ~default: "\b\000" in
  let (_marriage_date_before_birth, x) =
    Piqirun.parse_required_field 11 parse_bool x ~default: "\b\000" in
  let (_mother_dead_before_child_birth, x) =
    Piqirun.parse_required_field 12 parse_bool x ~default: "\b\000" in
  let (_parent_born_after_child, x) =
    Piqirun.parse_required_field 13 parse_bool x ~default: "\b\000" in
  let (_parent_too_young, x) =
    Piqirun.parse_required_field 14 parse_bool x ~default: "\b\000" in
  let (_title_dates_error, x) =
    Piqirun.parse_required_field 15 parse_bool x ~default: "\b\000" in
  let (_undefined_sex, x) =
    Piqirun.parse_required_field 16 parse_bool x ~default: "\b\000" in
  let (_young_for_marriage, x) =
    Piqirun.parse_required_field 17 parse_bool x ~default: "\b\000" in
  let (_close_children, x) =
    Piqirun.parse_required_field 18 parse_bool x ~default: "\b\000" in
  let (_parent_too_old, x) =
    Piqirun.parse_required_field 19 parse_bool x ~default: "\b\000" in
  let (_changed_order_of_marriages, x) =
    Piqirun.parse_required_field 20 parse_bool x ~default: "\b\000" in
  let (_big_age_between_spouses, x) =
    Piqirun.parse_required_field 21 parse_bool x ~default: "\b\000" in
  let (_dead_old, x) =
    Piqirun.parse_required_field 22 parse_bool x ~default: "\b\000" in
  let (_old_individual, x) =
    Piqirun.parse_required_field 23 parse_bool x ~default: "\b\000" in
  let (_witness_date_after_death, x) =
    Piqirun.parse_required_field 24 parse_bool x ~default: "\b\000" in
  let (_witness_date_before_birth, x) =
    Piqirun.parse_required_field 25 parse_bool x ~default: "\b\000" in
  let (_fevent_order, x) =
    Piqirun.parse_required_field 26 parse_bool x ~default: "\b\000" in
  let (_fwitness_event_after_death, x) =
    Piqirun.parse_required_field 27 parse_bool x ~default: "\b\000" in
  let (_fwitness_event_before_birth, x) =
    Piqirun.parse_required_field 28 parse_bool x ~default: "\b\000" in
  let (_pevent_order, x) =
    Piqirun.parse_required_field 29 parse_bool x ~default: "\b\000" in
  let (_pwitness_event_after_death, x) =
    Piqirun.parse_required_field 30 parse_bool x ~default: "\b\000" in
  let (_pwitness_event_before_birth, x) =
    Piqirun.parse_required_field 31 parse_bool x ~default: "\b\000"
  in
    (Piqirun.check_unparsed_fields x;
     {
       ...
       Base_warnings.dead_too_early_to_be_father =
         _dead_too_early_to_be_father;
       ...
       Base_warnings.mother_dead_before_child_birth =
         _mother_dead_before_child_birth;
       ...
       Base_warnings.fwitness_event_before_birth =
         _fwitness_event_before_birth;
       Base_warnings.pevent_order = _pevent_order;
       Base_warnings.pwitness_event_after_death = _pwitness_event_after_death;
       Base_warnings.pwitness_event_before_birth =
         _pwitness_event_before_birth;
     })
...
and gen__title code x =
  ...
  let _date_begin =
    Piqirun.gen_optional_field 5 gen__date x.Title.date_begin in
...
and gen__witness_event code x =
  ...
  let _witness =
    Piqirun.gen_required_field 2 gen__protobuf_int32 x.Witness_event.witness
...
and gen__event code x =
  ...
  let _witnesses =
    Piqirun.gen_repeated_field 8 gen__witness_event x.Event.witnesses in
  let _index_spouse =
    Piqirun.gen_optional_field 9 gen__protobuf_int32 x.Event.index_spouse
  in
    Piqirun.gen_record code
      [ _name; _text; _date; _place; _reason; _note; _src; _witnesses;
        _index_spouse ]
and gen__person code x =
  ...
  let _public_name =
    Piqirun.gen_optional_field 6 gen__string x.Person.public_name in
  let _aliases = Piqirun.gen_repeated_field 7 gen__string x.Person.aliases in
  let _qualifiers =
    Piqirun.gen_repeated_field 8 gen__string x.Person.qualifiers in
  let _firstname_aliases =
    Piqirun.gen_repeated_field 9 gen__string x.Person.firstname_aliases in
  let _surname_aliases =
    Piqirun.gen_repeated_field 10 gen__string x.Person.surname_aliases in
  let _image = Piqirun.gen_required_field 11 gen__bool x.Person.image in
  let _birth_date =
    Piqirun.gen_optional_field 12 gen__date x.Person.birth_date in
  let _birth_place =
    Piqirun.gen_optional_field 13 gen__string x.Person.birth_place in
  let _birth_src =
    Piqirun.gen_optional_field 14 gen__string x.Person.birth_src in
  let _baptism_date =
    Piqirun.gen_optional_field 15 gen__date x.Person.baptism_date in
  let _baptism_place =
    Piqirun.gen_optional_field 16 gen__string x.Person.baptism_place in
  let _baptism_src =
    Piqirun.gen_optional_field 17 gen__string x.Person.baptism_src in
  let _death_date =
    Piqirun.gen_optional_field 18 gen__date x.Person.death_date in
  let _death_place =
    Piqirun.gen_optional_field 19 gen__string x.Person.death_place in
  let _death_src =
    Piqirun.gen_optional_field 20 gen__string x.Person.death_src in
  let _death_type =
    Piqirun.gen_required_field 21 gen__death_type x.Person.death_type in
  let _burial_date =
    Piqirun.gen_optional_field 22 gen__date x.Person.burial_date in
  let _burial_place =
    Piqirun.gen_optional_field 23 gen__string x.Person.burial_place in
  let _burial_src =
    Piqirun.gen_optional_field 24 gen__string x.Person.burial_src in
  let _occupation =
    Piqirun.gen_optional_field 25 gen__string x.Person.occupation in
  let _psources =
    Piqirun.gen_optional_field 26 gen__string x.Person.psources in
  let _titles = Piqirun.gen_repeated_field 27 gen__title x.Person.titles in
  let _related =
    Piqirun.gen_repeated_field 28 gen__protobuf_int32 x.Person.related in
  let _rparents =
    Piqirun.gen_repeated_field 29 gen__relation_parent x.Person.rparents in
  let _access = Piqirun.gen_required_field 30 gen__access x.Person.access in
  let _parents =
    Piqirun.gen_optional_field 31 gen__protobuf_int32 x.Person.parents in
  let _families =
    Piqirun.gen_repeated_field 32 gen__protobuf_int32 x.Person.families in
  let _events = Piqirun.gen_repeated_field 33 gen__event x.Person.events
  in
    Piqirun.gen_record code
      [ _index; _sex; _lastname; _firstname; _occ; _public_name; _aliases;
        _qualifiers; _firstname_aliases; _surname_aliases; _image;
        _birth_date; _birth_place; _birth_src; _baptism_date; _baptism_place;
        _baptism_src; _death_date; _death_place; _death_src; _death_type;
        _burial_date; _burial_place; _burial_src; _occupation; _psources;
        _titles; _related; _rparents; _access; _parents; _families; _events ]
and gen__family code x =
  let _index =
    Piqirun.gen_required_field 1 gen__protobuf_int32 x.Family.index in
  let _marriage_date =
    Piqirun.gen_optional_field 2 gen__date x.Family.marriage_date in
  let _marriage_place =
    Piqirun.gen_optional_field 3 gen__string x.Family.marriage_place in
  let _marriage_src =
    Piqirun.gen_optional_field 4 gen__string x.Family.marriage_src in
  let _marriage_type =
    Piqirun.gen_required_field 5 gen__marriage_type x.Family.marriage_type in
  let _divorce_type =
    Piqirun.gen_required_field 6 gen__divorce_type x.Family.divorce_type in
  let _divorce_date =
    Piqirun.gen_optional_field 7 gen__date x.Family.divorce_date in
  let _witnesses =
    Piqirun.gen_repeated_field 8 gen__protobuf_int32 x.Family.witnesses in
  let _fsources =
    Piqirun.gen_optional_field 9 gen__string x.Family.fsources in
  let _father =
    Piqirun.gen_required_field 10 gen__protobuf_int32 x.Family.father in
  let _mother =
    Piqirun.gen_required_field 11 gen__protobuf_int32 x.Family.mother in
  let _children =
    Piqirun.gen_repeated_field 12 gen__protobuf_int32 x.Family.children
  in
    Piqirun.gen_record code
      [ _index; _marriage_date; _marriage_place; _marriage_src;
        _marriage_type; _divorce_type; _divorce_date; _witnesses; _fsources;
        _father; _mother; _children ]
and gen__base_warnings code x =
  let _already_defined =
    Piqirun.gen_required_field 1 gen__bool x.Base_warnings.already_defined in
  let _own_ancestor =
    Piqirun.gen_required_field 2 gen__bool x.Base_warnings.own_ancestor in
  let _bad_sex_of_married_person =
    Piqirun.gen_required_field 3 gen__bool
      x.Base_warnings.bad_sex_of_married_person in
  let _birth_after_death =
    Piqirun.gen_required_field 4 gen__bool x.Base_warnings.birth_after_death in
  let _incoherent_sex =
    Piqirun.gen_required_field 5 gen__bool x.Base_warnings.incoherent_sex in
  let _changed_order_of_children =
    Piqirun.gen_required_field 6 gen__bool
      x.Base_warnings.changed_order_of_children in
  let _children_not_in_order =
    Piqirun.gen_required_field 7 gen__bool
      x.Base_warnings.children_not_in_order in
  let _dead_too_early_to_be_father =
    Piqirun.gen_required_field 8 gen__bool
      x.Base_warnings.dead_too_early_to_be_father in
  let _incoherent_ancestor_date =
    Piqirun.gen_required_field 9 gen__bool
      x.Base_warnings.incoherent_ancestor_date in
  let _marriage_date_after_death =
    Piqirun.gen_required_field 10 gen__bool
      x.Base_warnings.marriage_date_after_death in
  let _marriage_date_before_birth =
    Piqirun.gen_required_field 11 gen__bool
      x.Base_warnings.marriage_date_before_birth in
  let _mother_dead_before_child_birth =
    Piqirun.gen_required_field 12 gen__bool
      x.Base_warnings.mother_dead_before_child_birth in
  let _parent_born_after_child =
    Piqirun.gen_required_field 13 gen__bool
      x.Base_warnings.parent_born_after_child in
  let _parent_too_young =
    Piqirun.gen_required_field 14 gen__bool x.Base_warnings.parent_too_young in
  let _title_dates_error =
    Piqirun.gen_required_field 15 gen__bool x.Base_warnings.title_dates_error in
  let _undefined_sex =
    Piqirun.gen_required_field 16 gen__bool x.Base_warnings.undefined_sex in
  let _young_for_marriage =
    Piqirun.gen_required_field 17 gen__bool
      x.Base_warnings.young_for_marriage in
  let _close_children =
    Piqirun.gen_required_field 18 gen__bool x.Base_warnings.close_children in
  let _parent_too_old =
    Piqirun.gen_required_field 19 gen__bool x.Base_warnings.parent_too_old in
  let _changed_order_of_marriages =
    Piqirun.gen_required_field 20 gen__bool
      x.Base_warnings.changed_order_of_marriages in
  let _big_age_between_spouses =
    Piqirun.gen_required_field 21 gen__bool
      x.Base_warnings.big_age_between_spouses in
  let _dead_old =
    Piqirun.gen_required_field 22 gen__bool x.Base_warnings.dead_old in
  let _old_individual =
    Piqirun.gen_required_field 23 gen__bool x.Base_warnings.old_individual in
  let _witness_date_after_death =
    Piqirun.gen_required_field 24 gen__bool
      x.Base_warnings.witness_date_after_death in
  let _witness_date_before_birth =
    Piqirun.gen_required_field 25 gen__bool
      x.Base_warnings.witness_date_before_birth in
  let _fevent_order =
    Piqirun.gen_required_field 26 gen__bool x.Base_warnings.fevent_order in
  let _fwitness_event_after_death =
    Piqirun.gen_required_field 27 gen__bool
      x.Base_warnings.fwitness_event_after_death in
  let _fwitness_event_before_birth =
    Piqirun.gen_required_field 28 gen__bool
      x.Base_warnings.fwitness_event_before_birth in
  let _pevent_order =
    Piqirun.gen_required_field 29 gen__bool x.Base_warnings.pevent_order in
  let _pwitness_event_after_death =
    Piqirun.gen_required_field 30 gen__bool
      x.Base_warnings.pwitness_event_after_death in
  let _pwitness_event_before_birth =
    Piqirun.gen_required_field 31 gen__bool
      x.Base_warnings.pwitness_event_before_birth
  in
    Piqirun.gen_record code
      [ _already_defined; _own_ancestor; _bad_sex_of_married_person;
        _birth_after_death; _incoherent_sex; _changed_order_of_children;
        _children_not_in_order; _dead_too_early_to_be_father;
        _incoherent_ancestor_date; _marriage_date_after_death;
        _marriage_date_before_birth; _mother_dead_before_child_birth;
        _parent_born_after_child; _parent_too_young; _title_dates_error;
        _undefined_sex; _young_for_marriage; _close_children;
        _parent_too_old; _changed_order_of_marriages;
        _big_age_between_spouses; _dead_old; _old_individual;
        _witness_date_after_death; _witness_date_before_birth; _fevent_order;
        _fwitness_event_after_death; _fwitness_event_before_birth;
        _pevent_order; _pwitness_event_after_death;
        _pwitness_event_before_birth ]
