...
carried out by a small team of dedicated volunteers (A2, Henri Gouraud,
michelmno) with significant technical contributions from Pierre Villemot at
OcamlPro, canonici and ilankri at Geneanet and valuable input from the
community. Special recognition to Claude AI models for development assistance.

This version brings major areas of improvement: a complete infrastructure
modernization with dune/opam build chain maintenance, the full implementation
of a RPC server and an option to cache the database in memory, extensive
performance optimizations particularly beneficial for large databases like
Roglo, and a comprehensive suite of new genealogical tools that significantly
enhance the user experience for both data entry and analysis.

## Templates & User Interface
The template system has undergone extensive modernization, introducing powerful
new tools for genealogists while improving the overall user experience. The
interface now benefits from enhanced JavaScript optimization, better caching
mechanisms, and a more consistent design language.

### New Genealogical Tools
Seven new reference books have been added to facilitate data entry and ensure
data consistency across the database. These books cover firstname aliases,
surname aliases, public names, qualifiers, general aliases, titles, and domains.
Each book provides a cache for autocomplete functionality and helps maintain
standardized entries across large databases (A2).

A new image mapping tool has been introduced, created in 2019 by Ludovic Ledieu
(1974-2024), allowing detailed annotation of historical photographs and
documents. This tool enables users to identify and link individuals within
images, creating an interactive experience for enhancing family photographs (A2)
Images can be either isolated or regrouped into albums (Michel de Lambilly).

The Fanchart visualization has been completely revamped, building on Ludovic
Ledieu's original work with improved rendering and interaction capabilities
(A2).

A comprehensive coat of arms management system has been integrated into the
carousel mechanism, allowing heraldic images to be associated with multiple
individuals (Michel de Lambilly).

The carousel system itself has been reworked to better handle multiple images,
coats of arms, and other media associated with an individual, moving beyond the
simple portrait limitation (A2 and Henri Gouraud).

### Data Quality and Analysis Tools
A new data checking tool (`m=CHK_DATA`) has been introduced to identify common
typographic errors in the reference books. This tool uses caching mechanisms to
efficiently scan large databases and suggest corrections, helping maintain data
quality across millions of records (A2).

The RLM (Multiple Relation Link) builder provides a JavaScript-based interface
accessible from the menubar, allowing users to manually construct complex
relationship matrices between individuals. This tool integrates with the
modernized graph editors for `upd_RLM` and `DAG`, providing a complete workflow
for relationship analysis (A2).

A new Relation Matrix tool (`m=RM`) calculates and displays multiple
relationship links between individuals, helping genealogists visualize
relationship percentages and the links between them (A2).

### Search and Navigation Improvements
The search system has been significantly reworked, delivering major performance
improvements. Search operations that previously took 30+ seconds on large
databases now complete in under 10 seconds, thanks to algorithmic optimizations
and better use of caching (A2 and Henri Gouraud).

Input autocompletion has been enhanced to work either through local cache files
or via the new RPC server, providing responsive suggestions even on databases
with hundreds of thousands of unique values (Halbaroth, Henri Gouraud and A2).

The full display mode for individuals has been modernized, with improved
ascending and descending view layouts that better utilize screen space (A2).

Notes display has been modernized with a tree view structure for directory
listings, making it easier to navigate complex note hierarchies (A2).

The database index now provides direct links to all available databases on the
server, simplifying multi-database navigation (Henri Gouraud).

The homonyms list has been reworked to provide more information for better
identification of individuals with similar names (Henri Gouraud).

### Automatic Updates and Cross-References
Person references in notes and image maps are now automatically updated when
individuals are modified or merged, maintaining data consistency across the
database without manual intervention (Henri Gouraud).

### Form Enhancements
Data entry forms now feature floating placeholders that improve the user
experience by clearly indicating what information belongs in each field (A2).

A new Ctrl+click functionality provides direct links from input fields to the
corresponding reference books for wizards, allowing users to quickly verify or
add standardized entries without leaving the form (A2).

### New Template Variables
Several new template variables enhance the flexibility of custom templates:
* **Place details**: `%xxx_place.main;` and `%xxx_place.subplace;` for all event
types (birth, marriage, death, etc.) - allows separate display of main location
and subdivision
* **Marriage span**: `%marriage_age_years;` - provides the marriage duration
* **Predictable mode**: `predictable_mode` boolean - enables deterministic
template rendering for testing purposes
* **Roglo support**: `roglo=yes` - activates Roglo-specific template extensions

### Event Display
Events in the NMBDS (baptism, marriage, death, burial) display now include notes
and sources directly in the interface, with the option to hide these tooltips
through the `event_tooltips=no` configuration parameter (A2).

### Access Control
The access system has been reworked to include a new SemiPublic status for
Roglo, providing finer-grained control over data visibility.
A new field has been added in the .auth file providing the key in the base of
identified users. This allow one click display of the path between the visitor
and the visited person (Henri Gouraud).

## Core Infrastructure (Pierre Villemot@OcamlPro)
Pierre Villemot at OcamlPro led a comprehensive infrastructure modernization
that transforms how GeneWeb is built, deployed, and maintained. These changes
provide a solid foundation for future development while improving performance
and maintainability.

### Cache in Memory, RPC Server and Backend Architecture
The groundbreaking memory caching system introduced through the `-cache-in-
memory <BASE>` parameter for `gwd` and developed by Keryan Didier at OcamlPro
has been maintained operationally by Pierre Villemot. This feature preloads
entire databases into memory at launch time using the `ocaml-ancient` library to
share read-only data across forked processes without duplication. The system
improves response times on large databases. This feature requires the no-naked-
pointers compiler variant and is Unix-only due to dependencies on `nnp` and
`ancient` libraries. It can be enabled at build time through `./configure --gwd-
caching`. The tradeoff is a longer server startup time (approximately 30 seconds
for large databases) as all data is preloaded.

A complete RPC server implementation has been introduced, featuring a full
search engine that can be deployed separately from the main web server. This
architecture enables better scalability and allows specialized optimizations for
search operations. The RPC server uses cmdliner for command-line parameter
handling and integrates seamlessly with the existing GeneWeb infrastructure.

The logging system has been completely refactored to use the standard `logs` and
`logs-syslog` libraries, replacing the previous custom logging implementation.
This provides better integration with modern Unix system logging and more
flexible log management.

Core libraries have been reorganized, with `gwdb` and `sosa` receiving
particular attention to improve modularity and reduce coupling between
components. The lock system that manages concurrent access to databases has been
refactored for better reliability and performance.

A simple worker pool has been added to `gwd`, controlled by the `-n_workers`
parameter. This allows the web server to handle multiple requests concurrently
while maintaining database consistency.

The legacy `gwrepl` tool has been removed as its functionality has been
superseded by more modern approaches.

### Build System
The `configure.ml` script and opam configuration files have been simplified and
updated. Support for Nix has been added, enabling reproducible builds and easier
deployment in Nix-based environments. GeneWeb's development environment now uses
`DUNE_PROFILE=release` or `dev` to control optimization levels and debugging
information.

Dependencies have been significantly updated. OCaml version 4.10 is now the
minimum requirement, with support extending up to OCaml 5.4.0. Dune requires
version 3.6 or later, and camlp5 needs version 8.03 minimum. New dependencies
include `decompress` for compression support, `logs` and `logs-syslog` for
logging, `yojson` for JSON handling, `digestif` for hashing, `pp_loc` for error
reporting, `utop` for interactive development, and `ancient` as an optional
dependency for in-memory caching optimization. The `num` library has been
removed (eliminating the `--sosa_num` configure option), and the custom`syslog`
implementation has been replaced by the standard library.

### Testing and Continuous Integration
All Github CI workflows have been optimized and extended. Added a native ARM64
Docker support (A2). The Dockerfile created by Florian Piesched in 2024) has
been maintained (Jeffery Fernandez) and upgraded to support in-memory cache
option (Pierre Villemot and A2). A Google Colaboratory script has been added
for online quick testing, enabling rapid validation of changes without local
environment setup (A2). A new `run-gw-test.sh` test script framework facilitates
automated database and template testing (michelmno and Henri Gouraud).

## Base Reorganization
A new database reorganization system available through the `-reorg` parameter
has been implemented. This feature restructures the file and directory layout of
databases optimizing storage of data linked to a single database (Henri Gouraud)

## Web Performance & Optimization (A2)
Significant performance improvements have been achieved across the web
interface, with particular focus on reducing bandwidth usage and improving
response times for large databases.

### Compression
Dynamic HTML compression has been implemented with configurable gzip compression
levels (0-9) through the `gzip_html_compression` configuration parameter. This
can reduce HTML transfer sizes by 70-80% for typical genealogical pages,
significantly improving load times on slower connections.

Static assets including CSS and JavaScript files are now pre-compressed and
served with `.gz` extensions, eliminating compression overhead on each request
while maintaining the bandwidth benefits.

### JavaScript and Asset Optimization
JavaScript handling has been optimized through better DOMContent loading
strategies, reducing the time before pages become interactive. Static assets now
include hash-based versioning, enabling aggressive browser caching while
ensuring users always receive the latest versions when files change.

### Cousin Caching Improvements
The cache file system has been enhanced with better performance characteristics.
The cousins calculation tool (`m=C`) now supports database-specific caching
through `cache_cousins_tool` and `cache_cousins_ttl` configuration parameters,
reducing calculation times for frequently accessed relationship data.

## Configuration Parameters (GWF)
The `.gwf` configuration file format has been extended with numerous new
parameters that control the advanced features introduced in this release:

**Compression and Performance:**
- `gzip_html_compression=0-9` sets the HTML compression level (0=off, 9=maximum)
- `cache_cousins_tool=yes` enables cousin calculation caching
- `cache_cousins_ttl=72` sets cache time-to-live in hours

**RPC Server Integration:**
- `rpc_server_url=myserver.org` specifies the RPC
server location
- `rpc_datalist=1` enables RPC-based datalists for
autocompletion

**Template Features:**
- `roglo=yes` activates Roglo-specific template extensions
- `event_tooltips=no` hides event notes and sources in tooltips

**Form Autocompletion:**
A new set of datalist parameters control which fields offer autocompletion:
`datalist_aliases`, `datalist_estates`, `datalist_fnames`, `datalist_occupations`,
`datalist_places`, `datalist_pub_names`, `datalist_qualifiers`,
`datalist_snames`, `datalist_titles`, and `datalist_sources`. Each can be set to
`1` to enable autocompletion for that field type.

## Tools & Utilities
Several command-line tools have been enhanced or introduced:
The `cache_files.exe` utility now accepts additional parameters for generating
binary caches used by the Check Data tool and supports all reference book types
(Henri Gouraud and A2).

A new `robot.exe` tool provides IP blacklist management with various parameters,
helping administrators control access and prevent abuse (A2).

The `gwc` compiler now includes a `-nopublic` option for processing databases
with privacy restrictions (Geneanet).

The `gwd` web server accepts the `-n_workers #` parameter to control the worker
pool size (Pierre Villemot).

## Translations
Community translators have contributed updates and new translations: Corsican
language support (Patriccollu), German improvements (ftrojahn), and Portuguese
enhancements (smmribeiro).

## Bug Fixes and Code Quality
Throughout the development cycle, hundreds of code optimizations, bug fixes, and
cleanup operations have been performed across the codebase. These improvements
enhance stability, maintainability, and performance, though they are too
numerous to list individually. The cumulative effect is a more robust and
reliable platform for genealogical research.

---
This release represents a significant step forward for GeneWeb, balancing
infrastructure improvements that ensure long-term sustainability with user-
facing features that enhance the genealogical research experience. The
foundation laid by this work enables continued evolution of the platform to meet
the needs of both small family databases and large-scale genealogical projects
like Roglo.


# GeneWeb version 7.0.0 and 7.1~alpha
Various minor corrections and updates in template files.

Added a "Relationships" button in the "Relationships" drop-down menu (m=C).
This feature includes an extended table of potential "cousin" relationships
up to an ascending degree v (v1/v2 represent ascending/descending degrees).
Note that "self" is now represented as v=v1=0 (previously 1) for consistency.
Please be aware that some translations for relationships are still pending.
A new .gwf parameter "cache_cousins_tool=yes" allow the use of a cache file
to help with loading time on big databases.

When browsing with a Sosa reference and the individual has a relationship with
the Cujus, the relationship name is now provided along with the names of common
ancestor(s) and the shortest path graph between them (m=RL). This feature also
considers half-relationships with a lone common ancestor.

The "places by surname" tool (m=PPS) on the welcome page has been updated. It
now explicitly lists persons related to a given place and offers the ability to
construct a multi-relation tree between these individuals. Additionally, a link
back to the places dictionary is provided.

A new "carousel" mechanism (m=SND_IMAGE_C) has been added, allowing for the
archival of several pictures associated with an individual, beyond the existing
portrait.

Introducing the new command m=DOC, similar to m=SRC, which now allows loading
PDF and HTML files. You can display the content of a PDF file using "<a
href="%sm=DOC&s=file.pdf">pdf file</a>". Unlike image files, PDF and HTML files
cannot be included in an <img src="..."> command. Arguments for m=SRC and m=DOC
can now be located in subfolders of the current images/basename and src/basename
folders.

Most template pages now feature new buttons to change image/spouse variables.
Portraits and spouses are now shown by default, but you can hide them by using
variables im=0 and sp=0 in the URLs.

A new bar of buttons for trees is now available at the bottom of pages. Switch
between tree types: agnatic/ascending/cognatic/descending/v/short. Navigate to
father/mother/siblings when available. The "<>" button switches the positions
of spouses (male by default on the left). "V" represents a descending tree in
template format, similar to the old one. "Short" uses Jean Vaucher's algorithm
tto reduce the width of trees.

In ascending trees, the marriage year is now replaced by a marriage-divorce or
relation-separation period, with information on widowhoods available in
tooltips.

A new parameter "visitor_access=no" in the .gwf file prevents access to the base
for anyone except friends and wizards.

The lexicon now supports an alias mechanism that allows for multiple names for
the same set of translations using "->:" (similar to language prefixes).

Accessing the server without specifying a base name opens the index.txt file.
This file allows users to capture the base name and now provides a list of all
available bases. If only one base exists, its welcome page is opened directly.

While searching, lists of homonyms are expanded with parents and spouses' names
for easier identification. The same details are shown in the merge tool (m=MRG).

If calculable, the individual's age in days is now given on the tooltip of the
icon (from templm).

In the ascendant table by couples (m=A&m=T), the middle column now displays the
number of children of the couple before the marriage event. All other spouses'
children and relations are shown in small text in blue/red (him/her). Relations
are printed in chronological order. The spouses of those other relations are
shown in the tooltip of the event, providing a better view of the various
existing families and a quick count of the total children of both ascendants.

In the Ancestors menu (m=A), the selection of generations is enhanced with
statistics, including the period of the generation, the number of added
individuals and implexes by generations, the total number of individuals and
implexes up to this generation, and the percent of completion of the generation
in the tooltip. This feature is allowed thanks to select2.min.js jQuery scripts
and may be resource-intensive for a large number of ancestors and implexes.

Individual and family forms (m=MOD_IND/MOD_FAM) now include three new components
for witnesses: an "occupation" input and two checkboxes for "maybe dead" and
"public." The types of witnesses and relationships have been extended. If "link"
is selected, a hyperlink to the witness' page is displayed.

In the authentication files for wizards and friends, the username can now be
expanded into username|userkey, where userkey is first_name.ocurence.surname of
the user in this base. This allows for a quick link to that individual after
login.

In "Books" (m=MOD_DATA), a new parameter "book_max_results" in gwf limits the
results of books, with the default set at 1000. Additionally, a new anchor
after a modification allows for a fallback to the edited entry, and a new
search input facilitates multiple corrections. Improved breadcrumb trail and
navigation are also included.

Home buttons are now set on the top right for all pages. Two new icons have
been added to facilitate debugging: - The hourglass icon indicates the CPU load
time of the gwd request, with a color indicator (green/yellow/red) based on
speed. The load time is provided in the tooltip of that icon. - The bug icon
indicates coding errors on the page, and its tooltip provides information about
the number of errors found while loading the page, including a list of
undefined variables. These new buttons can be hidden with the new gwf parameter
"hide_compilation_time=yes". Please report any issues if you encounter errors
in gwd logs/console.

The number of connected wizards/friends is now displayed on the right in the
footer, next to copyrights.

The "default_lang" parameter set in gwf is highlighted in the language select
menu if the language has been changed.

New inputs have been added in the population pyramid form, including interval
and limit, which were previously only available by modifying the URL.

A minimized monospace font based on DejaVu Sans Mono is now loaded for a better
rendering of the descending tree view, containing about 50 ASCII box-drawings
characters. The files are located at /webfonts/dejavusansmono-
genewebfont.woff/2.

The "static_path" parameter in gwf has been depreciated,
and is now set at the launch of gwd.
gwd takes three possible launch parameters:
-hd designates the location of gw. From this location
    gw/etc and gw/images can be derived.
-etc_prefix designates etc if it is not located in gw.
-images_prefix designates images if it is not located in gw
In templates, %gw_prefix;, %etc_prefix;, %images_prefix; provide these values.
These values provide the necessary directory separator when prepended to a file.
(For retro compatibility purposes, %static_path; = %etc_path; and
%ipage_prefix = %images_prefix;)

Remove GWPREFIX not used anymore during Unix install,
as "man" pages not maintained anymore either since geneweb 7.0.0

# GeneWeb version 7.0.0
This changelog note tries to reconstruct changes in GeneWeb since version 6.08.
It may miss some details which might be added later if relevants.

- Development environment: GeneWeb's development environment has been shifted to opam+dune,
  and relies on a variety of external libraries. Dependencies are managed by opam.
  See README for installation instructions

- Code revision: GeneWeb code has been profoundly reorganised:
  - files are now separated into a /bin and /lib folders.
  - some functions have been isolated as external libraries and/or separate
    repositories (contrib, unidecode)
  - some files have been split in two parts, isolating the display functions from
    the other functions
  - front-end (GUI, API) functions and back-end (database) functions have been isolated
    to facilitate development and direct plugin of new front and back-end solutions.
  - many code factorization, optimisation and bug fixes

- Archival format: the archival (gw) format has been upgraded to gwplus. gwc v7 reads
  old .gw files, and gwu v7 can write old format files when launched with the parameter -old_gw.
...
- Events are now first class citizens in GeneWeb (personal events and family events).
  ...
  The current implementation results in redundant data in the .gw file as birth, baptism, marriage,
  death and burial are both reported in the traditional way (part of the person or family record)
  and as personal or family events.

- History: bug fixes

- GUI: the look of GeneWeb's interface has been revamped with a modern tool: html 5 and Bootstrap,
  resulting in a better compatibility between screen formats (PC, tablet, phone).
  Icons are display as font elements rather than images.
...
-Templates: templates have been revised in multiple fashion
  ...
  - the variety of templates (a, b, c, ...) has been collapsed into a mechanism allowing
    interactive selection of modules to form a page (left-most icon on the menubar).
    Modules include identity, unions, parents, grand-parents, relations, notes,
    sources, chronology and various trees.
    Each module may take several forms inspired by the old a, b, c, .. templates.
    A standard mechanism allows to add private modules.
    A .gwf variable (p_mod) allows memorization of a particular module configuration
    for a given base (see a.gwf for details).
  - whenever possible, clickable items on a page point to the designated person
    (first_name surname), its update page (sex of dates), its family update page (marriage date).

- Advanced search: the advanced search page, and the underlying search mechanisms have
  been updated for generality and efficiency. This is an ongoing effort.
...
- gwd: new launch parameters
  ...
  - trace_templ -> Brackets template files with begin/end html comments (<!-- begin xxx ->)
    including the full path name of the file

- gwf:
    new variables in the gwf file are described in the a.gwf default configuration file
...
* ! New ! You can try the new graphical interface of GeneWeb. It's supposed
  ...
  thing is not finished yet. Feedback is welcome.
...
* Add a field in the advanced request to search in the marriage information.
...
** perso.txt :
     ...
     - Open the update individual/family form in a new page and automatic
       reload after clicking on the [x] at the bottom of the page.
...
* Added a new instruction in the template language : FOR.
...
* New implementation of the function that calculate if a person has a sosa
...
* Fix the duplicates of address in gwc2. Now the book of places if
...
* Added a book of places (see a.gwf to specify the maximum number of
  ...
  each modifications (because it will recalculate the indexes).
...
* Change "href=#" by "void" because internet explorer doesn't support it (go
...
family note/source comes before the information of death of the individual.
...
Few changes since 6.00 (which was set up to start a new development of
GeneWeb).
...
* Fix bug in hiding individual of the same family.
...
* New features : Wiki syntax and miscellaneous Notes
  - [06 Oct 05] It is now possible to use a specific ("Wiki") syntax
    ...
    forum. It is also possible to add new "miscellaneous notes". The
    ...
    - The syntax [[[name/text]]] makes a link to a "miscellaneous note"
      ...
      another (recursively) linked miscellaneous note). Simplified syntax
...
* Base configuration files (base.gwf)
  - [22 Oct 02] Added configuration variable public_if_no_date: all
    ...
    "access" field has been explicitly set to "Private" (default: no).
...
* GeneWeb server or CGI (gwd)
  ...
  - [14 Mar 01] Added option "-blang": if no language is specified in
    the request (i.e. explicitly selected by a flag, for example), choose
...
* Installation
  ...
  - [03 Mar 01] Changed Linux rpm package installation; it uses now
    ...
    when uninstallation, it can be used for the future GeneWeb versions,
...
* GeneWeb server or CGI (gwd)
  ...
  - [26 Oct 00] In forum, added "<table>..</table>" around messages
    to prevent unterminated tags to perturb the other messages.
...
* GeneWeb Service/CGI (gwd)
  - [25 Sep 00] When wizard or friend access failure, now display the
    page without the privileged access (instead of just saying "access
    failure").
...
* Gedcom to GeneWeb (ged2gwb)
  - [09 Aug 00] Added a consistency check: in some gedcoms (from bad
    genealogy software), it happens that a person can be noticed as
...
* GeneWeb Service/CGI (gwd)
  - [04 Jul 00] Put all statistics in a different page (accessible only
...
* GeneWeb Service/CGI (gwd)
  ...
  - [02 May 00] [technical] In Unix version now, each request gets the
    ...
    options -setuid and -setgid now useless.
...
* Base configuration files (base.gwf)
  - [31 Mar 00] Added variable "places_inverted" (default no) to specify that
    ...
    instead of "New York, USA"). Important to specify correctly this variable
...
* GeneWeb Service/CGI (gwd)
  ...
  - [24 Feb 00] In latest births and deaths pages, display months.
...
* Setup program (gwsetup)
  - [Oct 28, 99] Added option -p to select another port number (default: 2316).
...
* GeneWeb Service/CGI (gwd)
  ...
  - [Aug 19, 99] Added more information (places) in descendants display
    by list.
...
* GeneWeb to Gedcom (gwb2ged)
  - [Jul 15, 99] Added option "-charset" whose values can be "ANSEL" or
    "ASCII": "ASCII" is the default. Deleted option "-ascii" now useless.
...
* gwb2ged:
  - start now pointers with 1 instead of 0 because some software seem not
    to accept @I0@ and @F0@.
...
* gwd:
  ...
  - changed: displaying shortcuts when accessing by titles: for example,
    ...
    selecting "king" directly displays "kings of France".
