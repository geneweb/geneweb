(* $Id: wserver.ml,v 2.10 1999-08-05 17:03:36 ddr Exp $ *)
(* Copyright (c) INRIA *)
...
do W.iter (fun k v -> Printf.eprintf " --- address %s%s%d requests since %.0f seconds\n" k (String.make (18 - String.length k) ' ') (List.length v) (tm -. List.hd (List.rev [tm :: v]))) who.val; flush stderr; return
        if cnt > max_call then
          let str1 =
            match addr with
            [ Unix.ADDR_UNIX s -> s
            | Unix.ADDR_INET a _ ->
                try (Unix.gethostbyaddr a).Unix.h_name with _ ->
...
ifdef UNIX then
  ...
  [ 0 ->
      do if Unix.fork () <> 0 then exit 0 else ();
...
It will be reconnected at next server restart.\n" cnt sec
      ...
      return ()
  | pid ->
      let _ = Unix.waitpid [] pid in Unix.close t ]
else ()
;
...
value f port tmout max_clients (uid, gid) robot_excluder g =
