...
class DatabasePatches:
    ...
    def _patch_misc_names(self, ip: int, person: GenPerson) -> None:
        ...
        misc_names = NameUtils.person_misc_names(
            self.base,
            person,
            lambda p: p.titles
        )
    ...
    def _delete_person_aux(
        self,
        excluded_ipers: Set[int],
        excluded_ifams: Set[int],
        iper: int
        ...
        if ascend.parents is not None:
            ...
            if len(children) == 1 and children[0] == iper:
                ...
                if (self._is_empty_person(father, ifam) and
                    self._is_empty_person(mother, ifam)):
    ...
    def _is_empty_person(self, iper: int, exclude_ifam: Optional[int] = None) -> bool:
        ...
        # Check if person is "? ?"
        return (person.first_name == Istr.quest() and
                person.surname == Istr.quest())
