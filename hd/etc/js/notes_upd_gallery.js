    ...
    initFrameHandlers: function() {
        ...
        frame.addEventListener('mouseup', () => {
            ...
            if (this.redrawMode) {
                ...
                // Clear all data-* on redraw-target (jQuery .removeData() equivalent)
                if (redrawTarget) {
...
const TableManager = {
    ...
    init: function() {
        ...
        if (mapTable) {
            // Delegated handlers — mirror jQuery .on('click', selector, handler)
...
document.addEventListener('mouseout', (e) => {
    ...
    // originalGroup was stored via jQuery .data() in legacy code; read dataset
    // defensively — see P3/P5 where the setter is migrated.
...
function initializeHandlers() {
    ...
    // ---- Delegation helper: call `fn` with `el` as `this` when the target
    // ---- matches `selector` inside `root` (mirrors jQuery .on(evt, sel, fn))
    ...
    table
        ...
        .on('click', '.redraw-area', function(e) {
            ...
            if (redrawTarget) {
                ...
                // NOTE: legacy code stored `source-button: this` via jQuery .data().
                // The reference was never read elsewhere — dropped in migration.
            }
    ...
    // -------------------------------------------------------------------------
    // Global mouseleave from groups legend or grp_table: reset highlighting
    // Legacy listener was on "*" (every descendant). Replaced by a single
    // mouseleave on each container — same net effect, far fewer events fired.
    // -------------------------------------------------------------------------
...
function resetButtonHandler() {
    ...
    clearBtn.addEventListener('click', () => {
        ...
        // NOTE: legacy `removeData('maphilight-configured')` on every <area>
        // was a phantom flag (see migration notes). Dropped.
        if (img) maphilight.refresh(img);
        const redrawTarget = document.getElementById('redraw-target');
        if (redrawTarget) {
            Object.keys(redrawTarget.dataset).forEach(k => delete redrawTarget.dataset[k]);
    ...
    if (imgData.img) {
        ...
        imgEl.addEventListener('load', function onLoad() {
            ...
            requestAnimationFrame(() => {
                ...
                // setTimeout(50) dropped — v2 maphilight handles re-init idempotently
...
function setupFormHandler() {
    ...
    const onSubmit = (event) => {
        ...
        // Remove own handler then re-submit natively (jQuery .unbind('submit').submit() equivalent)
    ...
    img.addEventListener('load', function() {
        ...
        requestAnimationFrame(() => {
            ...
            requestAnimationFrame(() => {
                ...
                // setTimeout(100) dropped: v2 maphilight is idempotent
                maphilight(img, MAPHILIGHT_CONFIG);
            });
        });
    });
    ...
    imageChange: function() {
        ...
        imgEl.addEventListener('load', function onLoad() {
            ...
            requestAnimationFrame(() => {
                ...
                toggleTables(true);
                updateAlbumNav();
                // setTimeout(100) dropped — v2 maphilight handles idempotent re-init
                maphilight(imgEl, MAPHILIGHT_CONFIG);
            });
        }, { once: true });
    },
