...
open Gwdb
open Geneweb.Alln
open Geneweb.History
open Geneweb.UpdateIndOk
open Geneweb.Util

module Gutil = Gutil
module Gwdb = Gwdb
module Alln = Geneweb.Alln
module History = Geneweb.History
module UpdateIndOk = Geneweb.UpdateIndOk
...
let combine_by_ini ini list =
  ...
  Mutil.groupby
    ~key:(fun (_, s) -> Alln.ini len @@ V7_place.without_suburb s)
    ~value:(fun x -> x)
    list
...
let build_list conf base =
  ...
  if ini <> "" then
    Mutil.filter_map begin fun istr ->
      ...
      if Mutil.start_with_wildcard ini 0 @@ V7_place.without_suburb str
      then Some (istr, str)
      else None
    end list
  ...
  let rec build_ini l i =
    let inis =
      List.rev_map begin fun (_, s) ->
        let s = V7_place.without_suburb s in
