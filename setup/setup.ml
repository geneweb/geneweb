(* camlp4r *)
(* $Id: setup.ml,v 1.36 1999-08-05 13:34:15 ddr Exp $ *)
...
value gwf conf =
  ...
  if in_base = "" then print_file conf "err_miss.htm"
    ...
    in
    let (v1, v2, v3) =
      let v = get_all "enclosing_tag" in
      let v1 = if List.mem "strong" v then "on" else "" in
      let v2 = if List.mem "em" v then "on" else "" in
      let v3 =
        if List.mem "font size=-2" v then "s2"
        else if List.mem "font size=-1" v then "s1"
        else if List.mem "font size=+1" v then "b1"
        else if List.mem "font size=+2" v then "b2"
        else ""
      in
      (v1, v2, v3)
    in
    let conf =
      {(conf) with env =
         [("B", get "body_prop"); ("L", get "default_lang");
          ...
          ("R", get "renamed"); ("T", trailer);
          ("1", v1); ("2", v2); ("3", v3) :: conf.env]}
    in
...
value gwf_1 conf =
  ...
  do let oc = open_out (in_base ^ ".gwf") in
     ...
     do Printf.fprintf oc "# File generated by \"setup\"\n\n";
        ...
        if s_getenv conf.env "enclosing_tag_strong" = "on" then
          Printf.fprintf oc "enclosing_tag=strong\n"
        else ();
        if s_getenv conf.env "enclosing_tag_em" = "on" then
          Printf.fprintf oc "enclosing_tag=em\n"
        else ();
        match s_getenv conf.env "enclosing_tag_size" with
        [ "s2" -> Printf.fprintf oc "enclosing_tag=font size=-2\n"
        | "s1" -> Printf.fprintf oc "enclosing_tag=font size=-1\n"
        | "b1" -> Printf.fprintf oc "enclosing_tag=font size=+1\n"
        | "b2" -> Printf.fprintf oc "enclosing_tag=font size=+2\n"
        | _ -> () ];
...
value main () =
  do ifdef UNIX then intro ()
     ...
     Wserver.f 2316 0 None (None, None) None wrap_setup;
  return ()
;
