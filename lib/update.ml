...
let print_create_conflict conf base p var =
  ...
  aux conf.henv;
  aux conf.env;
  Util.hidden_input conf "field" (Mutil.encode var);
  ...
  |> Utf8.capitalize_fst
  |> Adef.encoded
  |> Util.hidden_input conf "create" ;
  Output.print_sstring conf {|<input type="submit" name="return" value="|} ;
  transl conf "back" |> Utf8.capitalize_fst |> Output.print_sstring conf;
