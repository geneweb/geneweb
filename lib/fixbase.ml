...
let fix_pevents ~report base pp =
  ...
  let empty_bi =
    (Geneweb_util.Date.cdate_None, Gwdb.empty_string, Gwdb.empty_string, Gwdb.empty_string)
  in
  let empty_bp =
    (Geneweb_util.Date.cdate_None, Gwdb.empty_string, Gwdb.empty_string, Gwdb.empty_string)
  in
  ...
  let aux name date place note src empty pevents =
    ...
    | Some e ->
        let e' =
          {
            ...
            epers_date =
              (if e.epers_date = Geneweb_util.Date.cdate_None then date else e.epers_date);
            epers_place =
        ...
        in
        (of_pevent e', Geneweb_util.Ext_list.replace e e' pevents)
  in
  ...
  let death =
    match p.death with
    | Death _ -> p.death
    | NotDead when death <> Geneweb_util.Date.cdate_None -> Death (Unspecified, death)
    | x -> x
  in
...
let fix_map_utf8_date ~report = function
  | Geneweb_util.Date.Dtext t ->
      let t' = Geneweb_util.Utf8.normalize t in
      if t <> t' then report ();
      Geneweb_util.Date.Dtext t'
  | d -> d
...
let fix_person_utf8_sequence ~report ~base ~person =
  ...
  let gen_pers' =
    Geneweb_util.Futil.map_person_ps ~fd:fix_map_date Fun.id fix_map_str gen_pers
  in
...
let fix_family_utf8_sequence ~report ~base ~family =
  ...
  let gen_fam' =
    Geneweb_util.Futil.map_family_ps ~fd:fix_map_date Fun.id Fun.id fix_map_str gen_fam
  in
...
let find_free_occ occ_set =
  let occ = ref 0 in
  let rec loop () =
    if Geneweb_util.Ext_int.Set.mem !occ occ_set then (
      incr occ;
      loop ())
    else
        ...
        let occ_set =
          List.fold_left
            (fun occ_set iper ->
              ...
              Geneweb_util.Ext_int.Set.add (Gwdb.get_occ p) occ_set)
            Geneweb_util.Ext_int.Set.empty ipers
        in
        ...
        let change, remaining_occ_set =
          List.fold_left
            (fun (change, remaining_occ_set) iper ->
              ...
              let set' = Geneweb_util.Ext_int.Set.remove occ remaining_occ_set in
            ...
            (false, occ_set) ipers
        in
        assert (Geneweb_util.Ext_int.Set.is_empty remaining_occ_set);
        change)
      else false
    else false
