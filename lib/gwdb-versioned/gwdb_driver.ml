...
(*  let genpers_with_witness_notes base genpers =
    let tbl = load_witness_notes base in
    let iper = genpers.Gwdb_legacy.Dbdisk.key_index in
    let witness_notes_opt = Hashtbl.find_opt tbl iper in
    match witness_notes_opt with
    | Some notes ->
       let pevents =
         List.mapi (fun i pe ->
             let witnesses = pe.Gwdb_legacy.Dbdisk.epers_witnesses in
             let witnesses = Array.map (fun (ip, wk) ->
                               ip, wk, empty_string) witnesses in
             let pe = Translate.legacy_to_def_pevent empty_string pe in
             { pe with Def.epers_witnesses = witnesses }
           ) genpers.Gwdb_legacy.Dbdisk.pevents
       in
       let genpers = Translate.legacy_to_def_person empty_string genpers in
       {genpers with pevents}
    | None -> assert false
 *)
