module type Driver_S = sig
  ...
  (** Open database associated with (likely situated in) the specified
      directory. *)
  ...
  (** Modify/add person with the giving id in the base. New names are added to
      the patched name index for the cosidered person and for evey member of
      family to which he belongs. Modification stay blocked until call of
      [commit_patches]. *)
  ...
  (** Modify/add family with a giving id. Modification stay blocked until call
      of [commit_patches]. *)
  ...
  (** Find if a given string already exists in the base and returns its istr if
      it does, None otherwise *)

  val insert_string : base -> string -> istr
  (** Modify/add string with a giving id. If string already exists return its
      id. Modification stay blocked until call of [commit_patches]. *)

  val commit_patches : base -> unit
  (** Commit blocked modifications (patches) and update database files in order
      to apply modifications on the disk. *)
  ...
  *)
  ...
  (** [base_visible_get base fct ip] get visibility of person [ip] ([true] for
      not visible (restrited)) from the [base]. If file {i restrict} is present
      then read it to get visibility information. If person's visibility isn't
      known, then set it with [fct]. Used when mode `use_restrict` is ativated
  *)
  ...
  (** Return regular expression that matches all defined in the [base]
      particles. *)

  val base_strings_of_first_name : base -> string -> istr list
  (** [base_strings_of_first_name base x] Return the list of first names (as
      [istr]) being equal or to [x] using {!val:Name.crush_lower} comparison.
      [x] could be also a substring of the matched first name. *)
  ...
  (** Load array of ascendants in the memory and cache it so it could be
      accessed instantly by other functions unless [clear_ascends_array] is
      called. *)
  ...
  (** Load array of descendants in the memory and cache it so it could be
      accessed instantly by other functions unless [clear_descends_array] is
      called. *)
  ...
  module Collection : sig
    ...
    val map : ('a -> 'b) -> 'a t -> 'b t
    (** [map fn c] Return a collection corresponding to [c] where [fn] would
        have been applied to each of its elements. *)
    ...
    *)
    ...
    (** [fold fn acc c] Combine each element of [c] into a single value using
        [fn]. [fn] first argument is the result computed so far as we traverse
        the collection, and second element is the current element being
        combined. [acc] is the starting combined value. Start at [from]-nth and
        finish with [until]-nth element (included). *)
  ...
  (** [dummy_collection x] create a dummy collection with no element. [x] is
  ...
  *)
  ...
  val sync :
    ?scratch:bool -> ?tasks:(unit -> unit) list -> save_mem:bool -> base -> unit
  (** [sync scratch base] Ensure that everything is synced on disk.
      ...
      Otherwise, only changes that occured since the last [sync] call are
      treated. *)
...
module Make (_ : DriverImplCompat) (_ : DriverImpl) : Driver_S
