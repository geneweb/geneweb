...
(* ic should be correctly placed at the start of the partition *)
...
(* All multi partition file starts with a master table of partitions storing their start offset
   in the file.
...
let partition_offset format base_path infos conv =
  ...
  if mult then
    let n = Format.ipart_of_infos infos in
    let offset = read_acc ic n in ic, offset
  else ic, 0
