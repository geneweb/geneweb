...
module Person = struct
  let access conf base p = Util.acces conf base p
  ...
  let birth_note conf base p = mk_person_note conf base p (get_birth_note p)
  ...
  let baptism_note conf base p = mk_person_note conf base p (get_baptism_note p)
  let burial p = get_burial p

  let burial_place conf base p =
    Util.string_of_place conf @@ sou base (get_burial_place p)

  let burial_note conf base p = mk_person_note conf base p (get_burial_note p)
  ...
  let death_note conf base p = mk_person_note conf base p (get_death_note p)
  let digest base p = Update.digest_person (UpdateInd.string_person_of base p)
  ...
  let first_name_key base p = code_varenv (Name.lower (p_first_name base p))
  let first_name_key_val base p = Name.lower (p_first_name base p)
  ...
  let image base p = sou base (get_image p)
  ...
  let notes conf base p =
    if not conf.no_note then mk_person_note conf base p (get_notes p) else ""

  let occ p = get_occ p
  let occupation conf base p = mk_note conf base [] (get_occupation p)
  ...
  let public_name base p = sou base (get_public_name p)

  let qualifier base p =
    match get_qualifiers p with nn :: _ -> sou base nn | _ -> ""

  let qualifiers base p = List.map (sou base) (get_qualifiers p)
  let sex p = index_of_sex (get_sex p)
  let psources base p = sou base (get_psources p)
  let surname base p = p_surname base p
  let surname_aliases base p = List.map (sou base) (get_surnames_aliases p)
  let surname_key base p = code_varenv (Name.lower (p_surname base p))
  let surname_key_val base p = Name.lower (p_surname base p)
  let titles p = get_titles p
end
...
module Family = struct
  ...
  let marriage_place base (_, fam, _, _) = sou base (get_marriage_place fam)

  let marriage_note conf base (_, fam, _, auth) =
    if auth then mk_note conf base [] (get_marriage_note fam) else ""

  let marriage_source base (_, fam, _, auth) =
    if auth then sou base (get_marriage_src fam) else ""
