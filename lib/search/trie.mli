module type S = sig
  ...
  (** [add w t v] inserts the word [w] with the value [v] into the trie [t]. If
      [w] alreadyt exists in [t], its value is updated to [v].

      This function is not tail recursive but the stack usage is bounded by the
  ...
  (** [fold f t acc] invokes [f] successively for every word in the trie [t] in
      lexicographic order. *)
