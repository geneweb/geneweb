...
let print_search_form conf from_note =
  ...
  hidden_env conf;
  Output.print_sstring conf {|<input type="hidden" name="m" value="MISC_NOTES_SEARCH">|};
...
let print_whole_notes conf base fnotes (title : Adef.safe_string) s ho =
  Hutil.header_no_page_title conf begin fun _ ->
    if (title :> string) = ""
    then Output.print_string conf (Util.escape_html fnotes)
  ...
  if (title :> string) <> "" then
    begin
      let title =
        match ho with
        | Some (case_sens, h) ->
          html_highlight case_sens h (title : Adef.safe_string :> string) |> Adef.safe
        | None -> title
      in
  ...
  let s =
    match ho with
      Some (case_sens, h) -> html_highlight case_sens h (s : Adef.safe_string :> string) |> Adef.safe
    | None -> s
  in
  ...
  List.iter begin fun pg ->
    ...
    begin match pg with
      ...
      | Def.NLDB.PgFam ifam ->
        ...
        Output.print_sstring conf "<span class=\"mx-2\">" ;
        Output.print_string conf (Util.referenced_person_title_text conf base fath) ;
        Output.print_string conf (DateDisplay.short_dates_text conf base fath) ;
        Output.print_sstring conf " &amp; " ;
        Output.print_string conf (Util.referenced_person_title_text conf base moth) ;
        Output.print_sstring conf " " ;
      ...
      | Def.NLDB.PgNotes ->
        ...
        if conf.wizard then begin
          Output.print_sstring conf {|<a class="mx-2" href="|} ;
          Output.print_string conf (commd conf) ;
          Output.print_sstring conf {|m=MOD_NOTES"><sup><i class="fa fa-cog"></i></sup></a>|}
        end ;
        Output.print_sstring conf "<a class=\"mx-2\" href=\"" ;
        Output.print_string conf (commd conf) ;
      ...
      | Def.NLDB.PgWizard wizname ->
        ...
        Output.print_sstring conf (transl_nth conf "wizard/wizards/friend/friends/exterior" 0) ;
...
let print_misc_notes conf base =
  ...
  let title h =
    Output.print_string conf begin
      if d = "" then
  ...
  let db =
    List.fold_right begin fun (f, _) list ->
      if String.length f >= String.length d then
        if String.sub f 0 (String.length d) = d then
          let r = String.sub f (String.length d) (String.length f - String.length d) in
          if d = "" || r <> "" && r.[0] = NotesLinks.char_dir_sep then
            let r = if d = "" then r else String.sub r 1 (String.length r - 1) in
            try
  ...
  if db <> [] then begin
    ...
    if d <> "" then begin
      ...
      begin match String.rindex_opt d NotesLinks.char_dir_sep with
        | Some i -> Output.print_string conf @@ "&d=" ^<^ Mutil.encode (String.sub d 0 i)
        | None -> ()
      end;
    ...
    List.iter begin function
      | (r, Some f) ->
        let txt =
          ...
          else "<em>" ^<^ (begin_text_without_html_tags 50 s |> Util.escape_html) ^>^ "</em>"
        in
        let c =
      ...
      | (r, None) ->
        ...
        if d = ""
        then Output.print_string conf (Mutil.encode r)
        else begin
...
let search_text conf base s =
  ...
  match noteo with
  | Some (fnotes, tit, nt) ->
    print_whole_notes conf base fnotes (Util.safe_html tit) nt (Some (case_sens, s))
  | None ->
    print_misc_notes conf base
