...
let pp_tm oc tm =
  Printf.fprintf oc "%04d-%02d-%02d %02d:%02d:%02d" (1900 + tm.Unix.tm_year)
...
let syslog (level : level) msg =
  let lvl =
    ...
    | `LOG_DEBUG -> 7
  in
  if Syslog.is_available && !syslog_flag then (
    let flags = if !debug_flag then [ `LOG_PERROR ] else [] in
    if !verbosity_level >= lvl then (
      let log =
  ...
  else if !verbosity_level >= lvl then (
