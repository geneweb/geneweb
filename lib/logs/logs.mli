...
type level =
  [ `LOG_EMERG
    (** A panic condition.
        Print if [!verbosity_level >= 0]. *)
  | `LOG_ALERT
    (** A condition that should be corrected immediately, such as a
        corrupted system database.
        Print if [!verbosity_level >= 1]. *)
  | `LOG_CRIT
    (** Critical conditions, such as hard device errors.
        Print if [!verbosity_level >= 2]. *)
  | `LOG_ERR  (** Errors.
        Print if [!verbosity_level >= 3]. *)
  | `LOG_WARNING
    (** Warning messages.
        Print if [!verbosity_level >= 4]. *)
  | `LOG_NOTICE
    (** Messages that contain information normally of use only when debugging
        a program.
        Print if [!verbosity_level >= 5]. *)
  | `LOG_INFO
    (** Informational messages.
        Print if [!verbosity_level >= 6]. *)
  | `LOG_DEBUG
    (** Conditions that are not error conditions, but that may require special
        handling.
        Print if [!verbosity_level >= 7]. *)
  ]
