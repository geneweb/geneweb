...
(** [bind t f] executes the parser [t]. If [t] succeeds, its result is passed
    to the function [f]. *)
...
(** [choice l] attemps to run the parsers in the list [l] in order. It
    succeeds with the result of the first parser that succeeds. *)
...
(** [option t] attempts to execute parser [t]. If [t] succeeds, its result
    is wrapped in [Some]. If [t] fails, [option t] succeeds and
    returns [None]. *)
...
(** [fix f init] constructs a recursive parser with states. *)

val skip : _ t -> unit t
(** [skip t] executes [t], consuming input as [t] woud, but discards
    its result. *)

val count : 'a t -> int t
(** [count t] executes [t] and returns the number of characters consumed
    by [t]. This combinator is more efficient than [List.length (many t)]. *)
