...
and eval_str_person_field conf base env ((p, p_auth) as ep) = function
  ...
  | "json_data" -> (
      match get_env "json_data" env with
...
let gen_interp_templ ?(no_headers = false) ?json_data ?output menu title
  ...
  let env =
    ...
    let open Ext_list.Infix in
    Option.map (fun data -> ("json_data", Vstring data)) json_data
    @?: [
          ("p", Vind p);
...
let interp_templ ?no_headers ?json_data ?output =
  gen_interp_templ ?no_headers ?json_data ?output false (fun _ -> ())
