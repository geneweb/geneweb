...
let get_burial_text conf p p_auth =
  ...
  let on_burial_date =
    ...
    | Buried cod ->
      begin match p_auth, Date.od_of_cdate cod with
        | true, Some d ->
          begin match List.assoc_opt "long_date" conf.base_env with
            | Some "yes" ->
              DateDisplay.string_of_ondate ~link:false conf d
...
and eval_simple_str_var conf base env (_, p_auth) =
  ...
  | "slash_marriage_date" ->
    begin match get_env "fam" env with
        Vfam (_, fam, _, m_auth) ->
        begin match m_auth, Date.od_of_cdate (get_marriage fam) with
...
and eval_person_field_var conf base env (p, p_auth as ep) loc =
  ...
  | "marriage_date" :: sl ->
      begin match get_env "fam" env with
      | Vfam (_, fam, _, true) ->
          begin match Date.od_of_cdate (get_marriage fam) with
          | Some d -> eval_date_field_var conf d sl
          | None -> null_val
          end
      | _ -> raise Not_found
  ...
  | "sosa_next" :: sl ->
      ...
      | Vsosa x ->
          begin match get_sosa conf base env x p with
          | Some (n, _) ->
              begin match SosaCache.next_sosa n with
              | (so, ip) ->
  ...
  | "sosa_prev" :: sl ->
      ...
      | Vsosa x ->
          begin match get_sosa conf base env x p with
          | Some (n, _) ->
              begin match SosaCache.prev_sosa n with
              | (so, ip) ->
...
and eval_str_event_field conf base (p, p_auth)
  ...
  | "date" ->
      if not p_auth then null_val else
      begin match Date.od_of_cdate date with
...
and eval_event_field_var conf base env (p, p_auth)
  ...
  function
    "date" :: sl ->
      begin match p_auth, Date.od_of_cdate date with
...
and eval_str_person_field conf base env (p, p_auth as ep) =
  ...
  | "slash_baptism_date" ->
    if p_auth
    then match Date.od_of_cdate (get_baptism p) with
  ...
  | "slash_birth_date" ->
      if p_auth then match Date.od_of_cdate (get_birth p) with
        | Some d -> DateDisplay.string_slash_of_date conf d |> safe_val
        | None -> null_val
      else null_val
