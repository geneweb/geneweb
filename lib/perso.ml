...
and eval_anc_paths_cnt conf base env (p, _) path_mode at_to ?(l1_l2 = (0, 0))
  ...
  | sl -> (
      ...
      | Vint lev -> (
          ...
          | Paths_cnt_raw -> (
              let list1 = Cousins.anc_cnt_aux base env lev at_to p in
              match list1 with
          ...
          | Paths_cnt -> (
              ...
              | None -> raise Not_found)
          | Paths -> (
              let l = Cousins.anc_cnt_aux base env lev at_to p in
              match l with
              | Some l -> (
...
(* TODO factorize eval_anc_paths_cnt and eval_desc_paths_cnt; why does does more on the " | Paths -> ... | None -> " branch *)
and eval_desc_paths_cnt conf base env (p, _) path_mode at_to ?(l1_l2 = (0, 0))
  ...
  | sl -> (
      ...
      | Vint lev -> (
          match path_mode with
          | Paths_cnt_raw -> (
              let list1 = Cousins.desc_cnt_aux base env lev at_to p in
              match list1 with
              | Some list1 -> VVstring (eval_int conf (List.length list1) sl)
              | None -> raise Not_found)
          | Paths_cnt -> (
              ...
              | None -> raise Not_found)
          | Paths -> (
              let l = Cousins.desc_cnt_aux base env lev at_to p in
              match l with
              | Some l -> (
                  match get_env "cousins" env with
                  | Vcousl cl ->
...
and eval_person_field_var conf base env ((p, p_auth) as ep) loc = function
  ...
  | [ "cous_paths_min_date"; l1; l2 ] ->
      Cousins.cous_paths_dates_aux conf base env p l1 l2 false
  | [ "cous_paths_max_date"; l1; l2 ] ->
      Cousins.cous_paths_dates_aux conf base env p l1 l2 true
  | [ "cous_paths_cnt_raw"; l1; l2 ] -> (
      let l = Cousins.cousins_l1_l2_aux conf base env l1 l2 p in
      match l with
  ...
  | [ "cous_paths"; l1; l2 ] -> (
      ...
      | None -> raise Not_found)
  | [ "cousins"; "max_a" ] ->
      let max_a, _ = Cousins.max_l1_l2 conf base env p in
      VVstring (string_of_int max_a)
  | [ "cousins"; "max_d" ] ->
      let _, max_d = Cousins.max_l1_l2 conf base env p in
...
let print_foreach conf base print_ast eval_expr =
  ...
  let print_foreach_descendant env al (p, _) count_paths =
    ...
    let ip_l =
      ...
      else Cousins.get_descendants_at_level base p lev
    in
    let rec loop i ip_l =
      match ip_l with
      | [] -> ()
  ...
  let print_foreach_cousin_path env el al ((p, _) as ep) in_or_less =
    ...
    let l =
      Cousins.cousins_l1_l2_aux conf base env (string_of_int l1)
        (string_of_int l2) p
    in
    match l with
    | Some l ->
