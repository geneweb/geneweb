...
let print conf base p =
  let title h =
    Output.print_sstring conf (Utf8.capitalize_fst (transl_decline conf "merge" ""));
    if not h then begin
  ...
  Output.print_sstring conf "<span class=\"form-row align-items-center\">\
                             <span class=\"col-auto\">\
  ...
  Output.print_sstring conf " ";
  Output.print_sstring conf (transl_nth conf "surname/surnames" 0);
  Output.print_sstring conf "\" size=\"50\" id=\"inlineinput\" autofocus>\
                             </span>\
                             </span>";
  if list <> [] then
    List.iter begin fun p ->
      Output.print_sstring conf "<div class=\"custom-control custom-radio\">";
      Output.print_sstring conf "<input type=\"radio\" class=\"custom-control-input\" \
                                 name=\"select\" id=\"";
  ...
  Output.print_sstring conf {|<button type="submit" class="btn btn-primary btn-lg mt-2">|};
            ...
            begin
              ...
              Output.print_sstring conf
                (transl_a_of_b conf
                   ...
                   (reference conf base p s :> string) (s :> string)
                ) ;
              Output.print_sstring conf {|)</p>|} ;
            end
      | None -> ()
