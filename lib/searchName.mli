...
val search_by_name :
  Config.config -> Gwdb.base -> string -> Authorized.Person.t list
...
val search_by_sosa :
  ...
  base:Gwdb.base ->
  sosa:Sosa.t ->
  Authorized.Person.t option

val search_by_key :
  Config.config -> Gwdb.base -> string -> Authorized.Person.t option
...
val print :
  Config.config ->
  Gwdb.base ->
  (Config.config -> Gwdb.base -> string -> Authorized.Person.t list -> unit) ->
...
val persons_starting_with :
  ...
  base:Gwdb.base ->
  filter:(Authorized.Person.t -> bool) ->
  first_name_prefix:string ->
  surname_prefix:string ->
  limit:int ->
  Gwdb.iper list

val search_by_sosa_in_env :
  Config.config -> Gwdb.base -> Authorized.Person.t option

val search_reject_p : Gwdb.base -> Authorized.Person.t -> bool
