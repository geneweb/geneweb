...
let print_place_list conf opt long link_to_ind max_rlm_nbr pl_l =
  ...
  let pl_l = if f_sort then
    List.sort
      (fun pl1 pl2 ->
        let ipl1 = List.fold_left (fun cnt (_, _, ipl) -> cnt + (List.length ipl)) 0 pl1 in
        let ipl2 = List.fold_left (fun cnt (_, _, ipl) -> cnt + (List.length ipl)) 0 pl2 in
        ...
        if link_to_ind && cnt < max_rlm_nbr then
          ...
          let rec loop3 cnt =
            ...
            | (so, _, ipl) :: t_pl ->
                ...
                List.iteri (fun i ip ->
                  Wserver.printf "&i%d=%d" (i + cnt) (Adef.int_of_iper ip))
                ipl ;
                loop3 (cnt + List.length ipl) t_pl
            | _ -> ()
          in loop3 0 pl ;
          Wserver.printf "\" title=\" %s\">%d</a>)"
            (capitale (transl conf "summary book ascendants")) cnt
          end
        else
...
let print_html_places_surnames conf base max_rlm_nbr link_to_ind
  ...
  let list = List.sort
      (fun ((_, pl1), _) ((_, pl2), _) -> sort_place_utf8 pl1 pl2) list
  in
