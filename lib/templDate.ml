...
let rec eval_date_var conf jd = function
  ...
  | [ "moon_age" ] -> (
      try
        ...
        TemplAst.VVstring (string_of_int md)
      with Failure _ -> TemplAst.VVstring "")
  | "moon_phase" :: sl -> (
      try
  ...
  | [ "week_day" ] ->
      let wday =
        let jd_today = Date.to_sdn ~from:Dgregorian conf.Config.today in
        let x = conf.Config.today_wd - jd_today + jd in
        if x < 0 then 6 + ((x + 1) mod 7) else x mod 7
      in
      TemplAst.VVstring (string_of_int wday)
...
and eval_moon_phase_var mp = function
  ...
  | [ "index" ] ->
      ...
      in
      TemplAst.VVstring (string_of_int i)
  | [ "minute" ] ->
      let s =
...
and eval_dmy_var dmy = function
  | [ "day" ] -> TemplAst.VVstring (string_of_int dmy.day)
  | [ "month" ] -> TemplAst.VVstring (string_of_int dmy.month)
  | "year" :: sl -> eval_integer dmy.year sl
  | [] ->
      TemplAst.VVstring
        (Printf.sprintf "%d-%02d-%02d" dmy.year dmy.month dmy.day)
  | _ -> raise Not_found

and eval_integer i = function
  | [ "roman" ] -> TemplAst.VVstring (Mutil.roman_of_arabian i)
  | [] -> TemplAst.VVstring (string_of_int i)
  | _ -> raise Not_found
