...
let map_pers_event ?(fd = identity) fp (fs : ?format:_ -> _) e =
  let epers_name =
...
let map_fam_event ?(fd = identity) fp (fs : ?format:_ -> _) e =
  let efam_name =
...
let map_person_ps ?(fd = identity) fp (fs : ?format:_ -> _) p =
  {
...
let map_family_ps ?(fd = identity) fp ff (fs : ?format:_ -> _) fam =
  {
