...
let date_interval conf base t x =
  ...
  let rec loop t x =
    let set d =
      if Geneweb_util.Date.compare_dmy d !d1 < 0 then d1 := d;
      if Geneweb_util.Date.compare_dmy d !d2 > 0 then d2 := d;
      found := true
    in
    Option.iter set (Geneweb_util.Date.cdate_to_dmy_opt (get_birth x));
    Option.iter set (Geneweb_util.Date.cdate_to_dmy_opt (get_baptism x));
    let death = get_death x in
    (match Geneweb_util.Date.dmy_of_death death with
    | Some d -> set d
    | None -> if death = NotDead then set conf.today);
    List.iter
      (fun t ->
        Option.iter set (Geneweb_util.Date.cdate_to_dmy_opt t.t_date_start);
        Option.iter set (Geneweb_util.Date.cdate_to_dmy_opt t.t_date_end))
    ...
    | AddSpouse | AddChildren ->
        ...
        Array.iter
          (fun ifam ->
            ...
            Option.iter set (Geneweb_util.Date.cdate_to_dmy_opt md);
...
let compare_title_dates conf base (x1, t1) (x2, t2) =
  match
    ( ( get_birth x1,
        Geneweb_util.Date.od_of_cdate t1.t_date_start,
        Geneweb_util.Date.od_of_cdate t1.t_date_end,
        get_death x1 ),
      ( get_birth x2,
        Geneweb_util.Date.od_of_cdate t2.t_date_start,
  ...
  | _ -> (
      ...
      | Some (d11, d12), Some (d21, d22) ->
          if Geneweb_util.Date.compare_dmy d12 d21 <= 0 then -1
          else if Geneweb_util.Date.compare_dmy d11 d22 >= 0 then 1
          else if Geneweb_util.Date.compare_dmy d21 d11 > 0 then -1
          else 1
      | _ -> (
          match
          ...
          | Some (d11, d12), Some (d21, d22) ->
              ...
              else if Geneweb_util.Date.compare_dmy d22 d12 >= 0 then -1
              else 1
          | _ -> (
              match
              ...
              with
              | Some (d11, d12), Some (d21, d22) ->
                  if Geneweb_util.Date.compare_dmy d21 d12 >= 0 then -1
                  else if Geneweb_util.Date.compare_dmy d11 d22 >= 0 then 1
                  else if Geneweb_util.Date.compare_dmy d22 d12 >= 0 then -1
                  else 1
              | Some _, None -> -1
...
let select_title_place conf base ~absolute title place =
  ...
  let title1 = Geneweb_util.Name.lower title in
  let place1 = Geneweb_util.Name.lower place in
  let select x t =
    if
      (absolute && sou base t.t_ident = title && sou base t.t_place = place)
      || (not absolute)
         && Geneweb_util.Name.lower (sou base t.t_ident) = title1
         && Geneweb_util.Name.lower (sou base t.t_place) = place1
    then (
...
let select_all_with_place conf base place =
  let list = ref [] in
  let clean_place = ref place in
  let place = Geneweb_util.Name.lower place in
  let select x t =
    if Geneweb_util.Name.lower (sou base t.t_place) = place then (
      clean_place := sou base t.t_place;
      list := (x, t) :: !list)
  in
  Gwdb.Collection.iter
...
let select_place conf base place =
  ...
  let place2 = Geneweb_util.Name.lower place in
  let add_title t =
    let pn = sou base t.t_place in
    if Geneweb_util.Name.lower pn = place2 then
      let tn = sou base t.t_ident in
