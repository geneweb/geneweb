...
let feed_file ctx path =
  ...
  let rec loop ctx ic =
    ...
    | read ->
        ...
        loop ctx ic
  in
  Compat.In_channel.with_open_bin path (loop ctx)

let checksum path =
  (* FIXME: As [Filesystem.walk_folder] does not walk directories in
     independent-platform order, we sort its output. *)
  let files =
    Filesystem.walk_folder ~recursive:true
      (fun e acc ->
        match e with
        | File f -> if Filename.extension f = ".cmxs" then f :: acc else acc
        | Dir _ -> acc
