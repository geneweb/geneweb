...
type patches =
  { mutable nb_per : int;
    ...
    nb_per_ini : int;
    nb_fam_ini : int;
    h_person : (int, (int, int, string) gen_person) Hashtbl.t;
    h_ascend : (int, int gen_ascend) Hashtbl.t;
    h_union : (int, int gen_union) Hashtbl.t;
    h_family : (int, (int, int, string) gen_family) Hashtbl.t;
    h_couple : (int, int gen_couple) Hashtbl.t;
    h_descend : (int, int gen_descend) Hashtbl.t;
    h_key : (string * string * int, int option) Hashtbl.t;
    h_name : (string, int list) Hashtbl.t }
...
type db2 =
  { phony : unit -> unit;
    ...
    patches : patches;
    mutable parents_array : int option array option;
    mutable consang_array : Adef.fix array option;
    mutable family_array : int array array option;
    mutable father_array : int array option;
    mutable mother_array : int array option;
    mutable children_array : int array array option }
...
let spi2gen_add pl db2 spi s =
  ...
  in
  Hashtbl.fold
    (fun _ p intl -> if proj p = s then p.key_index :: intl else intl)
...
let disk_person2_of_key db2 fn sn oc =
  ...
  try
    ...
    Some (key_hashtbl_find person_of_key_d "int_of_key.ht" key : int)
  with Not_found -> None
...
let load_couples_array2 db2 =
  ...
  | None ->
      let tab =
        load_array2 db2.bdir2 db2.patches.nb_fam_ini nb 0
          "family" "father"
          (fun ic_dat pos -> seek_in ic_dat pos; Iovalue.input ic_dat)
      in
      Hashtbl.iter (fun i c -> tab.(i) <- Adef.father c)
  ...
  | None ->
      let tab =
        load_array2 db2.bdir2 db2.patches.nb_fam_ini nb (0)
          ...
          (fun ic_dat pos -> seek_in ic_dat pos; Iovalue.input ic_dat)
      in
      Hashtbl.iter (fun i c -> tab.(i) <- Adef.mother c)
        db2.patches.h_couple;
      ...
      load_array2 db2.bdir2 nb_ini nb None "person" "parents"
        (fun ic_dat pos ->
           if pos = -1 then None
             ...
             begin seek_in ic_dat pos; Some (Iovalue.input ic_dat : int) end)
  in
  Hashtbl.iter (fun i a -> arr.(i) <- a.parents)
...
let consang_array2 db2 nb =
  ...
  in
  Hashtbl.iter (fun i a -> arr.(i) <- a.consang)
    db2.patches.h_ascend;
  arr
