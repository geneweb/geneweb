...
let search_by_name conf base n =
  ...
  | Some i ->
      ...
      let list, _ =
        Name_display.persons_of_fsname conf base Gwdb.base_strings_of_surname
...
let search conf base an search_order specify unknown =
  let rec loop l =
    ...
    | Surname :: l -> (
        let pl = Name_display.search_surname conf base an in
        match pl with
        | [] -> loop l
        | _ -> Name_display.search_surname_print conf base unknown an)
    | FirstName :: l -> (
        let pl = Name_display.search_first_name conf base an in
        match pl with
        | [] -> loop l
        | _ -> Name_display.search_first_name_print conf base an)
    | ApproxKey :: l -> (
    ...
    | PartialKey :: l -> (
        ...
        | pl -> specify conf base an pl)
    | DefaultSurname :: _ ->
        Name_display.search_surname_print conf base unknown an
  in
  loop search_order
