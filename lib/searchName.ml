...
let list_of_option = function Some p -> [ p ] | None -> []

let search_sosa conf base s =
  match Util.sosa_of_string s with
  | Some sosa -> list_of_option (search_by_sosa ~conf ~base ~sosa)
  | None -> []

let search_key conf base s = list_of_option (search_by_key conf base s)
