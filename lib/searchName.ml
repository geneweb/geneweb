...
let n_persons_of_stream n conf base filter stream =
  let rec consume n iperset =
    ...
    | Some (n, iperset) ->
        if n = 0 then IperSet.elements iperset
        else consume n iperset
    | None -> IperSet.elements iperset
  in
...
let persons_of_prefixes_stream max conf base filter fn_pfx sn_pfx =
      ...
      let rec aux n iperset ipers =
        if n = 0 then iperset
          ...
          | iper :: ipers ->
              ...
              then
                let iperset' = IperSet.add iper iperset in
  ...
  in
  IperSet.elements (consume max IperSet.empty)
