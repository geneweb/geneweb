...
(** Error that could occure while topological sorting, and raised when person
...
(** Returns result of topological sort of persons. Result is represented as
    marker that associates to every person in the base his topologic rank
    (let's suppose [r]). Global rule is : if person p1 is ancestor of p2 then
      r(p1) > r(p2). For example, all leaf persons (without children) have
      rank 0, their parents (if no another child that has child themself)
      - rank 1, parents of their
    parents - rank 2, etc.

    @raises TopologicalSortError if person is directly or undirectly is
            ancestor of himself (cycle). *)
...
(** Returns relationship rate between two person and common
    ancestors, if exists. *)
...
(** [check_noloop base onerror] scans database person's oriented graph (vertex
    ...
    with [OwnAncestor] error. Array of ascendants should be load in the
    memory. *)
