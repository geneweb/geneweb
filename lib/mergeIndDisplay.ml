...
let print_differences conf base branches p1 p2 =
  let gen_string_field chk1 chk2 (title : Adef.safe_string) (name : Adef.encoded_string) proj =
    ...
    then begin
      let aux i x chk =
        Output.print_sstring conf {|<div class="custom-control custom-radio ml-3">|} ;
        Output.print_sstring conf {|<input class="custom-control-input" type="radio" id="|} ;
  ...
  string_field (transl conf "death" |> Adef.safe) (Adef.encoded "death")
    (fun p ->
       let is = 2 in
  ...
  Output.print_sstring conf {|</p><p><button type="submit" class="btn btn-primary btn-lg">|};
...
let propose_merge_ind conf base branches p1 p2 =
  ...
  if branches <> [] then begin
    Output.print_sstring conf (Utf8.capitalize_fst (transl conf "you must first merge")) ;
  ...
  if branches <> [] then
    begin
      ...
      Output.print_sstring conf (Utf8.capitalize_fst (transl_nth conf "branch/branches" 1));
      Output.print_sstring conf (transl conf ":");
      Output.print_sstring conf "</p><table>";
      List.iter begin fun (ip1, ip2) ->
      ...
      end ((get_iper p1, get_iper p2) :: branches);
...
let same_person conf =
  ...
  Hutil.rheader conf title;
  Output.print_sstring conf (Utf8.capitalize_fst (transl conf "it is the same person!"));
  Hutil.trailer conf

let different_sexes conf base p1 p2 =
  let title _ = Output.print_sstring conf (Utf8.capitalize_fst (transl conf "error")) in
  Hutil.rheader conf title;
  Output.print_sstring conf (Utf8.capitalize_fst (transl conf "incompatible sexes"));
  Output.print_sstring conf (transl conf ":");
  Output.print_sstring conf {|<ul><li><a href="|};
  Output.print_string conf (commd conf);
  Output.print_string conf (acces conf base p1);
