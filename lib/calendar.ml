...
let modulo pAngle pVal =
  let x = truncate (pAngle /. pVal) in let y = float x *. pVal in pAngle -. y

let degVersRad pAngle =
  let a = modulo pAngle 360.0 in a *. 3.141592653589793 /. 180.0
...
let equinoxeAutomne pAnnee =
  ...
  let rec loop i jd jdn =
    if abs_float (jd -. jdn) > 1.0E-12 && i + 1 < 20 then
      ...
      let j = truncate jd in if d >= 0.5 then j + 1 else j
  in
  loop i jd (-1.0)
...
let french_of_sdn prec sdn =
  ...
  let (year, fst_vend_sdn) =
    if sdn < fst_vend_sdn then
      ...
      let year = greg_date.year - 1792 in year, fst_vend_sdn
    else let year = greg_date.year - 1791 in year, fst_vend_sdn
  in
...
let monthsPerYear =
  [| 12; 12; 13; 12; 12; 13; 12; 13; 12; 12; 13; 12; 12; 13; 12; 12; 13; 12;
     13 |]
...
let moladOfMetonicCycle metonicCycle =
  ...
  let pMoladHalakim = r1 in pMoladDay, pMoladHalakim
    ...
    if inputDay <= 0 then 0, 0, 0
      ...
      if inputDay >= tishri1 then
        ...
        if inputDay < tishri1 + 59 then
          ...
          let tishri1After =
            fTishri1 ((metonicYear + 1) mod 19) day halakim
          in
...
let rec conv f f_max_month g g_max_month d =
  let sdn =
    if d.day = 0 then
...
let affmoph i date_JJD leap_year first_moon_age_found month_day moon_age
    date =
...
let moon_phase_of_gregorian date =
  ...
  let tabm =
    [| 0.041; 0.126; 0.203; 0.288; 0.370; 0.455; 0.537; 0.622; 0.707; 0.789;
       0.874; 0.956 |]
  in
  let (year, date_month) =
  ...
  let ini_k =
    ...
    let k = float (truncate k) -. 0.25 in if k < 0.0 then k -. 1. else k
  in
