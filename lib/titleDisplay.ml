...
let give_access_someone conf base (x, t) list =
  ...
  begin match t.t_name, get_public_name x, get_qualifiers x with
    ...
    | Tname n, _, [] ->
      ...
      Output.print_string conf (escape_html @@ p_surname base x)
    | _ ->
      Output.print_string conf (gen_person_text conf base x)
  end;
  Output.print_sstring conf "\n";
  ...
  if t.t_nth <> 0 then begin
    Output.print_sstring conf " (" ;
    Output.print_sstring conf
      (if t.t_nth >= 100 then string_of_int t.t_nth else transl_nth conf "nth" t.t_nth) ;
    Output.print_sstring conf ")" ;
  end ;
...
let propose_tree_for_list list conf =
  let (list, _) =
    List.fold_left begin fun (list, n) (p, _) ->
  ...
  | _ :: _ :: _ as list ->
    ...
    ignore @@ List.fold_left begin fun i (p, n) ->
...
let print_title_place_list conf base t p t_equiv list =
  ...
  ignore @@ List.fold_left begin fun list x ->
...
let print_places_list conf base t t_equiv list =
  let title h =
    if h || List.length t_equiv = 1
    then Output.print_string conf (escape_html t)
    else
      Mutil.list_iter_first begin fun first t ->
...
let print_titles conf base p =
  ...
  if List.length list > 1 then begin
    ...
    Output.print_sstring conf {|">|};
    Output.print_sstring conf (Utf8.capitalize_fst (transl conf "the whole list"));
    Output.print_sstring conf "</a>"
  end;
  Hutil.trailer conf
...
let print_all_titles conf base =
  let title _ =
    Output.print_sstring conf (Utf8.capitalize_fst (transl conf "all the titles"))
  in
  let list =
...
let print_all_places conf base =
  let title _ =
    Output.print_sstring conf (Utf8.capitalize_fst (transl conf "all the estates"))
  in
  let list =
