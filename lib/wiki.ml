...
let make_edit_button conf fnotes ?(cnt=None) () =
  let href = Printf.sprintf "%sm=MOD_NOTES&f=%s%s"
    (commd conf :> string)
    (Mutil.encode fnotes :> string)
    (match cnt with None -> "" | Some n -> "&v=" ^ string_of_int n)
  in
  let title = match cnt with
    | None -> Utf8.capitalize_fst (transl conf "modify note")
    | Some n -> Utf8.capitalize_fst
      (Printf.sprintf (ftransl conf "modify note section %d") n)
  in
  Format.sprintf {|<a href="%s" class="align-self-center ml-3 mb-1"
  title="%s">(%s)</a>|} href title (transl conf "modify")
  ...
  | s :: sl -> (
      ...
      | None ->
          if len > 2 && s.[0] = '=' && s.[len - 1] = '=' then
            ...
            in
            let s =
            Format.asprintf "<div class=\"d-flex\"><h%d %s %s>%s%s</h%d>%s</div>"
              slev
...
let print_sub_part_text conf wi edit_opt cnt0 lines =
  ...
  let s =
    if cnt0 < first_cnt then modify_link conf 0 (s = "") edit_opt ^ s
    else s
  in
...
let print_mod_view_page conf can_edit mode fname title env s =
  ...
  if can_edit then (
    ...
    (if has_v then 
      Output.printf conf "&v=%d" v);
    Output.print_string conf sfn;
    Output.print_sstring conf {|" class="btn btn-sm btn-outline-primary align-self-center ml-3 mt-1">|};
  ...
  Util.include_template conf [ ("name", Adef.encoded "notes") ] "characters" ignore;
