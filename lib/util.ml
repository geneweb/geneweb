...
(* ************************************************************************ *)
(*  [Fonc] hash_file : string -> string option                              *)
(* ************************************************************************ *)

(** [Description] : calcule le hash MD5 d’un fichier
    [Args] :
      - file : chemin vers le fichier
    [Retour] : Some hash_hex si succès, None si erreur
    [Rem] : Utilise Digest.file pour éviter l’allocation mémoire complète   *)
...
(** [Description] : Trouve un fichier template selon la hiérarchie configurée
    [Args] :
      - conf : configuration de la base
      - fname : nom du fichier (ajout automatique de .txt)
    [Retour] : chemin complet vers le fichier trouvé
    [Rem] : Parcourt templates configurés -> base -> assets, comme un PATH    *)
...
(** [Description] : Résout le chemin d’un asset (CSS/JS) sans ajout de .txt
    [Args] :
      - conf : configuration de la base
      - fname : nom du fichier asset
    [Retour] : chemin complet vers l’asset
    [Rem] : Pour les assets non-template, pas de gestion des templates       *)
