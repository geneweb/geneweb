...
module type Gwdb_driver_S = sig
  ...
  val iper_marker : iper Common.Collection.t -> 'a -> (iper, 'a) Common.Marker.t

  (** [ifam_marker c v] create marker over collection of family's ids and initialise it
    for every element with [v] *)
  val ifam_marker : ifam Common.Collection.t -> 'a -> (ifam, 'a) Common.Marker.t
