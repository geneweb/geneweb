...
let pevent_of_form_pevent = function
  ...
  | FormBapt pe -> pe
  | FormPevent pe -> pe
  | FormDeath (death_status, pe) -> pe
  | FormBurial (burial_status, pe) -> pe
...
let infer_death birth bapt death_status burial_status burial_place
    death_pers_event =
...
let get_main_pevents ?(death_status = `DontKnowIfDead) pevents =
  ...
  let death = infer_death bi ba death_status burial_status bu.epers_place de in
  let burial_status =
...
let reconstitute_person conf =
  ...
  let death_status =
    infer_death birth bapt death_status burial_status burial.epers_place death
  in
  let burial_status =
    match burial_status with
...
let reconstitute_from_pevents pevents (_ext : bool) bi ba
    (death_status, _, _, _) (_burial_status, _, _, _) =
  let death_status =
    match death_status with
