...
let effective_del conf base _ip fam =
  ...
  History.record conf base (U_Delete_family (gen_p_of father, gen_fam)) "df";
  History.record conf base (U_Delete_family (gen_p_of mother, gen_fam)) "df";
  Array.iter
    (fun ip ->
      History.record conf base (U_Delete_family (gen_p_of ip, gen_fam)) "df")
    children
