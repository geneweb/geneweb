                  ...
                  tag "tr" begin
                    if k <> env_keys then do { 
                      ...
                      tag "td" "class=mod_places_table_modify" begin
                        stag "a" "style=\"font-size:80%%\" href=\"%sm=MOD_PLACE;%s;s=%s#mod\""
                      ...
                      tag "td" begin
                        ...
                        List.iter
                          ...
                          env_keys ;
                        xtag "input" "type=\"hidden\" name=\"m\" value=\"MOD_PLACE_OK\"" ;
...
value print_short conf base list len =
  ...
  do {
    ...
    tag "p" begin
      List.iter
        (fun s ->
          stagn "a" "href=\"%sm=MOD_PLACE;s=%s\"" (commd conf) (code_varenv s) begin
            Wserver.wprint "%s" s;
          end)
        list;
    end;
    ...
    (fun (old_place, perl) -> do {
      ...
      List.iter 
        (fun p -> do {
          ...
          let changed = U_Multi_places (Util.string_gen_person base np) in
          History.record conf base changed "cp"; } )
        perl } )
    list;
...
value print_mod_ok conf base = do {
  ...
  if nb_pers <> 0 && place_modified then do {
    ...
    tag "p" begin
      stag "a" "href=\"%sm=MOD_PLACE;s=%s\"" (commd conf) ini begin
