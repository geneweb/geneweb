...
(*  [Description] : Assainit une cha�ne de caract�res HTML en enlevant
                    les �l�ments dangereux.
    [Args] :
      - html_str : Cha�ne de caract�res � assainir.
    [Retour] : La cha�ne de caract�res assainie.                         *)
(* ********************************************************************* *)
value sanitize_html html_str =
  (* Enl�ve les �v�nements DOM. *)
...
value set_compilation_time s = compilation_time_hook.val := s;
value get_compilation_time () = compilation_time_hook.val;
value commit_hook = ref "";
value set_commit s = commit_hook.val := s;
value get_commit () = commit_hook.val;
value commit_date_hook = ref "";
value set_commit_date s = commit_date_hook.val := s;
value get_commit_date () = commit_date_hook.val;
...
value has_image conf base p =
  if not conf.no_image && authorized_age conf base p then
    ...
    (* Gestion des m�dias priv�s (contenant "private" dans leur URL) dans le cas du mode visiteur. *)
