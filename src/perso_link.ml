...
value make_efam_link conf base ip fam_link =
  ...
  let ifath = Adef.iper_of_int (Int32.to_int fam_link.MLink.Family.ifath) in
  let imoth = Adef.iper_of_int (Int32.to_int fam_link.MLink.Family.imoth) in
...
value get_children_of_parents base_prefix ifam ifath imoth =
  ...
  try
    let fam = Hashtbl.find Link.ht_family_cache (base_prefix, ifam) in
    if ifath = Adef.iper_of_int (Int32.to_int fam.MLink.Family.ifath) &&
       imoth = Adef.iper_of_int (Int32.to_int fam.MLink.Family.imoth)
    then
      List.fold_right
        (fun c accu ->
