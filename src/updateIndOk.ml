(* camlp4r ./pa_html.cmo *)
(* $Id: updateIndOk.ml,v 5.1 2005-12-20 20:04:26 ddr Exp $ *)
...
value all_checks_person conf base p a u =
  ...
  do {
    let _ : bool = Gutil.check_person base error warning p in
  ...
  try
    ...
    if ext || redisp then UpdateInd.print_update_ind conf base sp ""
      ...
      | None ->
          ...
          let k = (sp.first_name, sp.surname, sp.occ, p.cle_index) in
          do {
