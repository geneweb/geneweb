(* $Id: base64.ml,v 4.1 2001-04-22 17:50:34 ddr Exp $ *)
...
value decode s =
  ...
  do {
    while rpos.val < len do {
      ...
      res.[wpos.val] := Char.chr (i lsr 16);
...
value encode s =
  ...
  do {
    while rpos.val < len do {
      ...
      res.[wpos.val] := char64.(i lsr 18 land 0x3f);
    ...
    for i = 1 to len - origlen do { res.[String.length res - i] := '=' };
    res
  }
;
