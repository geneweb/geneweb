...
value decode s =
  ...
  do while rpos.val < len do
       ...
       do res.[wpos.val] := Char.chr (i lsr 16);
          res.[wpos.val + 1] := Char.chr (i lsr 8 land 0xFF);
          res.[wpos.val + 2] := Char.chr (i land 0xFF);
          rpos.val := rpos.val + 4;
       return wpos.val := wpos.val + 3;
     done;
  return
  let cut =
...
value char64 = Array.create 64 'a';
do for i = 0 to 25 do char64.(i) := Char.chr (Char.code 'A' + i); done;
...
value encode s =
  ...
  do while rpos.val < len do
       ...
       do res.[wpos.val] := char64.(i lsr 18 land 0x3f);
          res.[wpos.val + 1] := char64.(i lsr 12 land 0x3f);
          res.[wpos.val + 2] := char64.(i lsr 6 land 0x3f);
          res.[wpos.val + 3] := char64.(i land 0x3f);
          rpos.val := rpos.val + 3;
       return wpos.val := wpos.val + 4;
     done;
