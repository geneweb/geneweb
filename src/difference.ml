...
value diag fd bd sh xv yv xoff xlim yoff ylim = do {
  ...
  bd.(sh + bmid) := xlim;
  try
    let rec loop fmin fmax bmin bmax = do {
      let fmin =
      ...
      loop fmax where rec loop d =
        if d < fmin then ()
          ...
          let x =
            loop xv yv xlim ylim x (x - d) where rec loop
              xv yv xlim ylim x y =
      ...
      loop bmax where rec loop d =
        if d < bmin then ()
          ...
          let x =
            loop xv yv xoff yoff x (x - d) where rec loop
              xv yv xoff yoff x y =
...
value diff_loop a ai b bi n m = do {
  ...
  loop 0 n 0 m where rec loop xoff xlim yoff ylim =
    let (xoff, yoff) =
    ...
    in
    if xoff = xlim then
      for y = yoff to ylim - 1 do { chng2.(bi.(y)) := True }
