(* camlp4r *)
(* $Id: perso.ml,v 5.36 2006-11-11 08:03:38 ddr Exp $ *)
...
and eval_simple_bool_var conf base env (_, _, _, p_auth) =
  ...
  | "are_engaged" ->
      match get_env "fam" env with
      [ Vfam fam _ _ _ -> get_relation fam = Engaged
      | _ -> raise Not_found ]
  | "are_married" ->
      match get_env "fam" env with
      [ Vfam fam _ _ _ ->
  ...
  | "has_comment" ->
      match get_env "fam" env with
      [ Vfam fam _ _ m_auth -> m_auth && sou base (get_comment fam) <> ""
      | _ -> raise Not_found ]
  | "has_relation_her" ->
  ...
  | "has_witnesses" ->
      match get_env "fam" env with
      [ Vfam fam _ _ m_auth -> m_auth && Array.length (get_witnesses fam) > 0
      | _ -> raise Not_found ]
  | "is_first" ->
  ...
  | "is_no_mention" ->
      match get_env "fam" env with
      [ Vfam fam _ _ _ -> get_relation fam = NoMention
      | _ -> raise Not_found ]
  | "is_no_sexes_check" ->
      match get_env "fam" env with
      [ Vfam fam _ _ _ ->
...
and eval_simple_str_var conf base env (_, _, _, p_auth) =
 ...
 | "marriage_place" ->
  ...
  | "origin_file" ->
      if conf.wizard then
        match get_env "fam" env with
        [ Vfam fam _ _ _ -> sou base (get_origin_file fam)
        | _ -> "" ]
      else raise Not_found
  | "prefix_no_iz" ->
...
and eval_str_person_field conf base env ((p, a, u, p_auth) as ep) =
  ...
  | "prev_fam_index" ->
      match get_env "prev_fam" env with
      [ Vfam fam _ _ _ -> string_of_int (Adef.int_of_ifam (get_fam_index fam))
      | _ -> raise Not_found ]
  | "prev_fam_mother" ->
      match get_env "prev_fam" env with
...
and eval_str_family_field conf base env (f, _, _, _) loc =
  fun
  [ "index" -> string_of_int (Adef.int_of_ifam (get_fam_index f))
  | _ -> raise Not_found ]
...
value print conf base p =
  ...
  match passwd with
  [ Some (src, passwd)
    when is_that_user_and_password conf.ctime conf.auth_scheme "" passwd =
         False ->
