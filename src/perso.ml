(* camlp4r *)
(* $Id: perso.ml,v 5.40 2006-11-15 11:49:48 ddr Exp $ *)
...
and eval_simple_bool_var conf base env (_, _, _, p_auth) =
  ...
  | "are_engaged" ->
      match get_env "fam" env with
      [ Vfam _ fam _ _ _ -> get_relation fam = Engaged
      | _ -> raise Not_found ]
  | "are_married" ->
      match get_env "fam" env with
      [ Vfam _ fam _ _ _ ->
  ...
  | "has_comment" ->
      match get_env "fam" env with
      [ Vfam _ fam _ _ m_auth -> m_auth && sou base (get_comment fam) <> ""
      | _ -> raise Not_found ]
  | "has_relation_her" ->
  ...
  | "is_no_mention" ->
      match get_env "fam" env with
      [ Vfam _ fam _ _ _ -> get_relation fam = NoMention
      | _ -> raise Not_found ]
  | "is_no_sexes_check" ->
      match get_env "fam" env with
      [ Vfam _ fam _ _ _ ->
...
and eval_simple_str_var conf base env (_, _, _, p_auth) =
 ...
 | "marriage_place" ->
  ...
  | "origin_file" ->
      if conf.wizard then
        match get_env "fam" env with
        [ Vfam _ fam _ _ _ -> sou base (get_origin_file fam)
        | _ -> "" ]
      else raise Not_found
  | "prefix_no_iz" ->
...
and eval_compound_var conf base env ((_, a, _, _) as ep) loc =
  ...
  | ["family" :: sl] ->
      match get_env "fam" env with
      [ Vfam i f c d m ->
          eval_family_field_var conf base env (i, f, c, d, m) loc sl
      | _ -> raise Not_found ]
  | ["father" :: sl] ->
      match get_parents a with
      [ Some ifam ->
  ...
  | ["witness_relation" :: sl] ->
      match get_env "fam" env with
      [ Vfam i f c d m ->
          eval_witness_relation_var conf base env (i, f, c, d, m) loc sl
      | _ -> raise Not_found ]
...
and eval_cell_field_var conf base env ep cell loc =
  ...
  | ["family" :: sl] ->
      ...
      [ Cell p (Some ifam) _ _ _ ->
          let (f, c, d, a) = make_efam conf base (get_key_index p) ifam in
          eval_family_field_var conf base env (ifam, f, c, d, a) loc sl
      | _ -> VVstring "" ]
  | ["is_center"] ->
      match cell with
  ...
  [ ["family" :: sl] ->
      ...
      [ GP_person _ ip (Some ifam) ->
          ...
          let m_auth =
            ...
            authorized_age conf base (poi base imoth)
          in
          eval_family_field_var conf base env (ifam, f, c, d, m_auth) loc sl
      | _ -> raise Not_found ]
  | ["father" :: sl] ->
      match gp with
      [ GP_person _ ip _ ->
...
and eval_bool_person_field conf base env (p, a, u, p_auth) =
  ...
  | "has_children" ->
      match get_env "fam" env with
      [ Vfam _ _ _ des _ -> Array.length (get_children des) > 0
      | _ ->
          List.exists
            (fun ifam ->
  ...
  | "has_parents" -> get_parents a <> None
...
and eval_str_person_field conf base env ((p, a, u, p_auth) as ep) =
  ...
  | "consanguinity" ->
      if p_auth then
        string_of_decimal_num conf
          (round_2_dec (Adef.float_of_fix (get_consang a) *. 100.0)) ^ "%"
      else ""
  | "cremation_place" ->
      if p_auth then string_of_place conf base (get_burial_place p) else ""
  | "dates" ->
  ...
  | "nb_children" ->
      match get_env "fam" env with
      [ Vfam _ _ _ des _ -> string_of_int (Array.length (get_children des))
      | _ ->
          let n =
            List.fold_left
              (fun n ifam ->
  ...
  | "prev_fam_father" ->
      match get_env "prev_fam" env with
      [ Vfam _ fam (ifath, _, _) _ _ -> string_of_int (Adef.int_of_iper ifath)
      | _ -> raise Not_found ]
  | "prev_fam_index" ->
      match get_env "prev_fam" env with
      [ Vfam ifam _ _ _ _ -> string_of_int (Adef.int_of_ifam ifam)
      | _ -> raise Not_found ]
  | "prev_fam_mother" ->
      match get_env "prev_fam" env with
      [ Vfam _ fam (_, imoth, _) _ _ -> string_of_int (Adef.int_of_iper imoth)
      | _ -> raise Not_found ]
...
and eval_witness_relation_var conf base env
  ((_, fam, (ip1, ip2, _), _, m_auth) as fcd) loc =
  fun
  [ [] ->
...
and eval_family_field_var conf base env
  ((_, fam, (ifath, imoth, _), _, m_auth) as fcd) loc
=
  fun
  [ ["father" :: sl] ->
  ...
  | _ -> raise Not_found ]
and eval_str_family_field conf base env (ifam, _, _, _, _) loc =
  fun
...
and obsolete_eval conf base env (p, a, u, p_auth) loc =
  ...
  [ "married_to" ->
      let s =
        match get_env "fam" env with
        [ Vfam _ fam (_, _, ispouse) des m_auth ->
    ...
    let rec loop ((_, a, _, _) as ep) efam =
      ...
      | ["father" :: sl] ->
          ...
          [ Some ifam ->
              ...
              in
              let efam =
                Vfam ifam (foi base ifam) cpl (doi base ifam) m_auth
              in
              loop ep efam sl
          | None ->
    ...
    loop None 0 where rec loop prev i =
      if i = Array.length (get_family u) then ()
        ...
        let m_auth =
           ...
           authorized_age conf base (poi base imoth)
        in
        let vfam = Vfam ifam fam cpl des m_auth in
        let env = [("#loop", Vint 0) :: env] in
  ...
  and print_foreach_witness_relation env al ((p, _, _, _) as ep) =
    ...
    List.iter
      (fun (ifam, fam) ->
         ...
         in
         if m_auth then
           let env = [("fam", Vfam ifam fam cpl des True) :: env] in
           List.iter (print_ast env ep) al
         else ())
      list
  in
