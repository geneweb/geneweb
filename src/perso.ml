...
value print_what_links conf base p o_fn o_oc o_sn =
  if authorized_age conf base p then do {
    ...
    let old_pgl = links_to_ind conf base db old_key in
    let pub_nam = simple_person_text conf base p True in
    let title h = do {
      Wserver.printf "%s%s " (capitale (transl conf "linked pages")) (transl conf ":");
      if h then Wserver.printf "%s" pub_nam
      else
