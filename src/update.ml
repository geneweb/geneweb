...
(** [Description] : Renvoie le nom d'un individu, celui de ses parents ainsi
    que le nom de ses conjoints.
    [Args] :
...
(* ************************************************************************** *)
value print_person_parents_and_spouses conf base p =
  do {
    let print_person p link =
    do {
      if link then 
    ...
    in
    print_person p True;
    let son_of = Util.child_of_parent conf base p in
    if son_of != "" then 
        ...
        (fun (x, v) ->
           ...
           if x = "notes" || x = "comment" || is_label_note x then
             tag "textarea" "style=\"display:none;\" name=\"%s\"" x
               begin
                 Wserver.printf "%s" (quote_escaped (decode_varenv v));
               end
           else
...
value print_first_name_ref conf base p =
  Wserver.printf "%s" (someone_ref_text conf base p)
;
  ...
  if ml = [] then ()
    ...
    tag "ul" begin
      List.iter
        (fun m ->
          do {
            html_li conf; print_misc conf base m; Wserver.printf "\n"
          })
        ml;
    end
  }
