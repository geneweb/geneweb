"""
Function factory for pickle database.

Creates all the functions needed for PickleBaseFunc.
"""
...
class PickleFuncFactory:
    """Factory for creating pickle database functions."""
    ...
    def __init__(self, data: PickleBaseData):
        ...
        """
        self.data = data
        self.search_index = PickleSearchIndex()
        self.patches = PicklePatches(data)
    ...
    def _build_search_index(self) -> None:
        ...
        for iper, person in self.data.persons.items():
            ...
            if full_name and full_name != " ":
                ...
                self.search_index.add_person_name(person.first_name, iper)
                self.search_index.add_person_name(person.surname, iper)
    ...
    def create_strings_of_fname(self) -> Callable[[str], List[Istr]]:
        ...
        def strings_of_fname(fname: str) -> List[Istr]:
            return self.search_index.search_strings(fname)
        return strings_of_fname

    def create_strings_of_sname(self) -> Callable[[str], List[Istr]]:
        """Create strings_of_sname function."""
        def strings_of_sname(sname: str) -> List[Istr]:
            return self.search_index.search_strings(sname)
    ...
    def create_nb_of_real_persons(self) -> Callable[[], int]:
        ...
        def nb_of_real_persons() -> int:
            ...
            for person in self.data.persons.values():
                if person.first_name != "?" and person.surname != "?":
                    count += 1
            return count
