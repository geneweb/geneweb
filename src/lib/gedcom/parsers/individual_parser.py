...
from ..models import (
    GedcomIndividual, GedcomEvent, GedcomName, GedcomPlace,
    GedcomDate, GedcomAddress
)
...
class IndividualParser(RecordParser):
    ...
    def parse(self, lines: List[GedcomLine], start_index: int) -> tuple:
        ...
        if not start_line.xref_id:
            raise GedcomParseError(f"Individual record missing XREF at line {start_line.line_num}")
        ...
        while current_index < len(lines):
            ...
            if line.level == base_level + 1:
                if line.tag == 'NAME':
                    ...
                    current_attribute = None
                    self._assign_event_to_individual(individual, current_event, line.tag)
                    ...
                    if individual.source_citations and line.level > individual.source_citations[-1].sour_level:
                        last_citation = individual.source_citations[-1]
                        last_citation.sub_lines.append((line.level, line.tag, line.value))
                    else:
                ...
                elif individual.address and line.tag in ['ADR1', 'ADR2', 'CITY', 'STAE', 'POST', 'CTRY', 'PHON', 'EMAIL', 'FAX', 'WWW']:
                    self._parse_address_details(individual.address, line)
                elif line.tag in ['GIVN', 'SURN', 'NPFX', 'NSFX', 'NICK', 'ROMN', 'FONE', 'TRAN', 'TYPE'] and individual.names:
                ...
                if current_event and current_event.place and current_event.place.map and line.tag in ['LATI', 'LONG']:
                    if line.tag == 'LATI':
                        current_event.place.map.latitude = line.value
                    elif line.tag == 'LONG':
                        current_event.place.map.longitude = line.value
    ...
    def _get_individual_event_tags(self) -> List[str]:
        ...
        return [
            # Life events
            'BIRT', 'DEAT', 'BAPM', 'CHR', 'BURI', 'CREM',
            # Religious events
            'CONF', 'CHRA', 'BARM', 'BASM', 'BLES', 'ORDN',
            # Legal events
            'ADOP', 'NATU', 'PROB', 'WILL',
            # Migration events
            'EMIG', 'IMMI',
    ...
    def _assign_event_to_individual(self, individual: GedcomIndividual, event: GedcomEvent, tag: str):
    ...
    def _parse_individual_attribute(self, individual: GedcomIndividual, line: GedcomLine):
    ...
    def _parse_attribute_details(self, individual: GedcomIndividual, line: GedcomLine, current_attribute: str):
    ...
    def _parse_simple_date(self, date_str: str, date: Optional[GedcomDate] = None) -> GedcomDate:
    ...
    def _parse_place_geographic_details(self, place: GedcomPlace, line: GedcomLine) -> None:
