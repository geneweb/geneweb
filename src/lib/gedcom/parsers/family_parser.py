...
class FamilyParser(RecordParser):
    ...
    def parse(self, lines: List[GedcomLine], start_index: int) -> tuple:
        ...
        if not start_line.xref_id:
            raise GedcomParseError(f"Family record missing XREF at line {start_line.line_num}")
    ...
    def _assign_event_to_family(self, family: GedcomFamily, event: GedcomEvent, tag: str):
