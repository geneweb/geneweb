...
class GedcomIndividual:
    ...
    identification_numbers: List[Dict[str, str]] = field(default_factory=list)  # IDNO with TYPE
    ...
    def lifespan(self) -> str:
        ...
        death = str(self.death_year) if self.death_year else ("" if self.is_alive else "?")
...
class GedcomDatabase:
    ...
    sources: Dict[str, GedcomSource] = field(default_factory=dict)  # Changed to GedcomSource
    ...
    def get_individual(self, xref: str) -> Optional[GedcomIndividual]:
        ...
        return self.individuals.get(f'@{clean_xref}@') or self.individuals.get(clean_xref)
