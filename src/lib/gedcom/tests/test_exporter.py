...
from .. import (
    create_exporter, HeaderExporter, IndividualExporter,
    FamilyExporter, RawStructureExporter
)
from ..models import (
    GedcomDatabase, GedcomIndividual, GedcomFamily, GedcomHeader,
    GedcomEvent, GedcomName, GedcomDate, GedcomPlace
)
...
class TestGedcomExporter(unittest.TestCase):
    ...
    def test_round_trip(self):
        ...
        self.assertEqual(
            original_john.primary_name.full,
            reimported_john.primary_name.full
        )
    ...
    def test_multiline_text_export(self):
        ...
        long_note = "This is a very long note that should be split across multiple lines using CONC tags because it exceeds the GEDCOM line length limit of 248 characters. " * 3

        individual = GedcomIndividual(
            xref="@I1@",
            names=[GedcomName(full="Test /Person/")],
            notes=[long_note]
        )

        output = StringIO()
        self.exporter._export_records(output, GedcomDatabase(
            header=GedcomHeader(),
