...
class GedcomTokenizer:
    ...
    def tokenize(self, content: str) -> List[GedcomLine]:
        ...
        try:
            ...
            if self.errors:
                error_msg = f"Tokenization completed with {len(self.errors)} errors:\n" + "\n".join(self.errors[:5])
    ...
    def _preprocess_content(self, content: str) -> str:
        ...
        if content.startswith('\ufeff'):
            if self.preserve_bom:
                self.logger.debug("Found UTF-8 BOM - preserving for export but removing for parsing")
            else:
    ...
    def get_record_lines(self, lines: List[GedcomLine], start_line: GedcomLine) -> List[GedcomLine]:
...
def create_tokenizer(preserve_bom: bool = False, preserve_notes: bool = True) -> GedcomTokenizer:
    """
