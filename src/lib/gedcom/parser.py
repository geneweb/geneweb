...
from .tokenizer import GedcomTokenizer, GedcomLine
from .validators import GedcomValidator
from .exceptions import GedcomParseError
from .parsers import (
    FileReader, DefaultFileReader, RecordParser,
    HeaderParser, IndividualParser, FamilyParser,
    RepositoryParser, MultimediaParser, SubmitterParser, SubmitterParser,
...
class GedcomParser:
    ...
    def __init__(
        ...
        self.tokenizer = GedcomTokenizer(preserve_bom=preserve_bom, preserve_notes=preserve_notes)
    ...
    def parse_content(self, content: str) -> GedcomDatabase:
        ...
        try:
            ...
            self.logger.info(f"Successfully parsed {len(database.individuals)} individuals and {len(database.families)} families")
            return database
            ...
            if line.level == 0:
                ...
                if parser:
                    try:
                        ...
                        self._add_to_database(database, line.tag, line.xref_id, parsed_object)
                        current_index = next_index
                    except Exception as e:
                        self.logger.warning(f"Failed to parse {line.tag} record at line {line.line_num}: {e}")
                        current_index += 1
                elif line.tag == 'TRLR':
                    break
                else:
    ...
    def _ensure_stub_sources(self, database: GedcomDatabase) -> None:
        ...
        for xref in referenced:
            ...
            if key_with_at not in database.sources and key_without_at not in database.sources:
    ...
    def _add_to_database(self, database: GedcomDatabase, tag: str, xref_id: Optional[str], parsed_object):
