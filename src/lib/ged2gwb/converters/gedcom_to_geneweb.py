...
from ...db_pickle.models.family import GenFamily
from ...db_pickle.models.relations import GenCouple
from ...db_pickle.core.types import Iper, Ifam
from ...db_pickle.core.enums import Sex, RelationKind, DivorceStatus
from ...db_pickle.models.events import Date
...
class GedcomToGenewebConverter:
    ...
    def convert(self, gedcom_database: GedcomDatabase) -> Dict[str, Any]:
        ...
        """
        from ...db_pickle.database.base_data import PickleBaseData
        from ...db_pickle.models.relations import GenDescend
    ...
    def _convert_individual(self, individual) -> GenPerson:
        ...
        try:
            ...
            if hasattr(individual, 'sex') and individual.sex:
                sex_value = individual.sex.value if hasattr(individual.sex, 'value') else str(individual.sex)
                if sex_value == 'M':
                    sex = Sex.MALE
                elif sex_value == 'F':
                    sex = Sex.FEMALE
    ...
    def _convert_family(self, family) -> tuple[GenFamily, GenCouple, list]:
        ...
        from ...db_pickle.core.types import dummy_iper
        husband = dummy_iper()
        wife = dummy_iper()
    ...
    def _convert_date(self, gedcom_date) -> Date:
        ...
        try:
            year = int(gedcom_date.year) if gedcom_date.year and str(gedcom_date.year).strip() else 0
        except (ValueError, TypeError):
            year = 0

        try:
            month = int(gedcom_date.month) if gedcom_date.month and str(gedcom_date.month).strip() else 0
        except (ValueError, TypeError):
            month = 0

        try:
            day = int(gedcom_date.day) if gedcom_date.day and str(gedcom_date.day).strip() else 0
        except (ValueError, TypeError):
            day = 0
    ...
    def _print_warnings_and_errors(self):
        ...
        if self.warnings:
            print(f"\n{len(self.warnings)} warnings generated during conversion", file=sys.stderr)
        if self.errors:
            print(f"\n{len(self.errors)} errors generated during conversion", file=sys.stderr)
    ...
    def _validate_person_data(self, individual, person: GenPerson):
        ...
        if hasattr(individual, 'sex') and individual.sex:
            ...
            if sex_value == 'F' and person.sex == Sex.MALE:
                self._add_warning(f"Wife with male sex: {person.first_name} {person.surname}")
            elif sex_value == 'M' and person.sex == Sex.FEMALE:
                self._add_warning(f"Husband with female sex: {person.first_name} {person.surname}")

        # Check for death before baptism
        if person.death and person.baptism:
            if self._date_compare(person.death, person.baptism) < 0:
                self._add_warning(f"{person.first_name} {person.surname}'s death before his/her baptism")

        # Check for advanced age at death
        if person.birth and person.death:
            age = self._calculate_age(person.birth, person.death)
            if age > 100:
                self._add_warning(f"{person.first_name} {person.surname} died at the advanced age of {age} years old")
        ...
        if person.birth:
            ...
            if birth_year > 0:
                ...
                if current_year - birth_year > 70:
                    self._add_warning(f"{person.first_name} {person.surname} was parent at age of {current_year - birth_year}")
    ...
    def _check_children_birth_order(self, children):
        ...
        if len(children) > 5:
            self._add_warning(f"Family has {len(children)} children - check birth order")
