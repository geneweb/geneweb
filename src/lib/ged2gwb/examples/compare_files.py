...
def test_file(gedcom_path, file_name):
    ...
    print(f"   🔍 Search 'John': {len(geneweb_data.search_persons_by_first_name('John'))} results")
    print(f"   🔍 Search 'Smith': {len(geneweb_data.search_persons_by_surname('Smith'))} results")
    print(f"   🔍 Search 'Williams': {len(geneweb_data.search_persons_by_surname('Williams'))} results")
...
def main():
    ...
    if len(results) >= 2:
        ...
        print(f"   Individuals: {sample['individuals']:,} vs {uk['individuals']:,} ({uk['individuals']/sample['individuals']:.1f}x larger)")
        print(f"   Families: {sample['families']:,} vs {uk['families']:,} ({uk['families']/sample['families']:.1f}x larger)")
        print(f"   Sources: {sample['sources']:,} vs {uk['sources']:,} ({uk['sources']/sample['sources']:.1f}x larger)")

        print(f"\n⏱️  Performance Comparison:")
        print(f"   Parse time: {sample['parse_time']:.4f}s vs {uk['parse_time']:.4f}s ({uk['parse_time']/sample['parse_time']:.1f}x slower)")
        print(f"   Convert time: {sample['convert_time']:.4f}s vs {uk['convert_time']:.4f}s ({uk['convert_time']/sample['convert_time']:.1f}x slower)")
        print(f"   Total time: {sample['total_time']:.4f}s vs {uk['total_time']:.4f}s ({uk['total_time']/sample['total_time']:.1f}x slower)")

        print(f"\n🔍 Search Performance:")
        print(f"   1000 searches: {sample['search_time']:.4f}s vs {uk['search_time']:.4f}s")
        print(f"   Index build: {sample['index_time']:.4f}s vs {uk['index_time']:.4f}s")

        print(f"\n📈 Index Size:")
        print(f"   First names: {sample['first_names']} vs {uk['first_names']} ({uk['first_names']/sample['first_names']:.1f}x more)")
        print(f"   Surnames: {sample['surnames']} vs {uk['surnames']} ({uk['surnames']/sample['surnames']:.1f}x more)")
        ...
        print(f"   Time per person: {sample_per_person*1000:.2f}ms vs {uk_per_person*1000:.2f}ms")
        print(f"   Efficiency: {'Better' if uk_per_person < sample_per_person else 'Worse'} with larger files")
