"""
Base data structure for genealogical database.

Defines the data container for genealogical database (pickle and MessagePack).
"""
...
@dataclass
class BaseData:
    """Base data container for genealogical database."""
    ...
    def build_indexes(self, verbose: bool = False) -> None:
    ...
    def search_families_by_marriage_date(self, year: int) -> List[Ifam]:
        """Search families by marriage year."""
        results = []
        for family_id, family in self.families.items():
            # Check marriage date
            if family.marriage and family.marriage.year == year:
                results.append(family_id)
            # Also check events for marriage
            for event in family.events:
                if event.name == "MARR" and event.date and event.date.year == year:
                    results.append(family_id)
                    break
