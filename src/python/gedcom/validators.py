...
from .exceptions import GedcomSemanticError, GedcomStructureError
        ...
        for i, line in enumerate(lines):
            if line.level == 0:
                ...
                if line.tag not in self.valid_record_tags:
                    errors.append(
                        f"Line {line.line_num}: Unknown record type '{line.tag}'"
                    )

                # Validate XREF requirements
                if (
                    line.tag in {"INDI", "FAM", "NOTE", "SOUR", "REPO", "OBJE", "SUBM"}
                    and not line.xref_id
                ):
                    errors.append(
                        f"Line {line.line_num}: Record type '{line.tag}' requires XREF"
                    )

                if line.tag in {"HEAD", "TRLR", "SUBN"} and line.xref_id:
                    errors.append(
                        f"Line {line.line_num}: Record type '{line.tag}' should not have XREF"
                    )
        ...
        for line in lines:
            if line.xref_id:
                ...
                if line.xref_id in seen_xrefs:
                    errors.append(
                        f"Line {line.line_num}: Duplicate XREF '{line.xref_id}'"
                    )
                else:
        ...
        for xref, individual in database.individuals.items():
            ...
            for source_ref in individual.sources:
                if source_ref not in database.sources:
                    errors.append(
                        f"Individual {xref}: Invalid source reference '{source_ref}'"
                    )
        ...
        for xref, family in database.families.items():
            if family.husband:
                ...
                if husband_ref not in database.individuals:
                    errors.append(
                        f"Family {xref}: Invalid husband reference '{family.husband}'"
                    )
            ...
            if family.wife:
                ...
                if wife_ref not in database.individuals:
                    errors.append(
                        f"Family {xref}: Invalid wife reference '{family.wife}'"
                    )
        ...
        for family_xref, family in database.families.items():
            ...
            if family.husband:
                ...
                if husband and family_xref not in husband.fams:
                    errors.append(
                        f"Family {family_xref}: Husband {family.husband} doesn't reference this family"
                    )
            ...
            if family.wife:
                ...
                if wife and family_xref not in wife.fams:
                    errors.append(
                        f"Family {family_xref}: Wife {family.wife} doesn't reference this family"
                    )
            ...
            for child_xref in family.children:
                ...
                if child and family_xref not in child.famc:
                    errors.append(
                        f"Family {family_xref}: Child {child_xref} doesn't reference this family as FAMC"
                    )
    ...
    def _validate_dates(self, database: GedcomDatabase) -> List[str]:
        ...
        for xref, individual in database.individuals.items():
            ...
            if birth_year and death_year and birth_year > death_year:
                errors.append(
                    f"Individual {xref}: Birth year {birth_year} after death year {death_year}"
                )
            ...
            if birth_year and death_year:
                ...
                if age > 150:
                    errors.append(
                        f"Individual {xref}: Unrealistic lifespan of {age} years"
                    )
                elif age < 0:
    ...
    def _validate_individuals(self, database: GedcomDatabase) -> List[str]:
        ...
        for xref, individual in database.individuals.items():
            ...
            if individual.sex and individual.sex not in {"M", "F", "U"}:
                errors.append(
                    f"Individual {xref}: Invalid sex value '{individual.sex}'"
                )
...
class GedcomValidator:
    ...
    def validate_all(
        self, lines: List[GedcomLine], database: GedcomDatabase
    ) -> Dict[str, List[str]]:
        """
