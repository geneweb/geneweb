...
def main():
    ...
    for ifam, couple in geneweb_data.couples.items():
        ...
        if father and mother:
            print(
                f"   Family {ifam}: {father.first_name} {father.surname} + {mother.first_name} {mother.surname}"
            )
        elif father:
            print(
                f"   Family {ifam}: {father.first_name} {father.surname} + (unknown mother)"
            )
        elif mother:
            print(
                f"   Family {ifam}: (unknown father) + {mother.first_name} {mother.surname}"
            )
    ...
    print(
        f"   ⚡ Index size: {len(geneweb_data.first_name_index)} first names, {len(geneweb_data.surname_index)} surnames"
    )
    ...
    for name, count in sorted(
        first_name_counts.items(), key=lambda x: x[1], reverse=True
    )[:10]:
    ...
    print("\n   Most common surnames:")
    for name, count in sorted(surname_counts.items(), key=lambda x: x[1], reverse=True)[
        :10
    ]:
        print(f"     {name}: {count} persons")
