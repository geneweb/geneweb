...
def test_option_parsing():
    ...
    for i, test_case in enumerate(test_cases, 1):
        try:
            ...
            print(
                f"✅ Test {i:2d}: {' '.join(test_case[:3])}{'...' if len(test_case) > 3 else ''}"
            )
            passed += 1
        except SystemExit as e:
            print(
                f"❌ Test {i:2d}: {' '.join(test_case[:3])}{'...' if len(test_case) > 3 else ''} - Exit: {e.code}"
            )
            failed += 1
        except Exception as e:
            print(
                f"❌ Test {i:2d}: {' '.join(test_case[:3])}{'...' if len(test_case) > 3 else ''} - Error: {e}"
            )
            failed += 1
    ...
    test_cases = [
        {
            ...
            "expected": {"charset": "ANSEL", "udi": (70, 110), "fne": '""'},
        },
        {
            "args": ["sample.ged", "--dates-dm", "--efn", "--lf", "--compress"],
            "expected": {"dates_dm": True, "efn": True, "lf": True, "compress": True},
        },
    ]
        ...
        try:
            ...
            for key, expected_value in test_case["expected"].items():
                ...
                if actual_value != expected_value:
                    print(
                        f"❌ Test {i}: {key} = {actual_value}, expected {expected_value}"
                    )
                    all_correct = False
