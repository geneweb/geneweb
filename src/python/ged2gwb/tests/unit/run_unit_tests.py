...
def run_unit_tests():
    ...
    # Test results
    results = {"cli": False, "converter": False, "options": False}

    # Run CLI tests
    print("1. Running CLI Unit Tests...")
    print("-" * 30)
    try:
        from ged2gwb.tests.unit.test_cli import main as test_cli
    ...
    print("-" * 30)
    try:
        from ged2gwb.tests.unit.test_converter import main as test_converter
    ...
    print("-" * 30)
    try:
        from ged2gwb.tests.unit.test_options import main as test_options
    ...
    if passed == total:
        print(
            "SUCCESS: All unit tests passed! GED2GWB components are working correctly."
        )
        return 0
    else:
