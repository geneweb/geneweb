...
(* $Id: ascend.ml,v 4.6 2001-04-22 17:50:34 ddr Exp $ *)
...
value print_other_marriages conf base ws all_gp auth ifamo p u =
  if ws && (ifamo = None || Array.length u.family >= 2) then do {
...
value print_notes_ref conf base p wn n child_n =
  if wn && age_autorise conf base p && person_has_notes base p then do {
...
value print_other_families conf base wn all_gp excl_ifam moth_nb =
  ...
  do {
    print cpl.father (Num.sub moth_nb Num.one); print cpl.mother moth_nb;
  }
;
      ...
      do {
        ...
        [ Some ifam ->
            ...
            let prec =
              ...
              else let n1 = Num.twice n in (Some n1, Some (Num.inc n1 1))
            in
            match prec with
            [ (Some n1, Some n2) ->
                do {
...
value print_notes_for_someone conf base p n child_n =
  if age_autorise conf base p && person_has_notes base p then do {
...
value print_notes conf base all_gp ws =
  ...
  [ GP_person n ip ifamo ->
      do {
        print_notes_for_someone conf base (poi base ip) n "";
        match ifamo with
        [ Some ifam ->
        ...
        if Array.length u.family > 0 then
          ...
          Wserver.wprint "%s"
            (transl_decline2 conf "%1 of %2"
               ...
               (person_title_text conf base (poi base conj) ^
                  Date.short_dates_text conf base (poi base conj)))
        else do {
          afficher_personne_titre_referencee conf base p;
          Date.afficher_dates_courtes conf base p;
        };
...
value
  ...
  do {
    ...
    else print_spouses conf base p (uoi base p.cle_index);
  }
;
      ...
      List.fold_left
        (fun npl (n, p) ->
           ...
           [ (Some a1, Some a2) ->
               ...
               [ Some a ->
                   if a >= a1 && a <= a2 then [(n, p) :: npl] else npl
               | None -> npl ]
           | (Some a1, None) ->
               match one_year base p with
    ...
    do {
      ...
      stag "td" "align=center%s" (colspan po) begin
        let txt =
          match po with
          [ Empty -> "&nbsp;"
          | Cell p _ _ _ -> tree_reference gv conf base p "|" ]
        in
...
value print_horizontally conf base max_level p =
  ...
  do {
    ...
    tag "nobr" begin
      let suff13 = "&nbsp;&nbsp;&nbsp;" in
      let suff2 = "--&nbsp;" in loop 0 suff13 suff2 suff13 p.cle_index;
    end;
    trailer conf;
  }
;
...
value build_surnames_list conf base v p =
  ...
  let add_surname sosa p surn dp =
    let r =
      ...
      [ Not_found ->
          let r = ref ((fst dp, p), []) in do { Hashtbl.add ht surn r; r } ]
    in
